...
// The page object is also named RadiologyResultEntryAdminAuth — referenced by its fully-qualified name.

/**
 * Investigation &gt; Radiology &gt; <b>Result Entry Admin Auth</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Investigation</b> → <b>Radiology</b> → <b>Result Entry Admin Auth</b>.</li>
 ...
 * </ol>
 *
 * <p>The menu label is matched in full: Radiology also carries <b>Result Entry</b> and <b>Result Entry
 * Authentication</b>, and a loose match would open one of those instead.</p>
 *
 * <p>Pin values with {@code -Dfrom=}, {@code -Dto=}.</p>
 *
 * <p>Read-only apart from the report it prints: nothing is saved by this flow.</p>
 */
public class RadiologyResultEntryAdminAuth extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public RadiologyResultEntryAdminAuth() { super("Investigation_RadiologyResultEntryAdminAuth"); }

    public static void main(String[] args) {
        RadiologyResultEntryAdminAuth t = new RadiologyResultEntryAdminAuth();
    ...
    protected void body() {
        ...
        com.kpj.pages.Investigation_page.Radiology_page.RadiologyResultEntryAdminAuth re =
                new com.kpj.pages.Investigation_page.Radiology_page.RadiologyResultEntryAdminAuth(page);

        boolean rendered = re.navigateViaMenu(BASE);
        if (!rendered) addSummary("Radiology submenu offered", re.lastMenu);
        step(page, "Open Result Entry Admin Auth screen",
                "Click Investigation -> Radiology -> Result Entry Admin Auth",
                "The Result Entry Admin Auth screen is shown",
                rendered ? "Opened " + page.url()
                           + (re.lastRoute.isEmpty() ? "" : " (menu route " + re.lastRoute + ")")
                           + ". The label is matched in full — Radiology also has Result Entry and "
                           + "Result Entry Authentication."
        ...
        // Report tab — this screen carries a Test tab and a Report tab; the report button lives on the
        // latter.
        String tab = re.openTab("Report");
        boolean tabOpened = re.tabOpened();
        step(page, "Open the Report tab", "Open the Report tab, which carries the Result Entry Report",
                "The Report tab opens",
                (tabOpened ? "PASSES: " + tab : "FAILS: " + tab),
                tabOpened ? "PASS" : "FAIL");
