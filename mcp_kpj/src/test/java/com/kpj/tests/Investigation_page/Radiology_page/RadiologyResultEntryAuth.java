...
// The page object is also named RadiologyResultEntryAuth — referenced by its fully-qualified name.

/**
 * Investigation &gt; Radiology &gt; <b>Result Entry Authentication</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Investigation</b> → <b>Radiology</b> → <b>Result Entry Authentication</b>.</li>
 ...
 *   <li>Open the <b>Report</b> tab.</li>
 *   <li>Click <b>Result Entry Report</b>.</li>
 *   <li>Verify the report is generated — and carries content.</li>
 * </ol>
 *
 * <p>This is the same form as Result Entry — same models, same Search, same report button — reached by
 * a different menu entry ({@code #/ResultAuth}). The label is matched in full, since Radiology also has
 * plain <b>Result Entry</b> and <b>Result Entry Admin Auth</b>.</p>
 *
 * <p>Read-only apart from the report it prints: nothing is saved by this flow.</p>
 */
public class RadiologyResultEntryAuth extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public RadiologyResultEntryAuth() { super("Investigation_RadiologyResultEntryAuth"); }

    public static void main(String[] args) {
        RadiologyResultEntryAuth t = new RadiologyResultEntryAuth();
    ...
    protected void body() {
        ...
        com.kpj.pages.Investigation_page.Radiology_page.RadiologyResultEntryAuth re =
                new com.kpj.pages.Investigation_page.Radiology_page.RadiologyResultEntryAuth(page);

        boolean rendered = re.navigateViaMenu(BASE);
        if (!rendered) addSummary("Radiology submenu offered", re.lastMenu);
        step(page, "Open Result Entry Authentication screen",
                "Click Investigation -> Radiology -> Result Entry Authentication",
                "The Result Entry Authentication screen is shown",
                rendered ? "Opened " + page.url()
                           + (re.lastRoute.isEmpty() ? "" : " (menu route " + re.lastRoute + ")")
                           + ". The label is matched in full and the route compared whole — Radiology "
                           + "also has plain Result Entry (#/Result) and Result Entry Admin Auth, and "
                           + "this screen is the same form, so only the route tells them apart."
        ...
        step(page, "Click the tick symbol on one record",
                ...
                (ticked
                    ? "PASSES because the grid reports the row selected: " + tick
                      + ". The tick is ui-grid's selection button — the checkboxes in these rows are "
                      + "STATUS columns (Result Entry, Finalized) and clicking one selects nothing."
                    : "FAILS because the record did not select: " + tick),
                ticked ? "PASS" : "FAIL");

        // Report tab
        String tab = re.openTab("Report");
        boolean tabOk = re.tabOpened();
        step(page, "Open the Report tab", "Click the Report tab",
                "The Report tab is open",
                (tabOk ? "PASSES because the tab was clicked: " + tab
                       : "FAILS because the Report tab could not be opened: " + tab),
                tabOk ? "PASS" : "FAIL");
