...
// The page object is also named OperationStatus — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; OT Configuration &gt; <b>Operation Status</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>OT Configuration</b> → <b>Operation Status</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL operation status in the target environment.</p>
 */
public class OperationStatus extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public OperationStatus() { super("ApplicationConfiguration_OTConfiguration_OperationStatus"); }

    public static void main(String[] args) {
        OperationStatus t = new OperationStatus();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - OT Configuration - Operation Status", "Application Configuration > OT Configuration > Operation Status",
                "&#9888; Creates a REAL operation status: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "OPS" + stamp);
        String remark = System.getProperty("remark", "Auto operation status " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.OperationStatus tor =
                new com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.OperationStatus(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("OT Configuration submenu offered", tor.lastMenu);
        step(page, "Open Operation Status screen",
                "Click Application Configuration -> OT Configuration -> Operation Status",
                "The Operation Status screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.OperationStatus.isSuccess(toast);
        ...
        step(page, "Verify the operation status was created", "Look for " + code + " in the list",
                "The new operation status is listed",
                (inList
