...
// The page object is also named StockAdjustmentReasonMaster — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Inventory &gt; <b>Stock Adjustment Reason Master</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>Inventory</b> → <b>Stock Adjustment Reason Master</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL stock adjustment reason in the target environment.</p>
 */
public class StockAdjustmentReasonMaster extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public StockAdjustmentReasonMaster() { super("ApplicationConfiguration_Inventory_StockAdjustmentReasonMaster"); }

    public static void main(String[] args) {
        StockAdjustmentReasonMaster t = new StockAdjustmentReasonMaster();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Inventory - Stock Adjustment Reason Master", "Application Configuration > Inventory > Stock Adjustment Reason Master",
                "&#9888; Creates a REAL stock adjustment reason: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "SAR" + stamp);
        String remark = System.getProperty("remark", "Auto stock adjustment reason " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.Inventory_page.StockAdjustmentReasonMaster tor =
                new com.kpj.pages.ApplicationConfiguration_page.Inventory_page.StockAdjustmentReasonMaster(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("Inventory submenu offered", tor.lastMenu);
        step(page, "Open Stock Adjustment Reason Master screen",
                "Click Application Configuration -> Inventory -> Stock Adjustment Reason Master",
                "The Stock Adjustment Reason Master screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.Inventory_page.StockAdjustmentReasonMaster.isSuccess(toast);
        ...
        step(page, "Verify the stock adjustment reason was created", "Look for " + code + " in the list",
                "The new stock adjustment reason is listed",
                (inList
