...
// The page object is also named AppointmentCancellationReason — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Appointment Cancellation Reason</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Appointment Cancellation Reason</b>
 ...
 * </ol>
 */
public class AppointmentCancellationReason extends DevHisBase {
    ...
    public AppointmentCancellationReason() { super("ApplicationConfig_Patient_AppointmentCancellationReason"); }

    public static void main(String[] args) {
        AppointmentCancellationReason t = new AppointmentCancellationReason();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Appointment Cancellation Reason",
                "Application Configuration > Patient > Appointment Cancellation Reason",
                "Add an Appointment Cancellation Reason: enter Code + Remark, Submit; wait for the success "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.AppointmentCancellationReason acr =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.AppointmentCancellationReason(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = acr.navigateViaMenu();
        String landed = acr.currentScreen();
        step(page, "Open Appointment Cancellation Reason screen",
                "Application Configuration -> Patient -> Appointment Cancellation Reason",
                "The Appointment Cancellation Reason screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Appointment Cancellation Reason but the app opened: " + landed + "\n" + acr.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = acr.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = acr.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("AppointmentCancellationReason: filled-form screenshot failed - " + e.getMessage()); }
            toast = acr.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("AppointmentCancellationReason: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=AC") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + acr.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + acr.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (acr.toastPng != null && acr.toastPng.length > 0) {
            step(acr.toastPng, "Click Submit & success toast",
        ...
        addSummary("Appointment Cancellation Reason Code", acr.lastCode);
        addSummary("Remark", acr.lastRemark);
        addSummary("Field models", acr.lastCodeModel + " / " + acr.lastRemarkModel);
