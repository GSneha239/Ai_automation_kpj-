...
// The page object is also named BloodTestResult — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Blood Bank &gt; <b>Blood Test Result</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Blood Test Result</b> (clicking
 ...
 * </ol>
 */
public class BloodTestResult extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public BloodTestResult() { super("ApplicationConfig_BloodBank_BloodTestResult"); }

    public static void main(String[] args) {
        BloodTestResult t = new BloodTestResult();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Blood Bank - Blood Test Result",
                "Application Configuration > Blood Bank > Blood Test Result",
                "Add a Blood Test Result: enter Code + Remark, Submit; wait for the success toast. "
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = btr.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("BloodTestResult: filled-form screenshot failed - " + e.getMessage()); }
            toast = btr.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("BloodTestResult: attempt " + used + " (" + details + ") already exists — changing the details");
        }
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + btr.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + btr.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (btr.toastPng != null && btr.toastPng.length > 0) {
            step(btr.toastPng, "Click Submit & success toast",
        ...
        addSummary("Blood Test Result Code", btr.lastCode);
        addSummary("Remark", btr.lastRemark);
        addSummary("Field models", btr.lastCodeModel + " / " + btr.lastRemarkModel);
