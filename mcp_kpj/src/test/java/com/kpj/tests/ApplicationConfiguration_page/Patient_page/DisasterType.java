...
// The page object is also named DisasterType — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Disaster Type</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Disaster Type</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class DisasterType extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public DisasterType() { super("ApplicationConfig_Patient_DisasterType"); }

    public static void main(String[] args) {
        DisasterType t = new DisasterType();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Disaster Type",
                "Application Configuration > Patient > Disaster Type",
                "Add a Disaster Type: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.DisasterType dsy =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.DisasterType(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = dsy.navigateViaMenu();
        String landed = dsy.currentScreen();
        step(page, "Open Disaster Type screen", "Application Configuration -> Patient -> Disaster Type",
                "The Disaster Type screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Disaster Type but the app opened: " + landed + "\n" + dsy.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = dsy.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = dsy.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("DisasterType: filled-form screenshot failed - " + e.getMessage()); }
            toast = dsy.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("DisasterType: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=DS") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + dsy.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + dsy.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (dsy.toastPng != null && dsy.toastPng.length > 0) {
            step(dsy.toastPng, "Click Submit & success toast",
        ...
        addSummary("Disaster Type Code", dsy.lastCode);
        addSummary("Remark", dsy.lastRemark);
        addSummary("Field models", dsy.lastCodeModel + " / " + dsy.lastRemarkModel);
