...
// The page object is also named CustomField — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Custom Field</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Custom Field</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class CustomField extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public CustomField() { super("ApplicationConfig_Patient_CustomField"); }

    public static void main(String[] args) {
        CustomField t = new CustomField();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Custom Field",
                "Application Configuration > Patient > Custom Field",
                "Add a Custom Field: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.CustomField cf =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.CustomField(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = cf.navigateViaMenu();
        String landed = cf.currentScreen();
        step(page, "Open Custom Field screen", "Application Configuration -> Patient -> Custom Field",
                "The Custom Field screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Custom Field but the app opened: " + landed + "\n" + cf.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = cf.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = cf.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("CustomField: filled-form screenshot failed - " + e.getMessage()); }
            toast = cf.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("CustomField: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=CF") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + cf.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + cf.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (cf.toastPng != null && cf.toastPng.length > 0) {
            step(cf.toastPng, "Click Submit & success toast",
        ...
        addSummary("Custom Field Code", cf.lastCode);
        addSummary("Remark", cf.lastRemark);
        addSummary("Field models", cf.lastCodeModel + " / " + cf.lastRemarkModel);
