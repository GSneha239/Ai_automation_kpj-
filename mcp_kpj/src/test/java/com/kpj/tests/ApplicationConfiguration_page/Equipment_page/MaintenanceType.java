...
// The page object is also named MaintenanceType — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Equipment &gt; <b>Maintenance Type</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>Equipment</b> → <b>Maintenance Type</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL maintenance type in the target environment.</p>
 */
public class MaintenanceType extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public MaintenanceType() { super("ApplicationConfiguration_Equipment_MaintenanceType"); }
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Equipment - Maintenance Type", "Application Configuration > Equipment > Maintenance Type",
                "&#9888; Creates a REAL maintenance type: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "MNT" + stamp);
        String remark = System.getProperty("remark", "Auto maintenance type " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.Equipment_page.MaintenanceType tor =
                new com.kpj.pages.ApplicationConfiguration_page.Equipment_page.MaintenanceType(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("Equipment submenu offered", tor.lastMenu);
        step(page, "Open Maintenance Type screen",
                "Click Application Configuration -> Equipment -> Maintenance Type",
                "The Maintenance Type screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.Equipment_page.MaintenanceType.isSuccess(toast);
        ...
        step(page, "Verify the maintenance type was created", "Look for " + code + " in the list",
                "The new maintenance type is listed",
                (inList
