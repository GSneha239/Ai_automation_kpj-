...
// The page object is also named BloodGroup — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Blood Group</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Blood Group</b> (clicking <b>Add</b>
 *       first if the screen has one — a no-op on inline-add screens).</li>
 *   <li>Enter <b>Code*</b> and <b>Remark*</b> (a real blood group name — "A Positive", "O Negative", etc.).</li>
 ...
 * </ol>
 */
public class BloodGroup extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public BloodGroup() { super("ApplicationConfig_Patient_BloodGroup"); }

    public static void main(String[] args) {
        BloodGroup t = new BloodGroup();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Blood Group",
                "Application Configuration > Patient > Blood Group",
                "Add a Blood Group: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.BloodGroup bg =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.BloodGroup(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = bg.navigateViaMenu();
        String landed = bg.currentScreen();
        step(page, "Open Blood Group screen", "Application Configuration -> Patient -> Blood Group",
                "The Blood Group screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Blood Group but the app opened: " + landed + "\n" + bg.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = bg.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = bg.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("BloodGroup: filled-form screenshot failed - " + e.getMessage()); }
            toast = bg.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("BloodGroup: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=BG") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + bg.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + bg.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (bg.toastPng != null && bg.toastPng.length > 0) {
            step(bg.toastPng, "Click Submit & success toast",
        ...
        addSummary("Blood Group Code", bg.lastCode);
        addSummary("Remark", bg.lastRemark);
        addSummary("Field models", bg.lastCodeModel + " / " + bg.lastRemarkModel);
