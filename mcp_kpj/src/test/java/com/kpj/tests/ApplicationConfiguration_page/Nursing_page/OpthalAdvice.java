...
// The page object is also named OpthalAdvice — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Nursing &gt; <b>Opthal Advice</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Nursing</b> → <b>Opthal Advice</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class OpthalAdvice extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public OpthalAdvice() { super("ApplicationConfig_Nursing_OpthalAdvice"); }

    public static void main(String[] args) {
        OpthalAdvice t = new OpthalAdvice();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Nursing - Opthal Advice",
                "Application Configuration > Nursing > Opthal Advice",
                "Enter Code + Remark, click Submit; wait for the success toast. On 'already exists', change "
                        + "the details and Submit again. Any other toast is a FAIL.");

        LoginPage loginPage = new LoginPage(page);
        loginPage.login(BASE, USER, PASS);
        step("Login", USER + " login", "Authenticated; Patient Dashboard", "Logged in", "PASS");

        com.kpj.pages.ApplicationConfiguration_page.Nursing_page.OpthalAdvice oa =
                new com.kpj.pages.ApplicationConfiguration_page.Nursing_page.OpthalAdvice(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = oa.navigateViaMenu();
        String landed = oa.currentScreen();
        step(page, "Open Opthal Advice screen", "Application Configuration -> Nursing -> Opthal Advice",
                "The Opthal Advice screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Opthal Advice but the app opened: " + landed + "\n" + oa.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = oa.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = oa.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("OpthalAdvice: filled-form screenshot failed - " + e.getMessage()); }
            toast = oa.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message — stop either way
            System.out.println("OpthalAdvice: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=OA") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + oa.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Wrong/unexpected message - server returned: \"" + toast + "\"\nHTTP: " + oa.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (oa.toastPng != null && oa.toastPng.length > 0) {
            step(oa.toastPng, "Click Submit & success toast",
        ...
        addSummary("Opthal Advice Code", oa.lastCode);
        addSummary("Remark", oa.lastRemark);
        addSummary("Field models", oa.lastCodeModel + " / " + oa.lastRemarkModel);
