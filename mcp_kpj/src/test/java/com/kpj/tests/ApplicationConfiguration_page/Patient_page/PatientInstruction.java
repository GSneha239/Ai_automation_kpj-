...
// The page object is also named PatientInstruction — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Patient Instruction</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Patient Instruction</b> (clicking
 ...
 * </ol>
 */
public class PatientInstruction extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public PatientInstruction() { super("ApplicationConfig_Patient_PatientInstruction"); }

    public static void main(String[] args) {
        PatientInstruction t = new PatientInstruction();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Patient Instruction",
                "Application Configuration > Patient > Patient Instruction",
                "Add a Patient Instruction: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.PatientInstruction pi =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.PatientInstruction(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = pi.navigateViaMenu();
        String landed = pi.currentScreen();
        step(page, "Open Patient Instruction screen", "Application Configuration -> Patient -> Patient Instruction",
                "The Patient Instruction screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Patient Instruction but the app opened: " + landed + "\n" + pi.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = pi.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = pi.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("PatientInstruction: filled-form screenshot failed - " + e.getMessage()); }
            toast = pi.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("PatientInstruction: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=PI") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + pi.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + pi.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (pi.toastPng != null && pi.toastPng.length > 0) {
            step(pi.toastPng, "Click Submit & success toast",
        ...
        addSummary("Patient Instruction Code", pi.lastCode);
        addSummary("Remark", pi.lastRemark);
        addSummary("Field models", pi.lastCodeModel + " / " + pi.lastRemarkModel);
