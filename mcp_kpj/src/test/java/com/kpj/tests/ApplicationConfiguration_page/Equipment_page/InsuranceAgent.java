...
// The page object is also named InsuranceAgent — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Equipment &gt; <b>Insurance Agent</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>Equipment</b> → <b>Insurance Agent</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL insurance agent in the target environment.</p>
 */
public class InsuranceAgent extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public InsuranceAgent() { super("ApplicationConfiguration_Equipment_InsuranceAgent"); }

    public static void main(String[] args) {
        InsuranceAgent t = new InsuranceAgent();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Equipment - Insurance Agent", "Application Configuration > Equipment > Insurance Agent",
                "&#9888; Creates a REAL insurance agent: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "INS" + stamp);
        String remark = System.getProperty("remark", "Auto insurance agent " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.Equipment_page.InsuranceAgent tor =
                new com.kpj.pages.ApplicationConfiguration_page.Equipment_page.InsuranceAgent(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("Equipment submenu offered", tor.lastMenu);
        step(page, "Open Insurance Agent screen",
                "Click Application Configuration -> Equipment -> Insurance Agent",
                "The Insurance Agent screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.Equipment_page.InsuranceAgent.isSuccess(toast);
        ...
        step(page, "Verify the insurance agent was created", "Look for " + code + " in the list",
                "The new insurance agent is listed",
                (inList
