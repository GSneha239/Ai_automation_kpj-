...
// The page object is also named ChecklistCategoryMaster — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; OT Configuration &gt; <b>Checklist Category Master</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>OT Configuration</b> → <b>Checklist Category Master</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL checklist category in the target environment.</p>
 */
public class ChecklistCategoryMaster extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public ChecklistCategoryMaster() { super("ApplicationConfiguration_OTConfiguration_ChecklistCategoryMaster"); }

    public static void main(String[] args) {
        ChecklistCategoryMaster t = new ChecklistCategoryMaster();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - OT Configuration - Checklist Category Master", "Application Configuration > OT Configuration > Checklist Category Master",
                "&#9888; Creates a REAL checklist category: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "CCM" + stamp);
        String remark = System.getProperty("remark", "Auto checklist category " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.ChecklistCategoryMaster tor =
                new com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.ChecklistCategoryMaster(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("OT Configuration submenu offered", tor.lastMenu);
        step(page, "Open Checklist Category Master screen",
                "Click Application Configuration -> OT Configuration -> Checklist Category Master",
                "The Checklist Category Master screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.ChecklistCategoryMaster.isSuccess(toast);
        ...
        step(page, "Verify the checklist category was created", "Look for " + code + " in the list",
                "The new checklist category is listed",
                (inList
