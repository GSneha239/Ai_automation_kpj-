...
// The page object is also named CancellationReason — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Billing &gt; <b>Cancellation Reason</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Billing</b> → <b>Cancellation Reason</b> (clicking
 ...
 * </ol>
 */
public class CancellationReason extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public CancellationReason() { super("ApplicationConfig_Billing_CancellationReason"); }
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Billing - Cancellation Reason",
                "Application Configuration > Billing > Cancellation Reason",
                "Add a Cancellation Reason: enter Code + Remark, Submit; wait for the success toast. "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Billing_page.CancellationReason cr =
                new com.kpj.pages.ApplicationConfiguration_page.Billing_page.CancellationReason(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = cr.navigateViaMenu();
        String landed = cr.currentScreen();
        step(page, "Open Cancellation Reason screen", "Application Configuration -> Billing -> Cancellation Reason",
                "The Cancellation Reason screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Cancellation Reason but the app opened: " + landed + "\n" + cr.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = cr.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = cr.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("CancellationReason: filled-form screenshot failed - " + e.getMessage()); }
            toast = cr.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("CancellationReason: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=CR") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + cr.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + cr.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (cr.toastPng != null && cr.toastPng.length > 0) {
            step(cr.toastPng, "Click Submit & success toast",
        ...
        addSummary("Cancellation Reason Code", cr.lastCode);
        addSummary("Remark", cr.lastRemark);
        addSummary("Field models", cr.lastCodeModel + " / " + cr.lastRemarkModel);
