...
// The page object is also named ResultTemplate — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Investigation &gt; <b>Result Template</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Investigation</b> → <b>Result Template</b> (clicking
 ...
 * </ol>
 */
public class ResultTemplate extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public ResultTemplate() { super("ApplicationConfig_Investigation_ResultTemplate"); }

    public static void main(String[] args) {
        ResultTemplate t = new ResultTemplate();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Investigation - Result Template",
                "Application Configuration > Investigation > Result Template",
                "Add a Result Template: enter Code + Remark, Submit; wait for the success toast. On 'already "
                        + "exists', change the details and Submit again.");

        LoginPage loginPage = new LoginPage(page);
        loginPage.login(BASE, USER, PASS);
        step("Login", USER + " login", "Authenticated; Patient Dashboard", "Logged in", "PASS");

        com.kpj.pages.ApplicationConfiguration_page.Investigation_page.ResultTemplate rt =
                new com.kpj.pages.ApplicationConfiguration_page.Investigation_page.ResultTemplate(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = rt.navigateViaMenu();
        String landed = rt.currentScreen();
        step(page, "Open Result Template screen", "Application Configuration -> Investigation -> Result Template",
                "The Result Template screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Result Template but the app opened: " + landed + "\n" + rt.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = rt.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = rt.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("ResultTemplate: filled-form screenshot failed - " + e.getMessage()); }
            toast = rt.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("ResultTemplate: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=RT") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + rt.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + rt.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (rt.toastPng != null && rt.toastPng.length > 0) {
            step(rt.toastPng, "Click Submit & success toast",
        ...
        addSummary("Result Template Code", rt.lastCode);
        addSummary("Remark", rt.lastRemark);
        addSummary("Field models", rt.lastCodeModel + " / " + rt.lastRemarkModel);
