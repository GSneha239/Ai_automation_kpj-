...
// The page object is also named Religion — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Religion</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Religion</b> (clicking <b>Add</b> first if
 ...
 * </ol>
 */
public class Religion extends DevHisBase {

    /**
     * How many times to re-enter fresh details when the server says the code / remark already exists. Higher
     * than the simplest masters — core demographic masters (Nationality, Patient Relation, Language, Race)
     * turned out to be comprehensively pre-loaded.
     */
    private static final int MAX_ATTEMPTS = 40;

    public Religion() { super("ApplicationConfig_Patient_Religion"); }

    public static void main(String[] args) {
        Religion t = new Religion();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Religion",
                "Application Configuration > Patient > Religion",
                "Add a Religion: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.Religion rel =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.Religion(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = rel.navigateViaMenu();
        String landed = rel.currentScreen();
        step(page, "Open Religion screen", "Application Configuration -> Patient -> Religion",
                "The Religion screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Religion but the app opened: " + landed + "\n" + rel.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = rel.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = rel.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("Religion: filled-form screenshot failed - " + e.getMessage()); }
            toast = rel.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("Religion: attempt " + used + " (" + details + ") already exists — changing the details");
        }
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + rel.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + rel.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (rel.toastPng != null && rel.toastPng.length > 0) {
            step(rel.toastPng, "Click Submit & success toast",
        ...
        addSummary("Religion Code", rel.lastCode);
        addSummary("Remark", rel.lastRemark);
        addSummary("Field models", rel.lastCodeModel + " / " + rel.lastRemarkModel);
