...
// The page object is also named CCStatus — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Billing &gt; <b>CC Status</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Billing</b> → <b>CC Status</b> (clicking <b>Add</b> first
 ...
 * </ol>
 */
public class CCStatus extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public CCStatus() { super("ApplicationConfig_Billing_CCStatus"); }

    public static void main(String[] args) {
        CCStatus t = new CCStatus();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Billing - CC Status",
                "Application Configuration > Billing > CC Status",
                "Add a CC Status: enter Code + Remark, Submit; wait for the success toast. "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Billing_page.CCStatus ccs =
                new com.kpj.pages.ApplicationConfiguration_page.Billing_page.CCStatus(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = ccs.navigateViaMenu();
        String landed = ccs.currentScreen();
        step(page, "Open CC Status screen", "Application Configuration -> Billing -> CC Status",
                "The CC Status screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected CC Status but the app opened: " + landed + "\n" + ccs.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = ccs.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = ccs.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("CCStatus: filled-form screenshot failed - " + e.getMessage()); }
            toast = ccs.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("CCStatus: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=CS") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + ccs.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + ccs.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (ccs.toastPng != null && ccs.toastPng.length > 0) {
            step(ccs.toastPng, "Click Submit & success toast",
        ...
        addSummary("CC Status Code", ccs.lastCode);
        addSummary("Remark", ccs.lastRemark);
        addSummary("Field models", ccs.lastCodeModel + " / " + ccs.lastRemarkModel);
