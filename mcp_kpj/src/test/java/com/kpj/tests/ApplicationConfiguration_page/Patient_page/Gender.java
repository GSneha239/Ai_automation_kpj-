...
// The page object is also named Gender — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Gender</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Gender</b> (clicking <b>Add</b>
...
public class Gender extends DevHisBase {
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Gender",
                "Application Configuration > Patient > Gender",
                "Add a Gender: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.Gender gn =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.Gender(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = gn.navigateViaMenu();
        String landed = gn.currentScreen();
        step(page, "Open Gender screen", "Application Configuration -> Patient -> Gender",
                "The Gender screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Gender but the app opened: " + landed + "\n" + gn.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = gn.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = gn.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("Gender: filled-form screenshot failed - " + e.getMessage()); }
            toast = gn.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("Gender: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=GN") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + gn.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + gn.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (gn.toastPng != null && gn.toastPng.length > 0) {
            step(gn.toastPng, "Click Submit & success toast",
        ...
        addSummary("Gender Code", gn.lastCode);
        addSummary("Remark", gn.lastRemark);
        addSummary("Field models", gn.lastCodeModel + " / " + gn.lastRemarkModel);
