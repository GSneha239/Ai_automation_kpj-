...
// The page object is also named Triage — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Triage</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Triage</b> (clicking <b>Add</b> first if
 ...
 * </ol>
 */
public class Triage extends DevHisBase {
    ...
    public Triage() { super("ApplicationConfig_Patient_Triage"); }
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Triage",
                "Application Configuration > Patient > Triage",
                "Add a Triage: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.Triage tri =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.Triage(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = tri.navigateViaMenu();
        String landed = tri.currentScreen();
        step(page, "Open Triage screen", "Application Configuration -> Patient -> Triage",
                "The Triage screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Triage but the app opened: " + landed + "\n" + tri.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = tri.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = tri.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("Triage: filled-form screenshot failed - " + e.getMessage()); }
            toast = tri.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("Triage: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=TR") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + tri.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + tri.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (tri.toastPng != null && tri.toastPng.length > 0) {
            step(tri.toastPng, "Click Submit & success toast",
        ...
        addSummary("Triage Code", tri.lastCode);
        addSummary("Remark", tri.lastRemark);
        addSummary("Field models", tri.lastCodeModel + " / " + tri.lastRemarkModel);
