...
// The page object is also named IncomeCategory — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Income Category</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Income Category</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class IncomeCategory extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public IncomeCategory() { super("ApplicationConfig_Patient_IncomeCategory"); }

    public static void main(String[] args) {
        IncomeCategory t = new IncomeCategory();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Income Category",
                "Application Configuration > Patient > Income Category",
                "Add an Income Category: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.IncomeCategory ic =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.IncomeCategory(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = ic.navigateViaMenu();
        String landed = ic.currentScreen();
        step(page, "Open Income Category screen", "Application Configuration -> Patient -> Income Category",
                "The Income Category screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Income Category but the app opened: " + landed + "\n" + ic.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = ic.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = ic.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("IncomeCategory: filled-form screenshot failed - " + e.getMessage()); }
            toast = ic.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("IncomeCategory: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=IC") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + ic.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + ic.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (ic.toastPng != null && ic.toastPng.length > 0) {
            step(ic.toastPng, "Click Submit & success toast",
        ...
        addSummary("Income Category Code", ic.lastCode);
        addSummary("Remark", ic.lastRemark);
        addSummary("Field models", ic.lastCodeModel + " / " + ic.lastRemarkModel);
