...
// The page object is also named Qualification — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Qualification</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Qualification</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class Qualification extends DevHisBase {

    /**
     * How many times to re-enter fresh details when the server says the code / remark already exists. Higher
     * than the simplest masters — core demographic-adjacent masters like {@code Patient_page.Nationality} /
     * {@code Patient_page.PatientRelation} turned out to be comprehensively pre-loaded.
     */
    private static final int MAX_ATTEMPTS = 40;

    public Qualification() { super("ApplicationConfig_Patient_Qualification"); }

    public static void main(String[] args) {
        Qualification t = new Qualification();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Qualification",
                "Application Configuration > Patient > Qualification",
                "Add a Qualification: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.Qualification ql =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.Qualification(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = ql.navigateViaMenu();
        String landed = ql.currentScreen();
        step(page, "Open Qualification screen", "Application Configuration -> Patient -> Qualification",
                "The Qualification screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Qualification but the app opened: " + landed + "\n" + ql.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = ql.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = ql.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("Qualification: filled-form screenshot failed - " + e.getMessage()); }
            toast = ql.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("Qualification: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=QL") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + ql.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + ql.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (ql.toastPng != null && ql.toastPng.length > 0) {
            step(ql.toastPng, "Click Submit & success toast",
        ...
        addSummary("Qualification Code", ql.lastCode);
        addSummary("Remark", ql.lastRemark);
        addSummary("Field models", ql.lastCodeModel + " / " + ql.lastRemarkModel);
