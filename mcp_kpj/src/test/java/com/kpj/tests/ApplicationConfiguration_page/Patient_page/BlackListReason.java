...
// The page object is also named BlackListReason — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Black List Reason</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Black List Reason</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class BlackListReason extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public BlackListReason() { super("ApplicationConfig_Patient_BlackListReason"); }

    public static void main(String[] args) {
        BlackListReason t = new BlackListReason();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Black List Reason",
                "Application Configuration > Patient > Black List Reason",
                "Add a Black List Reason: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.BlackListReason blr =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.BlackListReason(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = blr.navigateViaMenu();
        String landed = blr.currentScreen();
        step(page, "Open Black List Reason screen", "Application Configuration -> Patient -> Black List Reason",
                "The Black List Reason screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Black List Reason but the app opened: " + landed + "\n" + blr.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = blr.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = blr.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("BlackListReason: filled-form screenshot failed - " + e.getMessage()); }
            toast = blr.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("BlackListReason: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=BL") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + blr.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + blr.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (blr.toastPng != null && blr.toastPng.length > 0) {
            step(blr.toastPng, "Click Submit & success toast",
        ...
        addSummary("Black List Reason Code", blr.lastCode);
        addSummary("Remark", blr.lastRemark);
        addSummary("Field models", blr.lastCodeModel + " / " + blr.lastRemarkModel);
