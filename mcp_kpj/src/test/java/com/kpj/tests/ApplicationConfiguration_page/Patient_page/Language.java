...
// The page object is also named Language — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Language</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Language</b> (clicking <b>Add</b>
...
public class Language extends DevHisBase {
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Language",
                "Application Configuration > Patient > Language",
                "Add a Language: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.Language lg =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.Language(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = lg.navigateViaMenu();
        String landed = lg.currentScreen();
        step(page, "Open Language screen", "Application Configuration -> Patient -> Language",
                "The Language screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Language but the app opened: " + landed + "\n" + lg.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = lg.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = lg.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("Language: filled-form screenshot failed - " + e.getMessage()); }
            toast = lg.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("Language: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=LG") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + lg.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + lg.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (lg.toastPng != null && lg.toastPng.length > 0) {
            step(lg.toastPng, "Click Submit & success toast",
        ...
        addSummary("Language Code", lg.lastCode);
        addSummary("Remark", lg.lastRemark);
        addSummary("Field models", lg.lastCodeModel + " / " + lg.lastRemarkModel);
