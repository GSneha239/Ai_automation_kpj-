...
// The page object is also named PurchaseExpense — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Inventory &gt; <b>Purchase Expense</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>Inventory</b> → <b>Purchase Expense</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL purchase expense in the target environment.</p>
 */
public class PurchaseExpense extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public PurchaseExpense() { super("ApplicationConfiguration_Inventory_PurchaseExpense"); }

    public static void main(String[] args) {
        PurchaseExpense t = new PurchaseExpense();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Inventory - Purchase Expense", "Application Configuration > Inventory > Purchase Expense",
                "&#9888; Creates a REAL purchase expense: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "PEX" + stamp);
        String remark = System.getProperty("remark", "Auto purchase expense " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.Inventory_page.PurchaseExpense tor =
                new com.kpj.pages.ApplicationConfiguration_page.Inventory_page.PurchaseExpense(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("Inventory submenu offered", tor.lastMenu);
        step(page, "Open Purchase Expense screen",
                "Click Application Configuration -> Inventory -> Purchase Expense",
                "The Purchase Expense screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.Inventory_page.PurchaseExpense.isSuccess(toast);
        ...
        step(page, "Verify the purchase expense was created", "Look for " + code + " in the list",
                "The new purchase expense is listed",
                (inList
