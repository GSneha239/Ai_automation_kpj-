...
// The page object is also named VipCategory — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>VIP Category</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>VIP Category</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class VipCategory extends DevHisBase {
    ...
    public VipCategory() { super("ApplicationConfig_Patient_VipCategory"); }

    public static void main(String[] args) {
        VipCategory t = new VipCategory();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - VIP Category",
                "Application Configuration > Patient > VIP Category",
                "Add a VIP Category: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.VipCategory vc =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.VipCategory(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = vc.navigateViaMenu();
        String landed = vc.currentScreen();
        step(page, "Open VIP Category screen", "Application Configuration -> Patient -> VIP Category",
                "The VIP Category screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected VIP Category but the app opened: " + landed + "\n" + vc.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = vc.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = vc.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("VipCategory: filled-form screenshot failed - " + e.getMessage()); }
            toast = vc.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("VipCategory: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=VC") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + vc.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + vc.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (vc.toastPng != null && vc.toastPng.length > 0) {
            step(vc.toastPng, "Click Submit & success toast",
        ...
        addSummary("VIP Category Code", vc.lastCode);
        addSummary("Remark", vc.lastRemark);
        addSummary("Field models", vc.lastCodeModel + " / " + vc.lastRemarkModel);
