...
// The page object is also named IntraOperativeInstructions — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; OT Configuration &gt; <b>Intra Operative Instructions</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>OT Configuration</b> → <b>Intra Operative Instructions</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL intra operative instruction in the target environment.</p>
 */
public class IntraOperativeInstructions extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public IntraOperativeInstructions() { super("ApplicationConfiguration_OTConfiguration_IntraOperativeInstructions"); }

    public static void main(String[] args) {
        IntraOperativeInstructions t = new IntraOperativeInstructions();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - OT Configuration - Intra Operative Instructions", "Application Configuration > OT Configuration > Intra Operative Instructions",
                "&#9888; Creates a REAL intra operative instruction: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "IOI" + stamp);
        String remark = System.getProperty("remark", "Auto intra operative instruction " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.IntraOperativeInstructions tor =
                new com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.IntraOperativeInstructions(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("OT Configuration submenu offered", tor.lastMenu);
        step(page, "Open Intra Operative Instructions screen",
                "Click Application Configuration -> OT Configuration -> Intra Operative Instructions",
                "The Intra Operative Instructions screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.IntraOperativeInstructions.isSuccess(toast);
        ...
        step(page, "Verify the intra operative instruction was created", "Look for " + code + " in the list",
                "The new intra operative instruction is listed",
                (inList
