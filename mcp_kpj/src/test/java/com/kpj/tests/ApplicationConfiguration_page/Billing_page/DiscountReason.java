...
// The page object is also named DiscountReason — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Billing &gt; <b>Discount Reason</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Billing</b> → <b>Discount Reason</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class DiscountReason extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public DiscountReason() { super("ApplicationConfig_Billing_DiscountReason"); }

    public static void main(String[] args) {
        DiscountReason t = new DiscountReason();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Billing - Discount Reason",
                "Application Configuration > Billing > Discount Reason",
                "Add a Discount Reason: enter Code + Remark, Submit; wait for the success toast. "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Billing_page.DiscountReason dr =
                new com.kpj.pages.ApplicationConfiguration_page.Billing_page.DiscountReason(page);
        ...
        step(page, "Open Discount Reason screen", "Application Configuration -> Billing -> Discount Reason",
                "The Discount Reason screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Discount Reason but the app opened: " + landed + "\n" + dr.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = dr.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = dr.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("DiscountReason: filled-form screenshot failed - " + e.getMessage()); }
            toast = dr.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("DiscountReason: attempt " + used + " (" + details + ") already exists — changing the details");
        }
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + dr.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + dr.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (dr.toastPng != null && dr.toastPng.length > 0) {
            step(dr.toastPng, "Click Submit & success toast",
        ...
        addSummary("Discount Reason Code", dr.lastCode);
        addSummary("Remark", dr.lastRemark);
        addSummary("Field models", dr.lastCodeModel + " / " + dr.lastRemarkModel);
