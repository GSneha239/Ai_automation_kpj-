...
// The page object is also named PreOperativeInstructions — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; OT Configuration &gt; <b>Pre Operative Instructions</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>OT Configuration</b> → <b>Pre Operative Instructions</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL pre operative instruction in the target environment.</p>
 */
public class PreOperativeInstructions extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public PreOperativeInstructions() { super("ApplicationConfiguration_OTConfiguration_PreOperativeInstructions"); }

    public static void main(String[] args) {
        PreOperativeInstructions t = new PreOperativeInstructions();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - OT Configuration - Pre Operative Instructions", "Application Configuration > OT Configuration > Pre Operative Instructions",
                "&#9888; Creates a REAL pre operative instruction: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "PRE" + stamp);
        String remark = System.getProperty("remark", "Auto pre operative instruction " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PreOperativeInstructions tor =
                new com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PreOperativeInstructions(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("OT Configuration submenu offered", tor.lastMenu);
        step(page, "Open Pre Operative Instructions screen",
                "Click Application Configuration -> OT Configuration -> Pre Operative Instructions",
                "The Pre Operative Instructions screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PreOperativeInstructions.isSuccess(toast);
        ...
        step(page, "Verify the pre operative instruction was created", "Look for " + code + " in the list",
                "The new pre operative instruction is listed",
                (inList
