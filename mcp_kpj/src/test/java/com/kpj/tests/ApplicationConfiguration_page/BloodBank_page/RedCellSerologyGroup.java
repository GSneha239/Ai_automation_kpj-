...
// The page object is also named RedCellSerologyGroup — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Blood Bank &gt; <b>Red Cell Serology Group</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Red Cell Serology Group</b> (clicking
 ...
 * </ol>
 */
public class RedCellSerologyGroup extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public RedCellSerologyGroup() { super("ApplicationConfig_BloodBank_RedCellSerologyGroup"); }

    public static void main(String[] args) {
        RedCellSerologyGroup t = new RedCellSerologyGroup();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Blood Bank - Red Cell Serology Group",
                "Application Configuration > Blood Bank > Red Cell Serology Group",
                "Add a Red Cell Serology Group: enter Code + Remark, Submit; wait for the success toast. "
        ...
        com.kpj.pages.ApplicationConfiguration_page.BloodBank_page.RedCellSerologyGroup rcs =
                new com.kpj.pages.ApplicationConfiguration_page.BloodBank_page.RedCellSerologyGroup(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = rcs.navigateViaMenu();
        String landed = rcs.currentScreen();
        step(page, "Open Red Cell Serology Group screen", "Application Configuration -> Blood Bank -> Red Cell Serology Group",
                "The Red Cell Serology Group screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Red Cell Serology Group but the app opened: " + landed + "\n" + rcs.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = rcs.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = rcs.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("RedCellSerologyGroup: filled-form screenshot failed - " + e.getMessage()); }
            toast = rcs.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("RedCellSerologyGroup: attempt " + used + " (" + details + ") already exists — changing the details");
        }
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + rcs.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + rcs.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (rcs.toastPng != null && rcs.toastPng.length > 0) {
            step(rcs.toastPng, "Click Submit & success toast",
        ...
        addSummary("Red Cell Serology Group Code", rcs.lastCode);
        addSummary("Remark", rcs.lastRemark);
        addSummary("Field models", rcs.lastCodeModel + " / " + rcs.lastRemarkModel);
