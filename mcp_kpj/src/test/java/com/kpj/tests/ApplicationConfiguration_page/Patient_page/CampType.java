...
// The page object is also named CampType — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Camp Type</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Camp Type</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class CampType extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public CampType() { super("ApplicationConfig_Patient_CampType"); }

    public static void main(String[] args) {
        CampType t = new CampType();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Camp Type",
                "Application Configuration > Patient > Camp Type",
                "Add a Camp Type: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.CampType ct =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.CampType(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = ct.navigateViaMenu();
        String landed = ct.currentScreen();
        step(page, "Open Camp Type screen", "Application Configuration -> Patient -> Camp Type",
                "The Camp Type screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Camp Type but the app opened: " + landed + "\n" + ct.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = ct.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = ct.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("CampType: filled-form screenshot failed - " + e.getMessage()); }
            toast = ct.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("CampType: attempt " + used + " (" + details + ") already exists — changing the details");
        }
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + ct.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + ct.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (ct.toastPng != null && ct.toastPng.length > 0) {
            step(ct.toastPng, "Click Submit & success toast",
        ...
        addSummary("Camp Type Code", ct.lastCode);
        addSummary("Remark", ct.lastRemark);
        addSummary("Field models", ct.lastCodeModel + " / " + ct.lastRemarkModel);
