...
// The page object is also named AllergyMaster — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Nursing &gt; <b>Allergy</b> ({@code #/Allergy}) — INLINE-ADD screen.
 *
 * <ol>
 ...
 * </ol>
 */
public class AllergyMaster extends DevHisBase {

    public AllergyMaster() { super("ApplicationConfig_Nursing_Allergy"); }

    public static void main(String[] args) {
        AllergyMaster t = new AllergyMaster();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Nursing - Allergy",
                "Application Configuration > Nursing > Allergy",
                "Add an Allergy (inline-add): enter Code + Remark, Submit; wait for the success toast.");

        LoginPage loginPage = new LoginPage(page);
        loginPage.login(BASE, USER, PASS);
        step("Login", USER + " login", "Authenticated; Patient Dashboard", "Logged in", "PASS");

        com.kpj.pages.ApplicationConfiguration_page.Nursing_page.AllergyMaster am =
                new com.kpj.pages.ApplicationConfiguration_page.Nursing_page.AllergyMaster(page);
        ...
        step(page, "Open Allergy screen", "Application Configuration -> Nursing -> Allergy",
                "The Allergy screen (inline-add form) is shown",
                on ? "Opened " + landed : "WRONG PAGE - expected Allergy but the app opened: " + landed,
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        String details = am.fillDetails();
        boolean detOk = details.contains("Code=AL") && !details.contains("Remark=(no)");
        ...
        addSummary("Allergy Code", am.lastCode);
        addSummary("Remark", am.lastRemark);
        addSummary("Route", "#/Allergy (inline-add)");
