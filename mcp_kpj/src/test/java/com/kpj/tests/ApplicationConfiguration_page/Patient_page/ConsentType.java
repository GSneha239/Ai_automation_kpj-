...
// The page object is also named ConsentType — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Consent Type</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Consent Type</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class ConsentType extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public ConsentType() { super("ApplicationConfig_Patient_ConsentType"); }
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Consent Type",
                "Application Configuration > Patient > Consent Type",
                "Add a Consent Type: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.ConsentType cty =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.ConsentType(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = cty.navigateViaMenu();
        String landed = cty.currentScreen();
        step(page, "Open Consent Type screen", "Application Configuration -> Patient -> Consent Type",
                "The Consent Type screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Consent Type but the app opened: " + landed + "\n" + cty.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = cty.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = cty.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("ConsentType: filled-form screenshot failed - " + e.getMessage()); }
            toast = cty.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("ConsentType: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=CY") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + cty.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + cty.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (cty.toastPng != null && cty.toastPng.length > 0) {
            step(cty.toastPng, "Click Submit & success toast",
        ...
        addSummary("Consent Type Code", cty.lastCode);
        addSummary("Remark", cty.lastRemark);
        addSummary("Field models", cty.lastCodeModel + " / " + cty.lastRemarkModel);
