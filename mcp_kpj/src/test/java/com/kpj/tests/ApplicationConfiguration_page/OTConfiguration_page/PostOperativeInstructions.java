...
// The page object is also named PostOperativeInstructions — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; OT Configuration &gt; <b>Post Operative Instructions</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>OT Configuration</b> → <b>Post Operative Instructions</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL post operative instruction in the target environment.</p>
 */
public class PostOperativeInstructions extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public PostOperativeInstructions() { super("ApplicationConfiguration_OTConfiguration_PostOperativeInstructions"); }

    public static void main(String[] args) {
        PostOperativeInstructions t = new PostOperativeInstructions();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - OT Configuration - Post Operative Instructions", "Application Configuration > OT Configuration > Post Operative Instructions",
                "&#9888; Creates a REAL post operative instruction: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "POI" + stamp);
        String remark = System.getProperty("remark", "Auto post operative instruction " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PostOperativeInstructions tor =
                new com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PostOperativeInstructions(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("OT Configuration submenu offered", tor.lastMenu);
        step(page, "Open Post Operative Instructions screen",
                "Click Application Configuration -> OT Configuration -> Post Operative Instructions",
                "The Post Operative Instructions screen is shown",
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.OTConfiguration_page.PostOperativeInstructions.isSuccess(toast);
        ...
        step(page, "Verify the post operative instruction was created", "Look for " + code + " in the list",
                "The new post operative instruction is listed",
                (inList
