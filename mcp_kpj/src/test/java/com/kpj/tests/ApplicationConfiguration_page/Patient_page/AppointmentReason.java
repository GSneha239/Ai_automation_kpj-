...
// The page object is also named AppointmentReason — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Patient &gt; <b>Appointment Reason</b>.
 *
 * <ol>
 *   <li>Open <b>Application Configuration</b> → <b>Patient</b> → <b>Appointment Reason</b> (clicking <b>Add</b>
 ...
 * </ol>
 */
public class AppointmentReason extends DevHisBase {

    /** How many times to re-enter fresh details when the server says the code / remark already exists. */
    private static final int MAX_ATTEMPTS = 40;

    public AppointmentReason() { super("ApplicationConfig_Patient_AppointmentReason"); }

    public static void main(String[] args) {
        AppointmentReason t = new AppointmentReason();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Patient - Appointment Reason",
                "Application Configuration > Patient > Appointment Reason",
                "Add an Appointment Reason: enter Code + Remark, Submit; wait for the success toast. On "
        ...
        com.kpj.pages.ApplicationConfiguration_page.Patient_page.AppointmentReason ar =
                new com.kpj.pages.ApplicationConfiguration_page.Patient_page.AppointmentReason(page);

        // 1) Navigate — if the app serves a different screen, FAIL and say so plainly.
        boolean on = ar.navigateViaMenu();
        String landed = ar.currentScreen();
        step(page, "Open Appointment Reason screen", "Application Configuration -> Patient -> Appointment Reason",
                "The Appointment Reason screen is shown",
                on ? "Opened " + landed
                   : "WRONG PAGE - expected Appointment Reason but the app opened: " + landed + "\n" + ar.describeForm(),
                on ? "PASS" : "FAIL");
        if (!on) { addSummary("Result", "FAILED - wrong page opened: " + landed); return; }

        // 2) Add, if this screen has one (inline-add screens already show the form).
        String addHow = ar.clickAddIfPresent();
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            details = ar.fillDetails(attempt);
            used++;
            ...
            catch (Exception e) { System.out.println("AppointmentReason: filled-form screenshot failed - " + e.getMessage()); }
            toast = ar.submitAndGetToast();
            String tl = toast == null ? "" : toast.toLowerCase();
            ok = tl.contains("saved") || tl.contains("success") || tl.contains("added") || tl.contains("updated");
            exists = tl.contains("exist") || tl.contains("already");
            if (ok || !exists) break;   // saved, or a different (non-retryable) message
            System.out.println("AppointmentReason: attempt " + used + " (" + details + ") already exists — changing the details");
        }

        boolean detOk = details.contains("Code=AR") && !details.contains("(no field)");
        ...
        String actual = toast == null || toast.isEmpty() ? "No toast appeared\n" + ar.describeForm()
                : (ok ? toast
                      : (exists ? "Still 'already exists' after " + used + " attempt(s) with different details: \"" + toast + "\""
                                : "Submit not confirmed - server returned: \"" + toast + "\"\nHTTP: " + ar.lastSaveHttp));
        String stepActual = (used > 1 && ok ? "(saved on attempt " + used + ") " : "") + actual;
        if (ar.toastPng != null && ar.toastPng.length > 0) {
            step(ar.toastPng, "Click Submit & success toast",
        ...
        addSummary("Appointment Reason Code", ar.lastCode);
        addSummary("Remark", ar.lastRemark);
        addSummary("Field models", ar.lastCodeModel + " / " + ar.lastRemarkModel);
