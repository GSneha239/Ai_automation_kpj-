package com.kpj.tests.ApplicationConfiguration_page.BioWaste_page;

import com.kpj.core.DevHisBase;
import com.kpj.pages.LoginPage;

// The page object is also named BioBagWaste — referenced by its fully-qualified name.

/**
 * Application Configuration &gt; Biowaste &gt; <b>BioBagWaste</b>.
 *
 * <ol>
 *   <li>Login (shared {@link LoginPage}, on an OPD counter so the menu exists).</li>
 *   <li>Open <b>Application Configuration</b> → <b>Biowaste</b> → <b>BioBagWaste</b>.</li>
 ...
 *
 * <p>&#9888; A successful run CREATES a REAL bio bag waste in the target environment.</p>
 */
public class BioBagWaste extends DevHisBase {

    /** DevHIS only builds the nav menu for an outpatient counter. Override with {@code -Dcounter=}. */
    public static final String COUNTER_FOR_MENU = "OPD-B-01";

    public BioBagWaste() { super("ApplicationConfiguration_Biowaste_BioBagWaste"); }

    public static void main(String[] args) {
        BioBagWaste t = new BioBagWaste();
    ...
    @Override
    protected void body() {
        meta("Application Configuration - Biowaste - BioBagWaste", "Application Configuration > Biowaste > BioBagWaste",
                "&#9888; Creates a REAL bio bag waste: enter Code and Remark, then Submit.");

        String counter = System.getProperty("counter", COUNTER_FOR_MENU);
        String stamp = String.format("%05d", Math.abs(System.nanoTime() % 100000));
        String code = System.getProperty("code", "BBW" + stamp);
        String remark = System.getProperty("remark", "Auto bio bag waste " + stamp);
        ...
        com.kpj.pages.ApplicationConfiguration_page.BioWaste_page.BioBagWaste tor =
                new com.kpj.pages.ApplicationConfiguration_page.BioWaste_page.BioBagWaste(page);

        // 1) Navigate
        boolean rendered = tor.navigateViaMenu(BASE);
        if (!rendered) addSummary("Biowaste submenu offered", tor.lastMenu);
        step(page, "Open BioBagWaste screen",
                "Click Application Configuration -> Biowaste -> BioBagWaste",
                "The BioBagWaste screen is shown",
                rendered ? "Opened " + page.url()
                           + (tor.lastRoute.isEmpty() ? "" : " (menu route " + tor.lastRoute + ")")
                         : "Did NOT reach the screen (" + page.url() + ") — see the Biowaste submenu dump",
                rendered ? "PASS" : "FAIL");
        ...
        boolean ok = com.kpj.pages.ApplicationConfiguration_page.BioWaste_page.BioBagWaste.isSuccess(toast);
        ...
        step(page, "Verify the bio bag waste was created", "Look for " + code + " in the list",
                "The new bio bag waste is listed",
                (inList
