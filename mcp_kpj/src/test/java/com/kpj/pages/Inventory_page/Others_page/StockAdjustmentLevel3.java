...
/**
 * Inventory &gt; Others &gt; <b>Stock Adjustment Level 3</b> — Page Object.
 *
 ...
 *
 * <p>Sibling of {@link StockAdjustmentLevel2} in the {@code Inventory_page.Others_page} submodule,
 * confirmed at route {@code #/StockAdjustmentLevel3}. Live-inspected before writing any selector and
 * confirmed to be STRUCTURALLY IDENTICAL to that sibling — same {@code StockAdjustment.*} ng-models, same
 * {@code getStockAdjustmentSearchList()}/{@code printReport()} handlers, same two {@code ui-grid}
 * components ({@code gridOptions}/{@code gridOptionsone}) — the only difference confirmed live is the
 * route and the server-side approval level context. Every finding on that sibling therefore applies
 * unchanged here:</p>
 * <ul>
 *   <li>The real "tick symbol" is {@code ui-grid}'s OWN row-selection icon
 *       ({@code ng-click="selectButtonClick(row, $event)"}, class {@code ui-grid-icon-ok}) — confirmed
 *       live it is what populates {@code gridOptionsone} and sets {@code StockAdjustment.stockadjid} —
 *       NOT the neighbouring {@code isapproved} checkbox column, which does nothing observable when
 *       clicked.</li>
 *   <li>{@code printReport()} opens a report tab only when {@code StockAdjustment.stockadjid} is set
 *       (i.e. a row was genuinely selected first); a real {@link com.microsoft.playwright.Locator#click}
 *       — as this class and {@link PdfReport} always use — is required for the resulting
 *       {@code window.open} to survive Chrome's popup blocker.</li>
 * </ul>
 ...
 */
public class StockAdjustmentLevel3 extends BasePage {

    public StockAdjustmentLevel3(Page page) { super(page); }
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*stock\\s*adjustment\\s*level\\s*3\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__sal3Menu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__sal3Menu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("StockAdjustmentLevel3.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__sal3Menu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3500);
        }
    ...
    public boolean onScreen() {
        ...
        if (route.contains("stockadjustmentlevel3")) return true;
    ...
    public String describeControls() {
        ...
        System.out.println("=== StockAdjustmentLevel3 CONTROLS ===\n" + s);
        return s;
    }
    ...
    public String enterDateRange(String from, String to) {
        ...
        System.out.println("StockAdjustmentLevel3: " + lastDateRange);
    ...
    public String clickSearch() {
        ...
        System.out.println("StockAdjustmentLevel3: " + lastSearch);
        return lastSearch;
    }
    ...
    public String clickFirstRowTick() {
        ...
        waitForAngular(2000);
        System.out.println("StockAdjustmentLevel3: " + lastRowTick);
        return lastRowTick;
    }
    ...
    public boolean detailsPopulated() {
        ...
        System.out.println("StockAdjustmentLevel3: " + lastDetailsCount);
        return n > 0;
    }
    ...
    public int clickPrint() {
        ...
        waitForAngular(1000);
        System.out.println("StockAdjustmentLevel3: " + lastPrint);
        return pagesBefore;
    }
