...
public class Emergency_Admission extends BasePage {
    ...
    public String fillAdmissionSection() {
        ...
        } catch (Exception ignore) { System.out.println("fillAdmissionDetails: doctor list not loaded in time"); com.kpj.core.Reasons.add("the Doctor dropdown did not load (no options loaded in time)"); }
        waitForAngular(600);
    ...
    public String saveAdmissionAndGetToast() {
        ...
        { String d = ensureDoctorSelected(); System.out.println("saveAdmission: re-assert " + d); if (d.contains("(no options)") || d.contains("(no select)")) com.kpj.core.Reasons.add("the Doctor dropdown is EMPTY (no options loaded) for the selected Department"); }
