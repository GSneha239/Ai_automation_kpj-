...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>OtTable</b> — Page Object.
 *
 ...
 */
public class OtTable extends BasePage {

    public OtTable(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; OtTable.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*ot\\s*table\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__otbMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("OtTable.nav: the OT Table link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__otbMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("OtTable.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__otbMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("OtTable.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the OtTable screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("ottable") || route.contains("otmaster")
                || route.contains("table")
    ...
    public String describeControls() {
        ...
        System.out.println("=== OtTable CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?ot|AddOtTable|OTTABLE/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__otbAdd'; }");
        try {
            page.locator("#__otbAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("OtTable.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__otbAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    // ---- OT Table's own fields -------------------------------------------

    public String lastTheatre = "", lastEntryPinned = "", lastSlot = "";
    ...
    /**
     * Select the <b>OT Theatre</b> ({@code OTTable.ottheatreid}).
     *
     * <p>Which theatre is used varies per run: a table's Code must be unique within its theatre, so
     * pinning one theatre would eventually collide with a table this suite created earlier.</p>
     */
    public String selectTheatre(int seed) {
        Object info = page.evaluate("() => {" + JS
                + " const e=[...document.querySelectorAll(\"select[ng-model='OTTable.ottheatreid']\")]"
                + "   .filter(vis)[0];"
                + " if(!e) return null; e.id='__otbTheatre';"
                + " const reals=[...e.options].map((o,i)=>({o,i}))"
                + "   .filter(x=>x.o.value && !/^-*\\s*select/i.test(norm(x.o.text)));"
                + " return {count:reals.length, indexes:reals.map(x=>x.i)}; }");
        if (info == null) { lastTheatre = "(no OT Theatre dropdown on this form)"; return lastTheatre; }
        @SuppressWarnings("unchecked")
        java.util.Map<String, Object> m = (java.util.Map<String, Object>) info;
        java.util.List<?> idx = (java.util.List<?>) m.get("indexes");
        if (idx.isEmpty()) { lastTheatre = "(the OT Theatre list has no real option)"; return lastTheatre; }
        int pick = ((Number) idx.get(Math.floorMod(seed, idx.size()))).intValue();
        try {
            page.locator("#__otbTheatre").selectOption(
                    new com.microsoft.playwright.options.SelectOption().setIndex(pick));
        } catch (Exception e) {
            lastTheatre = "(select failed: " + e.getMessage().split("\n")[0] + ")";
            return lastTheatre;
        }
        waitForAngular(800);
        Object t = page.evaluate("() => { const e=document.getElementById('__otbTheatre');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastTheatre = (t == null ? "" : t.toString()) + " [OTTable.ottheatreid, "
                + m.get("count") + " theatres]";
        System.out.println("OtTable: theatre -> " + lastTheatre);
        return lastTheatre;
    }

    public boolean theatreSelected() { return chosen(lastTheatre); }

    /** Enter the <b>Code</b> and <b>Remark</b>, each pinned to its exact model. */
    public String enterCodeAndRemark(String code, String remark) {
        String r1 = put("OTTable.code", code);
        String r2 = put("OTTable.description", remark);
        lastEntryPinned = "Code=" + r1 + " | Remark=" + r2;
        waitForAngular(500);
        System.out.println("OtTable: " + lastEntryPinned);
    ...
    /** Enter the <b>OT Slot</b> ({@code OTTable.otslot}) — a number field. */
    public String enterOtSlot(String slot) {
        lastSlot = put("OTTable.otslot", slot);
        waitForAngular(400);
        System.out.println("OtTable: OT slot -> " + lastSlot);
        return lastSlot;
    }

    public boolean otSlotEntered(String slot) {
        return lastSlot != null && !lastSlot.startsWith("(no field") && lastSlot.startsWith(slot);
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("OtTable: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("OtTable: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__otbSeen = [];"
                    + " if(window.__otbObs) window.__otbObs.disconnect();"
                    + " window.__otbObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__otbSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__otbObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__otbSeen||[];"
                    + " if(window.__otbObs) window.__otbObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("OtTable: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("OtTable: list check -> " + lastListCheck);
