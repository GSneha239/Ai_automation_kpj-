 ...
 */
public class ComplaintCategory extends BasePage {

    public ComplaintCategory(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Equipment &rarr; ComplaintCategory.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*complaint\\s*category\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__ccMenu'; return a.getAttribute('href')||''; }");
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("ComplaintCategory.nav: " + e.getMessage().split("\n")[0]); }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ComplaintCategory.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ComplaintCategory screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("complaintcategory") || route.contains("complaintcat")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ComplaintCategory CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?complaint|AddComplaintCategory|COMPLAINTCAT/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__ccAdd'; }");
        try {
            page.locator("#__ccAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ComplaintCategory.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ComplaintCategory: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("ComplaintCategory: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__ccMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__ccMims';"
        ...
        try {
            page.locator("#__ccMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__ccMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("ComplaintCategory: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ComplaintCategory: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ComplaintCategory: list check -> " + lastListCheck);
