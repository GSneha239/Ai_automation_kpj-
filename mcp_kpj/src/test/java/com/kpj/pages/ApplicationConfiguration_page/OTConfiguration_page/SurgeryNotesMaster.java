...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>SurgeryNotesMaster</b> — Page Object.
 *
 ...
 */
public class SurgeryNotesMaster extends BasePage {

    public SurgeryNotesMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; SurgeryNotesMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*surgery\\s*notes\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__snmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("SurgeryNotesMaster.nav: the Surgery Notes Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__snmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("SurgeryNotesMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__snmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("SurgeryNotesMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the SurgeryNotesMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("surgerynotes") || route.contains("surgery")
                || route.contains("notesmaster")
    ...
    public String describeControls() {
        ...
        System.out.println("=== SurgeryNotesMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?surg|AddSurgeryNotesMaster|SURGERY/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__snmAdd'; }");
        try {
            page.locator("#__snmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("SurgeryNotesMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__snmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    /**
     * Enter the <b>Code</b> and the <b>Short Description</b>, each pinned to its EXACT model.
     *
     * <p>This form carries TWO description boxes: {@code shortdescription} (the input this step means)
     * and {@code description} (a textarea further down). A match on "description" takes the textarea, and
     * the screen then refuses the save with "Please Enter Short Description!" while the run reports the
     * value as entered — which is exactly what happened before these were pinned.</p>
     */
    public String enterDetails(String code, String shortDescription) {
        Object r = page.evaluate("(a) => {" + JS
                + " const get=m=>[...document.querySelectorAll(\"[ng-model='SurgeryNotesMaster.\"+m+\"']\")]"
                + "   .filter(vis)[0];"
                + " const c=get('code'), d=get('shortdescription');"
                + " const rc = c? setEl(c,a[0])+' [SurgeryNotesMaster.code]' : '(no-code-field)';"
                + " const rd = d? setEl(d,a[1])+' [SurgeryNotesMaster.shortdescription]'"
                + "            : '(no-short-description-field)';"
                + " return 'Code='+rc+' | ShortDescription='+rd; }",
                java.util.List.of(code, shortDescription));
        lastEntry = r == null ? "" : r.toString();
        waitForAngular(500);
        System.out.println("SurgeryNotesMaster: " + lastEntry);
        return lastEntry;
    }

    /** Both boxes hold their OWN value — the check that catches the textarea taking the short one. */
    public boolean detailsEntered(String code, String shortDescription) {
        return lastEntry != null && !lastEntry.contains("(no-")
                && lastEntry.contains("Code=" + code)
                && lastEntry.contains("ShortDescription=" + shortDescription);
    }

    public String lastNoteType = "", lastRemark = "";

    /**
     * Tick <b>Pre</b>, <b>Intra</b> or <b>Post</b>.
     *
     * <p>The three boxes are separate models ({@code PreNote} / {@code IntraNote} / {@code PostNote}),
     * matched on the model rather than the label — the labels are one word each and appear elsewhere on
     * the page. The box is CLICKED, never assigned: an Angular checkbox set by hand leaves the model
     * untouched, so the tick would show on screen while the record saved without it.</p>
     */
    public String tickNoteType() {
        Object r = page.evaluate("() => {" + JS
                + " const boxes=[...document.querySelectorAll(\"input[type='checkbox']\")].filter(vis)"
                + "   .filter(e=>/prenote|intranote|postnote/i.test(ngOf(e)));"
                + " if(!boxes.length) return '(no Pre/Intra/Post checkbox on this form)';"
                + " const b=boxes[0];"
                ...
                + " return 'ticked \"'+norm(labelOf(b)).slice(0,20)+'\" ['+ngOf(b)+'] -> checked='"
                + "   +b.checked+', model='+model+' (the form offers '+boxes.length"
                + "   +': Pre, Intra and Post)'; }");
        lastNoteType = r == null ? "" : r.toString();
        waitForAngular(500);
        System.out.println("SurgeryNotesMaster: note type -> " + lastNoteType);
        return lastNoteType;
    }

    /** The box is ticked AND its model took the value. */
    public boolean noteTypeTicked() {
        return lastNoteType != null && lastNoteType.contains("checked=true")
                && !lastNoteType.contains("model=false");
    }

    /**
     * Enter the <b>Remark</b> ({@code SurgeryNotesMaster.description}).
     *
     * <p>Pinned to that model, and it is the TEXTAREA — the short description above it is a separate
     * input whose model also ends in "description", so a keyword match writes into the wrong one.</p>
     */
    public String enterRemark(String remark) {
        Object r = page.evaluate("(v) => {" + JS
                + " const e=[...document.querySelectorAll("
                + "   \"[ng-model='SurgeryNotesMaster.description']\")].filter(vis)[0]"
                + "   || [...document.querySelectorAll('textarea')].filter(vis)[0];"
                + " return e? setEl(e,v)+' ['+(ngOf(e)||'?')+', '+e.tagName+']' : '(no remark field)'; }",
                remark);
        lastRemark = r == null ? "" : r.toString();
        waitForAngular(400);
        System.out.println("SurgeryNotesMaster: remark -> " + lastRemark);
        return lastRemark;
    }

    public boolean remarkEntered(String remark) {
        return lastRemark != null && !lastRemark.startsWith("(no ") && lastRemark.startsWith(remark);
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("SurgeryNotesMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("SurgeryNotesMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__snmSeen = [];"
                    + " if(window.__snmObs) window.__snmObs.disconnect();"
                    + " window.__snmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__snmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__snmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__snmSeen||[];"
                    + " if(window.__snmObs) window.__snmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("SurgeryNotesMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("SurgeryNotesMaster: list check -> " + lastListCheck);
