...
/**
 * Application Configuration &gt; Patient &gt; <b>Disaster Type</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Disaster Type</b> (route
 * {@code #/DisasterType}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 * <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>Not to be confused with {@code Visit.DisasterTypeID} — a field on the Emergency Registration Visit
 * Information section that this master's entries populate.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.CustomField} (this screen's exact ng-model names
 ...
 */
public class DisasterType extends BasePage {

    public DisasterType(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/DisasterType";
    ...
    private static final String[] REMARKS = {
            "Natural Disaster", "Fire Outbreak", "Flood", "Building Collapse", "Chemical Spill", "Mass Casualty Incident"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("DisasterType.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*disaster\\s*types?\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__dsyMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("DisasterType.nav DEBUG: visible links => " + dbg);
                    System.out.println("DisasterType.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__dsyMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("DisasterType.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__dsyMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__dsyMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("DisasterType.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("DisasterType.nav: direct route " + route);
    ...
    /**
     * On the Disaster Type screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/disastertype/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/disaster\\s*type/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__dsyAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__dsyAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("DisasterType.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__dsyAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__dsyToasts=[]; if(window.__dsyObs) window.__dsyObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__dsyToasts.includes(t)) window.__dsyToasts.push(t); }); };"
                + " window.__dsyObs=new MutationObserver(grab); window.__dsyObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("disastertype") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__dsySubmit'; return 'click'; }"
        ...
        System.out.println("DisasterType submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__dsySubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DisasterType submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__dsySubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__dsyToasts||[]).some(a=>/disaster|type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("DisasterType save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__dsyToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
