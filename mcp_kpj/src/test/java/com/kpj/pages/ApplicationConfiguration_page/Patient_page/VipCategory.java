...
/**
 * Application Configuration &gt; Patient &gt; <b>VIP Category</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>VIP Category</b> (route
 * {@code #/VIPCategory} — confirmed live per the sibling {@code Patient_page.VipMaster} menu note: "the
 * sibling VIP Category (#/VIPCategory) is a different screen — match the link on EXACT text") → (Add, if
 ...
 *
 * <p><b>Menu trap (from VipMaster's note):</b> match the link on EXACT text {@code /^\s*vip\s*category\s*$/i} —
 * a loose {@code /vip/i} match can land on <b>VIP Master</b> instead, which is a different screen at a
 * different route.</p>
 */
public class VipCategory extends BasePage {

    public VipCategory(Page page) { super(page); }

    /** Confirmed live per the VipMaster sibling menu note. */
    public static final String ROUTE = "#/VIPCategory";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /**
     * Remarks are real VIP category names, not generic filler text.
     */
    private static final String[] REMARKS = {
            "Platinum", "Gold", "Silver", "Diamond", "Royal", "Corporate VIP", "Board Member",
            "Government Dignitary", "Diplomat", "Celebrity", "Sponsor VIP", "Executive"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("VipCategory.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT text match on a VISIBLE link ONLY — a loose /vip/i match can land on the sibling
                // "VIP Master" screen instead (different route, different fields).
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*vip\\s*category\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__vcMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("VipCategory.nav DEBUG: visible links => " + dbg);
                    System.out.println("VipCategory.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__vcMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("VipCategory.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__vcMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__vcMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("VipCategory.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("VipCategory.nav: direct route " + route);
    ...
    /**
     * On the VIP Category screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/vipcategory/i.test((location.hash||'').replace(/\\s+/g,''));" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/^\\s*vip\\s*category\\s*$/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__vcAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__vcAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("VipCategory.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__vcAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "VC" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__vcToasts=[]; if(window.__vcObs) window.__vcObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__vcToasts.includes(t)) window.__vcToasts.push(t); }); };"
                + " window.__vcObs=new MutationObserver(grab); window.__vcObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("vipcategory") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__vcSubmit'; return 'click'; }"
        ...
        System.out.println("VipCategory submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__vcSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("VipCategory submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__vcSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__vcToasts||[]).some(a=>/vip|category|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("VipCategory save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__vcToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
