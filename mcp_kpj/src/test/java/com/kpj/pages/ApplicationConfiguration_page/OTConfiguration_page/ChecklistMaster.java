...
/**
 * Application Configuration &gt; Inventory &gt; <b>ChecklistMaster</b> — Page Object.
 *
 ...
 */
public class ChecklistMaster extends BasePage {

    public ChecklistMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; ChecklistMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*checklist\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__clmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("ChecklistMaster.nav: the Checklist Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__clmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ChecklistMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__clmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ChecklistMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ChecklistMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("checklist") || route.contains("chklist")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ChecklistMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?checklist|AddChecklistMaster|CHECKLIST/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__clmAdd'; }");
        try {
            page.locator("#__clmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ChecklistMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__clmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ChecklistMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    public String lastCodeName = "", lastTag = "", lastCategory = "", lastRemark = "",
            lastNormalRemark = "";
    ...
    private String selectIndex(String model, String id, int index) {
        ...
        } catch (Exception e) { return "(select failed: " + e.getMessage() + ")"; }
        waitForAngular(700);
    ...
    /**
     * Enter the <b>Code</b> and the <b>Checklist Name</b>, each pinned to its exact model.
     *
     * <p>Every model here is prefixed {@code checklist.}, so keyword matching is unusable: a match on
     * "name" would also hit the screen's own prefix, and "code" appears in more than one place.</p>
     */
    public String enterCodeAndName(String code, String name) {
        String r1 = put("checklist.code", code);
        String r2 = put("checklist.checklistname", name);
        lastCodeName = "Code=" + r1 + " | ChecklistName=" + r2;
        waitForAngular(500);
        System.out.println("ChecklistMaster: " + lastCodeName);
        return lastCodeName;
    }

    public boolean codeAndNameEntered(String code, String name) {
        return lastCodeName != null && !lastCodeName.contains("(no field")
                && lastCodeName.contains("Code=" + code)
                && lastCodeName.contains("ChecklistName=" + name);
    }

    /** Select the <b>Tag</b> ({@code checklist.tagid}). */
    public String selectTag() {
        java.util.List<Integer> all = realIndexes("checklist.tagid");
        if (all.isEmpty()) { lastTag = "(the Tag list is empty)"; return lastTag; }
        lastTag = selectIndex("checklist.tagid", "__clmTag", all.get(0))
                + " [checklist.tagid, " + all.size() + " options]";
        System.out.println("ChecklistMaster: tag -> " + lastTag);
        return lastTag;
    }

    public boolean tagSelected() { return chosen(lastTag); }

    /** Select the <b>Checklist Category</b> ({@code checklist.checklistcategoryid}). */
    public String selectCategory() {
        java.util.List<Integer> all = realIndexes("checklist.checklistcategoryid");
        if (all.isEmpty()) { lastCategory = "(the Checklist Category list is empty)"; return lastCategory; }
        lastCategory = selectIndex("checklist.checklistcategoryid", "__clmCat", all.get(0))
                + " [checklist.checklistcategoryid, " + all.size() + " options]";
        System.out.println("ChecklistMaster: category -> " + lastCategory);
        return lastCategory;
    }
    ...
    /**
     * Enter the <b>Remark</b> ({@code checklist.remark}, a textarea).
     *
     * <p>The form also carries a <b>Normal Remark</b> checkbox whose label is marked with an asterisk. If
     * the remark box will not take a value, that box is ticked and the remark tried again — and the
     * report says so, rather than the step simply failing.</p>
     */
    public String enterRemark(String remark) {
        String r = put("checklist.remark", remark);
        waitForAngular(400);
        if (r.startsWith("(no field") || !r.startsWith(remark)) {
            Object tick = page.evaluate("() => {" + JS
                    + " const b=[...document.querySelectorAll(\"input[type='checkbox']\")].filter(vis)"
                    + "   .find(e=>/normalremark/i.test(ngOf(e)));"
                    + " if(!b) return '(no Normal Remark checkbox)';"
                    + " if(!b.checked) b.click();"
                    + " return 'ticked Normal Remark ['+ngOf(b)+'] -> checked='+b.checked; }");
            lastNormalRemark = tick == null ? "" : tick.toString();
            waitForAngular(600);
            String retry = put("checklist.remark", remark);
            lastRemark = "Remark=" + retry + "  ||  the box would not take the value at first; "
                    + lastNormalRemark;
        } else {
            lastRemark = "Remark=" + r;
        }
        System.out.println("ChecklistMaster: " + lastRemark);
        return lastRemark;
    }

    public boolean remarkEntered(String remark) {
        return lastRemark != null && !lastRemark.contains("(no field")
                && lastRemark.contains("Remark=" + remark);
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("ChecklistMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("ChecklistMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__clmSeen = [];"
                    + " if(window.__clmObs) window.__clmObs.disconnect();"
                    + " window.__clmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__clmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__clmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__clmSeen||[];"
                    + " if(window.__clmObs) window.__clmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("ChecklistMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ChecklistMaster: list check -> " + lastListCheck);
