...
/**
 * Application Configuration &gt; Patient &gt; <b>Payor</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Payor</b> → enter
 * <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast. INLINE-ADD generic CommonMaster screen
 * (Form Name pre-selected, no Add button — confirmed live via screenshot).</p>
 *
 * <p>Do NOT confuse with {@code Billing_page.Payor} (Receivable Master, route {@code #/receivableMaster},
 * Payor Type / phone / e-mail form) — an earlier version of this class incorrectly subclassed that screen
 * because a menu-link search with no visibility filter matched a DOM-present-but-hidden leftover link from
 * the Billing submenu instead of Patient's own "Payor" link. The two are unrelated screens.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.CampType} (this screen's exact route and
 * ng-model names were not yet confirmed live; sibling masters can live at routes that do not literally match
 * their menu label, so {@link #onScreen()} verifies by the rendered HEADER text via a substring match).
...
public class Payor extends BasePage {
    ...
    public static final String ROUTE = "#/Payor";
    ...
    private static final String[] REMARKS = {
            "Self Pay", "Corporate Panel", "Insurance Company", "Government Scheme", "Third Party Administrator", "Staff Benefit"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("Payor.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT text match on a VISIBLE link ONLY — a href-pattern search with no offsetParent!==null
                // filter previously matched a DOM-present-but-hidden leftover link from the Billing submenu's
                // OWN "Payor" (Receivable Master) instead of this Patient submenu's own link.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*payor\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("Payor.nav DEBUG: visible links => " + dbg);
                    System.out.println("Payor.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__pyMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("Payor.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__pyMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__pyMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("Payor.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Payor.nav: direct route " + route);
    ...
    /**
     * On the Payor screen — verified by a Code/Remark box actually present (this app's CommonMaster-family
     * header text is unreliable to check by itself — confirmed live it can show "COMPANY", the pre-selected
     * Form Name value, rather than "Payor" — so field presence plus the URL are the real signals).
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const urlOk=/payor/i.test(location.hash||'');" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__pyAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__pyAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Payor.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__pyAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "PY" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__pyToasts=[]; if(window.__pyObs) window.__pyObs.disconnect();"
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("payor") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__pySubmit'; return 'click'; }"
        ...
        System.out.println("Payor submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__pySubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Payor submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__pySubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__pyToasts||[]).some(a=>/payor|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Payor save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__pyToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
