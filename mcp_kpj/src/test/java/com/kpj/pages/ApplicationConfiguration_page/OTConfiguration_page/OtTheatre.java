...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>OtTheatre</b> — Page Object.
 *
 ...
 */
public class OtTheatre extends BasePage {

    public OtTheatre(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; OtTheatre.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*ot\\s*theatre\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__othMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("OtTheatre.nav: the OT Theatre link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__othMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("OtTheatre.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__othMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("OtTheatre.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the OtTheatre screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("ottheatre") || route.contains("theatre")
                || route.contains("otmaster")
    ...
    public String describeControls() {
        ...
        System.out.println("=== OtTheatre CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?ot|AddOtTheatre|OTTHEATRE/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__othAdd'; }");
        try {
            page.locator("#__othAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("OtTheatre.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__othAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String lastEntryPinned = "", lastStore = "";
    ...
    public String enterCodeAndRemark(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("OtTheatre: " + lastEntryPinned);
    ...
    /**
     * Select the <b>Store</b>.
     *
     * <p>Found by the model's last segment rather than by a keyword: the screen's own prefix contains
     * the word OT, and several of these screens carry more than one dropdown. Which store is used varies
     * per run so repeated runs do not keep pointing at the same one.</p>
     */
    public String selectStore(int seed) {
        Object info = page.evaluate("() => {" + JS
                ...
                + " const e=sels.find(x=>/store/i.test(tail(x)))"
                + "   || sels.find(x=>/store/i.test(labelOf(x)));"
                + " if(!e) return null; e.id='__othStore';"
        ...
        if (info == null) { lastStore = "(no Store dropdown on this form)"; return lastStore; }
        @SuppressWarnings("unchecked")
        java.util.Map<String, Object> m = (java.util.Map<String, Object>) info;
        java.util.List<?> idx = (java.util.List<?>) m.get("indexes");
        if (idx.isEmpty()) { lastStore = "(the Store list has no real option) [" + m.get("ng") + "]"; return lastStore; }
        int pick = ((Number) idx.get(Math.floorMod(seed, idx.size()))).intValue();
        try {
            page.locator("#__othStore").selectOption(
        ...
        waitForAngular(800);
        Object t = page.evaluate("() => { const e=document.getElementById('__othStore');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastStore = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count")
                + " stores]";
        System.out.println("OtTheatre: store -> " + lastStore);
        return lastStore;
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("OtTheatre: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("OtTheatre: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__othSeen = [];"
                    + " if(window.__othObs) window.__othObs.disconnect();"
                    + " window.__othObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__othSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__othObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__othSeen||[];"
                    + " if(window.__othObs) window.__othObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("OtTheatre: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("OtTheatre: list check -> " + lastListCheck);
