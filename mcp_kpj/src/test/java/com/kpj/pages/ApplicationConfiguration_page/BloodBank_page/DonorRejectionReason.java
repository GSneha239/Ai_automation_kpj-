...
/**
 * Application Configuration &gt; Blood Bank &gt; <b>Donor Rejection Reason</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Donor Rejection Reason</b> → (Add, if
 ...
 * {@code Investigation_page.Agency} / {@code Billing_page.BillStatus} / {@code BloodBank_page.BloodDiscardReason}
 * / {@code BloodBank_page.BloodDonorType} / {@code BloodBank_page.BloodTestResult} (this screen's exact
 ...
 */
public class DonorRejectionReason extends BasePage {

    public DonorRejectionReason(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/DonorRejectionReason";
    ...
    private static final String[] REMARKS = {
            "Low Hemoglobin", "Underweight", "Recent Tattoo", "High Blood Pressure", "Recent Travel History"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    ...
                    + " const a = links.find(x=>/^\\s*donor\\s*rejection\\s*reason\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__drrMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("DonorRejectionReason.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__drrMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DonorRejectionReason.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__drrMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("DonorRejectionReason.nav: direct route " + route);
    ...
    /**
     * On the Donor Rejection Reason screen — verified by the rendered HEADER text rather than a guessed URL
     * substring (sibling masters can live at routes that do not literally match their menu label — see the
     * class doc), plus a Code/Remark box or an Add button actually present.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/donorrejection/i.test(location.hash||'');"
                + " if(!/donor\\s*rejection/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__drrAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__drrAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("DonorRejectionReason.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__drrAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "DJ" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__drrToasts=[]; if(window.__drrObs) window.__drrObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__drrToasts.includes(t)) window.__drrToasts.push(t); }); };"
                + " window.__drrObs=new MutationObserver(grab); window.__drrObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("donorrejection") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__drrSubmit'; return 'click'; }"
        ...
        System.out.println("DonorRejectionReason submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__drrSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DonorRejectionReason submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__drrSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__drrToasts||[]).some(a=>/donor|rejection|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("DonorRejectionReason save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__drrToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
