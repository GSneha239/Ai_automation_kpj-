...
/**
 * Application Configuration &gt; Billing &gt; <b>Bill Status</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Billing</b> → <b>Bill Status</b> → (Add, if the screen has
 ...
 * {@code Investigation_page.Agency} (this screen's exact ng-model names were not yet confirmed live, and
 * sibling masters vary: {@code commonmaster.Code}/{@code .Description}, {@code X.code}/{@code .description}).
 * Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class BillStatus extends BasePage {

    public BillStatus(Page page) { super(page); }

    public static final String ROUTE = "#/BillStatus";
    ...
    private static final String[] REMARKS = {
            "Bill Pending Approval", "Bill Finalized", "Bill On Hold", "Bill Under Review", "Bill Closed"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1000);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*bill\\s*status\\s*$/i.test(norm(x.textContent)) || /billstatus/i.test(x.getAttribute('href')||'')); if(!a) return ''; a.id='__bsMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("BillStatus.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            ...
            catch (Exception e) { System.out.println("BillStatus.nav: click failed - " + e.getMessage()); }
        ...
        if (!onScreen()) {
            ...
            System.out.println("BillStatus.nav: direct route " + route);
    ...
    /** On the Bill Status screen — URL plus something real on the page (a Code/Remark box or an Add button). */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("billstatus")) return false;
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__bsAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__bsAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("BillStatus.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__bsAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__bsToasts=[]; if(window.__bsObs) window.__bsObs.disconnect();"
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("billstatus") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__bsSubmit'; return 'click'; }"
        ...
        System.out.println("BillStatus submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__bsSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("BillStatus submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__bsToasts||[]).some(a=>/status|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("BillStatus save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__bsToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
