...
/**
 * Application Configuration &gt; Patient &gt; <b>Black List Reason</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Black List Reason</b> → (Add, if
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.AppointmentReason} (this screen's exact route and
 * ng-model names were not yet confirmed live; sibling masters can live at routes that do not literally match
 * their menu label, so {@link #onScreen()} verifies by the rendered HEADER text, not a guessed URL substring).
 * Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class BlackListReason extends BasePage {

    public BlackListReason(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/BlackListReason";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks that read like genuine black-list reasons, not machine noise. */
    private static final String[] REMARKS = {
            "Repeated No-Show for Appointments", "Non-Payment of Outstanding Bills", "Abusive Behaviour Towards Staff",
            "Fraudulent Insurance Claim", "Falsified Medical Records", "Multiple Identity Misuse"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                // poll for ANY visible link before trusting the "not found" result (an earlier run saw only 2
                // leftover top-level links because it queried before the submenu had expanded).
                try {
                ...
                } catch (Exception ignore) { System.out.println("BlackListReason.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*black\\s*list\\s*reason\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__blrMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    Object dbg = page.evaluate("() => [...document.querySelectorAll('a[href]')].filter(x=>x.offsetParent!==null).map(x=>'\"'+(x.textContent||'').replace(/\\s+/g,' ').trim()+'\" -> '+x.getAttribute('href')).filter(t=>t.trim()!=='\"\" -> null').join(' | ')");
                    System.out.println("BlackListReason.nav DEBUG: visible links under Patient submenu => " + dbg);
                    System.out.println("BlackListReason.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__blrMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("BlackListReason.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__blrMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__blrMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("BlackListReason.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("BlackListReason.nav: direct route " + route);
    ...
    /**
     * On the Black List Reason screen — verified by the rendered HEADER text rather than a guessed URL
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/blacklistreason/i.test(location.hash||'');"
                + " if(!/black\\s*list\\s*reason/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__blrAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__blrAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("BlackListReason.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__blrAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "BL" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__blrToasts=[]; if(window.__blrObs) window.__blrObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__blrToasts.includes(t)) window.__blrToasts.push(t); }); };"
                + " window.__blrObs=new MutationObserver(grab); window.__blrObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("blacklistreason") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__blrSubmit'; return 'click'; }"
        ...
        System.out.println("BlackListReason submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__blrSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("BlackListReason submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__blrSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__blrToasts||[]).some(a=>/black|list|reason|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("BlackListReason save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__blrToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
