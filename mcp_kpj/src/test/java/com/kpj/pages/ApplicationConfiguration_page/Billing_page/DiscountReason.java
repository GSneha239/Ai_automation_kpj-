...
/**
 * Application Configuration &gt; Billing &gt; <b>Discount Reason</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Billing</b> → <b>Discount Reason</b> → (Add, if the screen
 * has one) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Billing_page.BillStatus} / {@code Billing_page.CancellationReason}
 * / {@code Billing_page.CodeType} (this screen's exact route and ng-model names were not yet confirmed live;
 * sibling masters on this Billing submenu turned out to live on the shared CommonMaster "Form Name" screen at
 * an UPPERCASE route that does not literally match the menu label — e.g. Cancellation Reason is
 * {@code #/CANCELLATION} — so {@link #onScreen()} verifies by the rendered HEADER text, not a guessed URL
 * substring). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class DiscountReason extends BasePage {

    public DiscountReason(Page page) { super(page); }

    /** Confirmed live route — the "Discount Reason" menu link routes here (app-internal name: Concession Reason). */
    public static final String ROUTE = "#/ConcessionReason";
    ...
    private static final String[] REMARKS = {
            "Staff Discount", "Senior Citizen Discount", "Corporate Discount", "Promotional Discount", "Loyalty Discount"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    ...
                    + " const a = links.find(x=>/^\\s*discount\\s*reason\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__drMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("DiscountReason.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__drMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DiscountReason.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__drMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("DiscountReason.nav: direct route " + route);
    ...
    /**
     * On the Discount Reason screen — verified by the rendered HEADER text rather than a guessed URL substring
     * (sibling masters on this submenu live at routes that do not literally match their menu label — see the
     * class doc). Confirmed live: the Billing submenu's "Discount Reason" LINK routes to {@code #/ConcessionReason}
     * and the CommonMaster form itself renders "Concession Reason" as its Form Name / header — an app-internal
     * naming mismatch (the menu label and the master's own name differ), so both texts are accepted here.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " if(!/discount\\s*reason|concession\\s*reason/i.test(hdr)) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__drAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__drAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("DiscountReason.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__drAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "DR" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("discount") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__drSubmit'; return 'click'; }"
        ...
        System.out.println("DiscountReason submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__drSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DiscountReason submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__drSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__drToasts||[]).some(a=>/reason|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("DiscountReason save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__drToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
