...
/**
 * Application Configuration &gt; Investigation &gt; <b>Result Template</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Result Template</b> → (Add,
 * if the screen has one) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Investigation_page.Film} / {@code Investigation_page.Result}
 * (this screen's exact route and ng-model names were not yet confirmed live; sibling masters can live at
 * routes that do not literally match their menu label, so {@link #onScreen()} verifies by the rendered HEADER
 * text, not a guessed URL substring). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact
 * are excluded.</p>
 *
 * <p>Menu note: the submenu parent is matched on EXACT text — a prefix match can hit a different module.</p>
 */
public class ResultTemplate extends BasePage {

    public ResultTemplate(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/ResultTemplate";
    ...
    private static final String[] REMARKS = {
            "Normal CBC Report", "Standard Lipid Profile Comment", "Renal Panel Interpretation",
            "Liver Function Interpretation", "Thyroid Profile Comment", "Standard Urinalysis Report"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                waitForAngular(1500);
                // EXACT text match ONLY, ANCHORED — "Result Template" must not match the sibling "Result" or
                // "Result Entry Comment" links on the same submenu.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*result\\s*template\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__rtMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("ResultTemplate.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__rtMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    ...
                    System.out.println("ResultTemplate.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__rtMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__rtMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("ResultTemplate.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("ResultTemplate.nav: direct route " + route);
    ...
    /**
     * On the Result Template screen — verified by the rendered HEADER text rather than a guessed URL substring
     * (sibling masters can live at routes that do not literally match their menu label — see
     * {@code Investigation_page.Result}, which lives at {@code #/RESULTMASTER}), plus a Code/Remark box or an
     * Add button actually present.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/result.*template|template.*result/i.test(location.hash||'');"
                + " if(!/result\\s*template/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__rtAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__rtAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("ResultTemplate.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__rtAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "RT" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__rtToasts=[]; if(window.__rtObs) window.__rtObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__rtToasts.includes(t)) window.__rtToasts.push(t); }); };"
                + " window.__rtObs=new MutationObserver(grab); window.__rtObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("resulttemplate") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__rtSubmit'; return 'click'; }"
        ...
        System.out.println("ResultTemplate submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__rtSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ResultTemplate submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__rtSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__rtToasts||[]).some(a=>/result|template|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("ResultTemplate save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__rtToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
