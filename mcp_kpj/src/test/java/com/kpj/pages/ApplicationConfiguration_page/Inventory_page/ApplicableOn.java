 ...
 */
public class ApplicableOn extends BasePage {

    public ApplicableOn(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; ApplicableOn.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*applicable\\s*on\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__aponMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__aponMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ApplicableOn.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__aponMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ApplicableOn.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ApplicableOn screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("applicableon") || route.contains("appon")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ApplicableOn CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?applicable|AddApplicableOn|APPON/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__aponAdd'; }");
        try {
            page.locator("#__aponAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ApplicableOn.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__aponAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ApplicableOn: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("ApplicableOn: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__aponMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__aponMims';"
        ...
        try {
            page.locator("#__aponMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__aponMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("ApplicableOn: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ApplicableOn: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ApplicableOn: list check -> " + lastListCheck);
