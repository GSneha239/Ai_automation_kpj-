...
/**
 * Application Configuration &gt; Blood Bank &gt; <b>Screening Test Master</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Screening Test Master</b> → (Add, if
 * the screen has one) → enter <b>Code*</b> + <b>Remark*</b> + <b>Comment</b> → select <b>Default Result</b>
 * → <b>Submit</b> → success toast.</p>
 *
 * <p>Code / Remark / Comment fields and the Default Result select are DISCOVERED at runtime rather than
 * hard-coded — same reusable shape as {@code BloodBank_page.RedCellSerologyGroup} / {@code Billing_page.BillStatus}
 * (this screen's exact route and ng-model names were not yet confirmed live; sibling masters can live at routes
 * that do not literally match their menu label, so {@link #onScreen()} verifies by the rendered HEADER text, not
 * a guessed URL substring). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class ScreeningTestMaster extends BasePage {

    public ScreeningTestMaster(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/ScreeningTestMaster";
    public String lastCode = "", lastRemark = "", lastComment = "", lastResult = "";
    public String lastCodeModel = "", lastRemarkModel = "", lastCommentModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks that read like genuine configuration rather than machine noise. */
    private static final String[] REMARKS = {
            "HIV Screening", "Hepatitis B Screening", "Hepatitis C Screening", "Syphilis Screening", "Malaria Screening"
    };
    private static final String[] COMMENTS = {
            "Mandatory pre-transfusion test", "Run on every donation", "Confirm with repeat test if reactive",
            "Standard screening protocol", "Result must be verified before release"
    };

    private static final int TOAST_SHOT_WAIT_MS = 5000;

    /** JS helper: the visible Code / Remark / Comment inputs and the Default Result select, whatever they are called. */
    private static final String FIND_FIELDS =
            ...
            + " const labelOf=e=>{ if(e.id){ const l=document.querySelector('label[for=\"'+e.id+'\"]'); if(l) return (l.textContent||'').replace(/\\s+/g,' ').trim(); }"
            + "   let p=e.closest('.form-group,.row,td,tr,div'); for(let hop=0; hop<4 && p; hop++){ const l=p.querySelector('label,.control-label'); if(l) return (l.textContent||'').replace(/\\s+/g,' ').trim(); p=p.parentElement; } return ''; };"
            ...
            + " const cmtEl  = boxes.find(e=>e!==codeEl && e!==remEl && (/comment/i.test(e.getAttribute('ng-model')||'') || /comment/i.test(e.placeholder||'') || /comment/i.test(labelOf(e))));"
            + " const sels=[...document.querySelectorAll('select')].filter(e=>e.offsetParent!==null);"
            + " const resultSel = sels.find(e=>/default\\s*result|result/i.test(e.getAttribute('ng-model')||'') || /default\\s*result|result/i.test(labelOf(e)));";
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            // spelled menu item on the Billing submenu (see CancellationReason.nav's "Receipt Cancellation" trap).
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    + " const links=[...document.querySelectorAll('a[href]')];"
                    + " const a = links.find(x=>/^\\s*screening\\s*test\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__stmMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("ScreeningTestMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__stmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ScreeningTestMaster.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__stmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("ScreeningTestMaster.nav: direct route " + route);
    ...
    /**
     * On the Screening Test Master screen — verified by the rendered HEADER text rather than a guessed URL
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/screeningtest/i.test(location.hash||'');"
                + " if(!/screening\\s*test/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__stmAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__stmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("ScreeningTestMaster.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__stmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    /**
     * Enter <b>Code*</b>, <b>Remark*</b> and <b>Comment</b>. {@code attempt} shifts the values so a retry after
     * an "already exists" toast submits genuinely different details.
     */
    public String fillDetails(int attempt) {
        String code = "ST" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
        lastRemark = REMARKS[(int) (Math.abs(System.nanoTime() / 1000 + attempt) % REMARKS.length)];
        lastComment = COMMENTS[(int) (Math.abs(System.nanoTime() / 1000 + attempt) % COMMENTS.length)];
        Object r = page.evaluate("(a) => { const A=window.angular;" + FIND_FIELDS
                ...
                + " const cd=set(codeEl, a.code); const rm=set(remEl, a.remark); const cm=set(cmtEl, a.comment);"
                + " return 'Code='+cd+' | Remark='+rm+' | Comment='+cm"
                + "   +' | [models: '+(codeEl?(codeEl.getAttribute('ng-model')||'?'):'none')+' / '+(remEl?(remEl.getAttribute('ng-model')||'?'):'none')+' / '+(cmtEl?(cmtEl.getAttribute('ng-model')||'?'):'none')+']'; }",
                java.util.Map.of("code", code, "remark", lastRemark, "comment", lastComment));
        waitForAngular(500);
        String res = r == null ? "" : r.toString();
        java.util.regex.Matcher m = java.util.regex.Pattern.compile("\\[models: ([^/]+) / ([^/]+) / ([^\\]]+)\\]").matcher(res);
        if (m.find()) { lastCodeModel = m.group(1).trim(); lastRemarkModel = m.group(2).trim(); lastCommentModel = m.group(3).trim(); }
        return res;
    }

    /** Select <b>Default Result</b> (first real option) — matched by its own ng-model or LABEL text. */
    public String selectDefaultResult() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                + " if(!resultSel) return '(not found)';"
                + " const cur=resultSel.options[resultSel.selectedIndex];"
                + " if(cur && cur.value && !/^-*\\s*select/i.test(norm(cur.textContent))) return norm(cur.textContent)+' (kept)';"
                + " const i=[...resultSel.options].findIndex(o=>o.value && !/^-*\\s*select/i.test(norm(o.textContent))); if(i<0) return '(no-options)';"
                + " resultSel.selectedIndex=i; resultSel.dispatchEvent(new Event('change',{bubbles:true})); const $=window.jQuery||window.$; if($){try{$(resultSel).trigger('change');}catch(err){}}"
                + " return norm(resultSel.options[i].textContent); }");
        waitForAngular(400);
    ...
    /**
     * Click <b>Submit</b> (accepts "Save" too) and return the toast. Re-asserts Code/Remark/Comment first —
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__stmToasts=[]; if(window.__stmObs) window.__stmObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__stmToasts.includes(t)) window.__stmToasts.push(t); }); };"
                + " window.__stmObs=new MutationObserver(grab); window.__stmObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("screeningtest") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " set(codeEl, a.code); set(remEl, a.remark); set(cmtEl, a.comment);"
                ...
                + " if(b){ b.id='__stmSubmit'; return 'click'; }"
                ...
                + " return 'none'; }",
                java.util.Map.of("code", lastCode, "remark", lastRemark, "comment", lastComment));
        System.out.println("ScreeningTestMaster submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__stmSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ScreeningTestMaster submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__stmSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__stmToasts||[]).some(a=>/screening|test|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("ScreeningTestMaster save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__stmToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
