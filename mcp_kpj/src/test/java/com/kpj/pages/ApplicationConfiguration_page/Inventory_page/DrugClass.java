...
/**
 * Application Configuration &gt; Inventory &gt; <b>DrugClass</b> — Page Object.
 *
 ...
 */
public class DrugClass extends BasePage {

    public DrugClass(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; DrugClass.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*drug\\s*class\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__dclMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("DrugClass.nav: the Drug Class link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__dclMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DrugClass.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__dclMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("DrugClass.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the DrugClass screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("drugclass") || route.contains("drugcls")
    ...
    public String describeControls() {
        ...
        System.out.println("=== DrugClass CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?drug|AddDrugClass|DRUGCLASS/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__dclAdd'; }");
        try {
            page.locator("#__dclAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("DrugClass.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__dclAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("DrugClass: " + lastEntry);
        return lastEntry;
    }
    ...
    public String lastCheckbox = "";

    /**
     * Tick <b>Is Carbapenems</b> or <b>Is Chemotherapy</b>.
     *
     * <p>The box is CLICKED rather than having {@code checked} assigned: an Angular checkbox set by hand
     * leaves the model untouched, so the tick shows on screen while the record saves without it. The
     * state is then read back off the control AND out of the ngModel — a box that looks ticked but whose
     * model is still false is exactly what this catches.</p>
     */
    public String tickDrugFlag() {
        Object r = page.evaluate("() => {" + JS
                + " const boxes=[...document.querySelectorAll(\"input[type='checkbox']\")].filter(vis)"
                + "   .filter(e=>!/row\\.entity|colFilter|selectAll/i.test(ngOf(e)));"
                + " const want=e=>/carbapenem|chemo/i.test(ngOf(e)+' '+labelOf(e));"
                + " const b=boxes.find(want) || boxes[0];"
                + " if(!b) return '(no checkbox on this form)';"
                + " const label=(labelOf(b)||'').slice(0,40), ng=ngOf(b)||'?';"
                ...
                + " return 'ticked \"'+label+'\" ['+ng+'] -> the control reads checked='+b.checked"
                + "   +' and its model reads '+model; }");
        lastCheckbox = r == null ? "" : r.toString();
        waitForAngular(600);
        System.out.println("DrugClass: checkbox -> " + lastCheckbox);
        return lastCheckbox;
    }

    /** The box is ticked AND its model took the value — not merely ticked on screen. */
    public boolean flagTicked() {
        return lastCheckbox != null && lastCheckbox.contains("checked=true")
                && !lastCheckbox.contains("its model reads false");
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("DrugClass: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("DrugClass: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__dclSeen = [];"
                    + " if(window.__dclObs) window.__dclObs.disconnect();"
                    + " window.__dclObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__dclSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__dclObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__dclSeen||[];"
                    + " if(window.__dclObs) window.__dclObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("DrugClass: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("DrugClass: list check -> " + lastListCheck);
