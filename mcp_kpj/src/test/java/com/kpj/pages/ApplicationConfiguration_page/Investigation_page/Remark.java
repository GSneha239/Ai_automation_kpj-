...
/**
 * Application Configuration &gt; Investigation &gt; <b>Remark</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Remark</b> → (Add, if the
...
public class Remark extends BasePage {
    ...
    public Remark(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/Remark";
    ...
    private static final String[] REMARKS = {
            "Sample Haemolysed", "Insufficient Sample Volume", "Fasting Not Confirmed",
            "Wrong Anticoagulant Used", "Sample Received Late", "Patient Identification Pending"
    };
        ...
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*remark\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__rmMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("Remark.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                ...
                try { page.locator("#__rmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("Remark.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__rmMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__rmMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("Remark.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Remark.nav: direct route " + route);
    ...
    /**
     * On the Remark screen — verified by the rendered HEADER text rather than a guessed URL substring
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/remark/i.test(location.hash||'');"
                + " if(!/remark/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__rmAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__rmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Remark.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String fillDetails(int attempt) {
        String code = "RM" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("remark") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__rmSubmit'; return 'click'; }"
        ...
        System.out.println("Remark submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__rmSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Remark submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__rmSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__rmToasts||[]).some(a=>/remark|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Remark save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__rmToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
