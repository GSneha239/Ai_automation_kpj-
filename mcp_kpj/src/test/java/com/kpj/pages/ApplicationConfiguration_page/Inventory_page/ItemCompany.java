...
/**
 * Application Configuration &gt; Inventory &gt; <b>Item Company</b> — Page Object.
 *
 ...
 */
public class ItemCompany extends BasePage {

    public ItemCompany(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; ItemCompany.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*item\\s*company\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__icoMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__icoMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ItemCompany.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__icoMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ItemCompany.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ItemCompany screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        return route.contains("itemcompany") || route.contains("company") || route.contains("cmpmst");
    }
    ...
    public String describeControls() {
        ...
        System.out.println("=== ItemCompany CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?item|AddItemCompany/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__icoAdd'; }");
        try {
            page.locator("#__icoAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ItemCompany.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__icoAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ItemCompany: " + lastEntry);
        return lastEntry;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ItemCompany: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ItemCompany: list check -> " + lastListCheck);
