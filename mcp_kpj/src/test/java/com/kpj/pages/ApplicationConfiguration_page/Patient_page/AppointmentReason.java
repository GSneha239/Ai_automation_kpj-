...
/**
 * Application Configuration &gt; Patient &gt; <b>Appointment Reason</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Appointment Reason</b> → (Add, if
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.PatientStatus} (this screen's exact route and
 ...
 */
public class AppointmentReason extends BasePage {

    public AppointmentReason(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/AppointmentReason";
    ...
    private static final String[] REMARKS = {
            "Follow-up Consultation", "Routine Check-up", "Vaccination", "Lab Test Review",
            "Second Opinion", "Physiotherapy Session"
    };
        ...
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*appointment\\s*reason\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__arMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("AppointmentReason.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                ...
                try { page.locator("#__arMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("AppointmentReason.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__arMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__arMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("AppointmentReason.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("AppointmentReason.nav: direct route " + route);
    ...
    /**
     * On the Appointment Reason screen — verified by the rendered HEADER text rather than a guessed URL
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/appointmentreason/i.test(location.hash||'');"
                + " if(!/appointment\\s*reason/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        try { page.locator("#__arAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("AppointmentReason.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__arAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "AR" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__arToasts=[]; if(window.__arObs) window.__arObs.disconnect();"
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("appointmentreason") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__arSubmit'; return 'click'; }"
        ...
        System.out.println("AppointmentReason submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__arSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("AppointmentReason submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__arSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__arToasts||[]).some(a=>/appointment|reason|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("AppointmentReason save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__arToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
