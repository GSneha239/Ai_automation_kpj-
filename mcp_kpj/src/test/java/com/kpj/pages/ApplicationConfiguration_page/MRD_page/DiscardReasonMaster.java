...
/**
 * Application Configuration &gt; MRD &gt; <b>Discard Reason Master</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>MRD</b> (submenu) → <b>Discard Reason Master</b>
 * ({@code #/DiscardReasonMaster}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.
 ...
 * wrong-page signature — {@link #onScreen()} rejects it rather than trusting the URL, because filling that
 * leftover markup would "pass" the earlier steps and then fail at Submit for an unexplained reason.</p>
 */
public class DiscardReasonMaster extends BasePage {

    public DiscardReasonMaster(Page page) { super(page); }

    public static final String ROUTE = "#/DiscardReasonMaster";
    ...
    private static final String[] REMARKS = {
            "Retention Period Expired", "Damaged Beyond Repair", "Duplicate Record",
            "Digitised And Archived", "Water Damage", "Transferred To Another Facility"};
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); document.querySelectorAll('#__drMenu').forEach(e=>e.removeAttribute('id')); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*discard\\s*reason(\\s*master)?\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/DiscardReasonMaster'); if(!a) return ''; a.id='__drMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("DiscardReasonMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__drMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(4000)); }
            catch (Exception e) { page.evaluate("() => { const a=document.getElementById('__drMenu'); if(a) a.click(); }"); }
            ...
            } catch (Exception ignore) { System.out.println("DiscardReasonMaster.nav: screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    /**
     * On the REAL Discard Reason Master screen — verified by its own <b>Remark</b> field, NOT the URL (see the
     * class note on the leftover Code + Store commonmaster form).
     */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("discardreason")) return false;
    ...
    public String fillDetails() {
        String code = "DR" + String.format("%05d", Math.abs(System.nanoTime() % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        for (int attempt = 0; attempt < 40; attempt++) {
            waitForAngular(500);
            Object tagged = page.evaluate("() => { document.querySelectorAll('#__drSubmit').forEach(e=>e.removeAttribute('id')); const b=[...document.querySelectorAll('button,input[type=submit]')].find(x=>/^\\s*(submit|save)\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(!b) return ''; b.id='__drSubmit'; return b.getAttribute('ng-click')||'submit'; }");
            String how = tagged == null ? "" : tagged.toString();
            System.out.println("DiscardReasonMaster submit[" + attempt + "] => " + (how.isEmpty() ? "(no Submit button)" : how));
            if (how.isEmpty()) return last;
            try { page.locator("#__drSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(5000)); } catch (Exception ignore) { }
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(el.textContent||''))"
                        + " || (window.__drToasts||[]).some(a=>/discard|reason|saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(a))",
                        null, new Page.WaitForFunctionOptions().setTimeout(9000));
            } catch (Exception ignore) { page.waitForTimeout(1500); }
            captureToastShot();
            Object t = page.evaluate("() => { const a=window.__drToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>x) || ''; }");
            ...
            page.evaluate("() => { window.__drToasts=[]; document.querySelectorAll('.toast-message,.toast').forEach(t=>t.remove()); }");
        }
        return last;
    }
}
