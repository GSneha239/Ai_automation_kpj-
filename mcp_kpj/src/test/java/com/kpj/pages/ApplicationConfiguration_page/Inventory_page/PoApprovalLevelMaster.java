...
/**
 * Application Configuration &gt; Inventory &gt; <b>PO Approval Level Master</b> — Page Object.
 *
 * <p>Flow: select the <b>PO Approval Level Master</b>, enter the <b>Min</b> and <b>Max</b>, then
 ...
 */
public class PoApprovalLevelMaster extends BasePage {

    public PoApprovalLevelMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; PoApprovalLevelMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                // PO, not PR — the two labels differ by one letter and sit next to each other in this
                // submenu, so the match is anchored and the sibling is rejected outright below.
                + "   .find(x=>/^\\s*po\\s*approval\\s*level\\s*master\\s*$/i.test(norm(x.textContent))"
                + "        || /^\\s*po\\s*approval\\s*level\\s*$/i.test(norm(x.textContent)));"
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("PoApprovalLevelMaster.nav: " + e.getMessage().split("\\n")[0]); }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("PoApprovalLevelMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the PoApprovalLevelMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        // Reject the SIBLING PR screen outright. "PR" and "PO" differ by one letter and sit together in
        // this submenu, and a loose "approval" match would accept #/PRApprovalLevelMaster — a whole run
        // against the wrong screen, exactly as once happened with Compliance and Complaint Type.
        if (route.contains("prapproval") || route.contains("prlevel")) return false;
        ...
        return route.contains("poapproval") || route.contains("polevel")
                || (route.contains("approval") && !route.contains("pr"));
    }
    ...
    public String describeControls() {
        ...
        System.out.println("=== PoApprovalLevelMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?po|AddPOApproval|AddPoApproval/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__poAdd'; }");
        try {
            page.locator("#__poAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("PoApprovalLevelMaster.openFormIfNeeded: " + e.getMessage().split("\\n")[0]);
        }
    ...
    /**
     * Select the <b>PO Approval Level Master</b> dropdown.
     *
    ...
    public String selectLevel() {
        ...
        System.out.println("PoApprovalLevelMaster: level = " + lastLevel);
        return lastLevel;
    }
    ...
    public String selectLocation() {
        ...
        waitForAngular(1200);
        System.out.println("PoApprovalLevelMaster: location = " + lastLocation);
        return lastLocation;
    }
    ...
    public String enterMinMax(String min, String max) {
        ...
        waitForAngular(500);
        System.out.println("PoApprovalLevelMaster: " + lastMinMax);
        return lastMinMax;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("PoApprovalLevelMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean rowInList(String min, String max) {
        ...
        System.out.println("PoApprovalLevelMaster: list check -> " + lastListCheck);
