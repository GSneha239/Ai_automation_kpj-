...
/**
 * Application Configuration &gt; Nursing &gt; <b>Medication Time Master</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu, EXACT text — "Nursing Station" is a
 * different module and a prefix match hits it first) → <b>Medication Time Master</b> (route
 * {@code #/MedicationTimeMaster}, best-guess fallback — navigation is primarily by the live menu link) →
 ...
 */
public class MedicationTimeMaster extends BasePage {

    public MedicationTimeMaster(Page page) { super(page); }

    /** Best-guess fallback — navigation is primarily by the live menu link, not this hardcoded route. */
    public static final String ROUTE = "#/MedicationTimeMaster";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Realistic medication-time names, not "Auto ... <code>" filler. */
    private static final String[] REMARKS = {
            "Early Morning", "Before Breakfast", "After Breakfast", "Mid Morning", "Before Lunch",
            "After Lunch", "Afternoon", "Before Dinner", "After Dinner", "Bedtime", "Midnight", "Pre-Dawn"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("MedicationTimeMaster.nav: Nursing submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT-ish text match on a VISIBLE link ONLY — accepts "Medication Time Master" or the
                // shorter "Medication Time" (menu labels sometimes drop the trailing "Master").
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*medication\\s*time\\s*master\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null)"
                        + "        || links.find(x=>/^\\s*medication\\s*time\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__mtMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("MedicationTimeMaster.nav DEBUG: visible links => " + dbg);
                    System.out.println("MedicationTimeMaster.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__mtMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("MedicationTimeMaster.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__mtMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__mtMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("MedicationTimeMaster.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("MedicationTimeMaster.nav: direct route " + route);
    ...
    /** On the Medication Time Master screen — verified by Code/Remark boxes present plus the URL. */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/medicationtime/i.test((location.hash||'').replace(/\\s+/g,''));" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/medication\\s*time/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__mtAdd'; return true; }"));
        ...
        catch (Exception e) { System.out.println("MedicationTimeMaster.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__mtToasts=[]; if(window.__mtObs) window.__mtObs.disconnect();"
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("medicationtime") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__mtSubmit'; return 'click'; }"
        ...
        System.out.println("MedicationTimeMaster submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__mtSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("MedicationTimeMaster submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__mtSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__mtToasts||[]).some(a=>/medication|time|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("MedicationTimeMaster save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__mtToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
