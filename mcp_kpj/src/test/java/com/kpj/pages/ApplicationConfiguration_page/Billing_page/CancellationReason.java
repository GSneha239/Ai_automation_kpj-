...
/**
 * Application Configuration &gt; Billing &gt; <b>Cancellation Reason</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Billing</b> → <b>Cancellation Reason</b> → (Add, if the
 ...
 * {@code Investigation_page.Agency} / {@code Billing_page.BillStatus} (this screen's exact ng-model names
 * were not yet confirmed live). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are
 * excluded.</p>
 */
public class CancellationReason extends BasePage {

    public CancellationReason(Page page) { super(page); }

    public static final String ROUTE = "#/CANCELLATION";
    ...
    private static final String[] REMARKS = {
            "Patient Request", "Duplicate Bill", "Wrong Charges", "Billing Error", "Insurance Rejection"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1000);
            // EXACT text match FIRST (a bare href-contains fallback picked "Receipt Cancellation" instead of
            // "Cancellation Reason" when both are on the Billing submenu — never combine the two in one find()).
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    + " const links=[...document.querySelectorAll('a[href]')];"
                    + " const a = links.find(x=>/^\\s*cancellation\\s*reason\\s*$/i.test(norm(x.textContent)))"
                    + "   || links.find(x=>/^\\s*cancellation\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__crMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("CancellationReason.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            ...
            catch (Exception e) { System.out.println("CancellationReason.nav: click failed - " + e.getMessage()); }
        ...
        if (!onScreen()) {
            ...
            System.out.println("CancellationReason.nav: direct route " + route);
    ...
    /** On the Cancellation Reason screen — URL plus something real on the page (a Code/Remark box or an Add button). */
    public boolean onScreen() {
        // Confirmed live: the app's own route is #/CANCELLATION (no "Reason" in the URL) — it's the shared
        // CommonMaster screen (Form Name* + Code* + Remark*) also used by ~177 other masters, selected via
        // "Form Name" rather than having its own dedicated route.
        if (!page.url().toLowerCase().contains("cancellation")) return false;
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("CancellationReason.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String fillDetails(int attempt) {
        String code = "CR" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("cancellation") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__crSubmit'; return 'click'; }"
        ...
        System.out.println("CancellationReason submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("CancellationReason submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__crToasts||[]).some(a=>/reason|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("CancellationReason save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__crToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
