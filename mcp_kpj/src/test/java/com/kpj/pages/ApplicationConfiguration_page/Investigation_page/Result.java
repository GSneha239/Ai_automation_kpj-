...
/**
 * Application Configuration &gt; Investigation &gt; <b>Result</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Result</b> → (Add, if the
 ...
 * {@code Investigation_page.Remark} / {@code Investigation_page.ResultEntryComment} (this screen's exact route
 * and ng-model names were not yet confirmed live; sibling masters can live at routes that do not literally
 * match their menu label, so {@link #onScreen()} verifies by the rendered HEADER text, not a guessed URL
 * substring). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 *
...
public class Result extends BasePage {
    ...
    public Result(Page page) { super(page); }
    ...
    private static final String[] REMARKS = {
            "Positive", "Negative", "Reactive", "Non-Reactive", "Inconclusive", "Not Detected"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                waitForAngular(1500);
                // EXACT text match ONLY, ANCHORED — "Result" must not match the sibling "Result Entry Comment"
                // link on the same submenu.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*result\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__rsMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("Result.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__rsMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    ...
                    System.out.println("Result.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__rsMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__rsMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("Result.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Result.nav: direct route " + route);
    ...
    /**
     * On the Result screen — confirmed live to be a CommonMaster-family screen at route {@code #/RESULTMASTER}
     * (same family as {@code Location_page.ConsultationRoom}'s "~177 masters via Form Name"), with header text
     * "Result" and {@code commonmaster.Code} / {@code commonmaster.Description} fields. The header/URL check is
     * a plain substring match (not anchored) — "result" is a substring of "resultmaster" and this page object
     * only ever navigates itself, so it cannot be confused with the sibling "Result Entry Comment" screen here.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/result/i.test(location.hash||'');"
                + " if(!/result/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__rsAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__rsAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Result.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__rsAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "RS" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__rsToasts=[]; if(window.__rsObs) window.__rsObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__rsToasts.includes(t)) window.__rsToasts.push(t); }); };"
                + " window.__rsObs=new MutationObserver(grab); window.__rsObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().matches(".*\\bresult\\b.*") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__rsSubmit'; return 'click'; }"
        ...
        System.out.println("Result submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__rsSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Result submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__rsSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__rsToasts||[]).some(a=>/result|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Result save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__rsToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
