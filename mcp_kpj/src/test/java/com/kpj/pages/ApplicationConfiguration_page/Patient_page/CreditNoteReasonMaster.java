...
/**
 * Application Configuration &gt; Patient &gt; <b>CreditNote Reason Master</b> — configuration screen Page
 * Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>CreditNoteReasonMaster</b> (route
 * {@code #/CreditNoteReasonMaster}, confirmed live in a prior menu dump) → (Add, if the screen has one) →
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.Qualification} (this screen's exact ng-model names
 ...
 */
public class CreditNoteReasonMaster extends BasePage {

    public CreditNoteReasonMaster(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/CreditNoteReasonMaster";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks that read like genuine credit-note reasons, not machine noise. */
    private static final String[] REMARKS = {
            "Billing Error Correction", "Duplicate Charge Reversal", "Service Not Rendered",
            "Insurance Adjustment", "Overpayment Refund", "Discount Applied Post-Billing"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("CreditNoteReasonMaster.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                // live: the menu label reads "CreditNoteReasonMaster" as one run-together word (no spaces).
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*credit\\s*note\\s*reason\\s*master\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__cnrMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("CreditNoteReasonMaster.nav DEBUG: visible links => " + dbg);
                    System.out.println("CreditNoteReasonMaster.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__cnrMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("CreditNoteReasonMaster.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__cnrMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__cnrMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("CreditNoteReasonMaster.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("CreditNoteReasonMaster.nav: direct route " + route);
    ...
    /**
     * On the CreditNote Reason Master screen — verified by a Code/Remark box actually present plus the URL
     * (the app's CommonMaster-family header text can show the pre-selected Form Name rather than the menu
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/creditnotereason/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/credit\\s*note\\s*reason/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__cnrAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__cnrAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("CreditNoteReasonMaster.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__cnrAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__cnrToasts=[]; if(window.__cnrObs) window.__cnrObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__cnrToasts.includes(t)) window.__cnrToasts.push(t); }); };"
                + " window.__cnrObs=new MutationObserver(grab); window.__cnrObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("creditnotereason") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__cnrSubmit'; return 'click'; }"
        ...
        System.out.println("CreditNoteReasonMaster submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__cnrSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("CreditNoteReasonMaster submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__cnrSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__cnrToasts||[]).some(a=>/credit|note|reason|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("CreditNoteReasonMaster save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__cnrToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
