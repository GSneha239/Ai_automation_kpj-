...
/**
 * Application Configuration &gt; Inventory &gt; <b>Type of Return</b> — Page Object.
 *
 ...
 */
public class ItemClassMaster extends BasePage {

    public ItemClassMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; ItemClassMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*item\\s*class\\s*master\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__icmMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__icmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ItemClassMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__icmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ItemClassMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ItemClassMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("itemclass") || route.contains("itmclsmst")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ItemClassMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?item|AddItemClass|GENMST/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__icmAdd'; }");
        try {
            page.locator("#__icmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ItemClassMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__icmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ItemClassMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("ItemClassMaster: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__icmMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__icmMims';"
        ...
        try {
            page.locator("#__icmMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__icmMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("ItemClassMaster: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    /**
     * Enter the <b>MIMS Description</b> on its own.
     *
     * <p>Pinned to {@code commonmaster.mimsDescription}. The record's own Remark lives in
     * {@code commonmaster.Description}, so a match on "description" fills the wrong box and leaves this
     * one empty while both steps look like they passed.</p>
     */
    public String enterMimsDescription(String description) {
        Object r = page.evaluate("(v) => {" + JS
                + " const boxes=[...document.querySelectorAll('input,textarea')].filter(vis)"
                + "   .filter(e=>e.type!=='hidden' && e.type!=='checkbox' && e.type!=='radio')"
                + "   .filter(e=>!/search|colFilter|row\\.entity|pagination/i.test(ngOf(e)+' '+(e.placeholder||'')));"
                + " const el=boxes.find(e=>/^mimsdescription$/i.test(tail(e)))"
                + "   || boxes.find(e=>/mims.*desc/i.test(tail(e)+' '+(e.placeholder||'')));"
                + " return el? setEl(el,v)+' ['+(ngOf(el)||'?')+']' : '(no-MIMS-Description-field)'; }",
                description);
        lastMims = r == null ? "" : r.toString();
        waitForAngular(500);
        System.out.println("ItemClassMaster: MIMS Description = " + lastMims);
        return lastMims;
    }

    public boolean mimsDescriptionEntered(String description) {
        return lastMims != null && !lastMims.startsWith("(no-") && lastMims.startsWith(description);
    }

    public String lastInstruction = "";

    /**
     * Enter the <b>Instruction</b>.
     *
     * <p>Pinned by the model's last segment and kept away from the Remark box: "instruction" and
     * "description" sit together on these forms, and the sibling Route Master screen carries both
     * {@code Instruction} and {@code RouteInstruction}, where a loose match takes the wrong one.</p>
     */
    public String enterInstruction(String instruction) {
        Object r = page.evaluate("(v) => {" + JS
                + " const boxes=[...document.querySelectorAll('input,textarea')].filter(vis)"
                + "   .filter(e=>e.type!=='hidden' && e.type!=='checkbox' && e.type!=='radio')"
                + "   .filter(e=>!/search|colFilter|row\\.entity|pagination/i.test(ngOf(e)+' '+(e.placeholder||'')));"
                + " const el=boxes.find(e=>/^instruction$/i.test(tail(e)))"
                + "   || boxes.find(e=>/instruction/i.test(tail(e)))"
                + "   || boxes.find(e=>/instruction/i.test((e.placeholder||'')+' '+labelOf(e)));"
                + " return el? setEl(el,v)+' ['+(ngOf(el)||'?')+']' : '(no-Instruction-field)'; }",
                instruction);
        lastInstruction = r == null ? "" : r.toString();
        waitForAngular(500);
        System.out.println("ItemClassMaster: Instruction = " + lastInstruction);
        return lastInstruction;
    }

    public boolean instructionEntered(String instruction) {
        return lastInstruction != null && !lastInstruction.startsWith("(no-")
                && lastInstruction.startsWith(instruction);
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ItemClassMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ItemClassMaster: list check -> " + lastListCheck);
