...
/**
 * Application Configuration &gt; Inventory &gt; <b>IndentFrequencyMaster</b> — Page Object.
 *
 ...
 */
public class IndentFrequencyMaster extends BasePage {

    public IndentFrequencyMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; IndentFrequencyMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*indent\\s*frequency\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__ifmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("IndentFrequencyMaster.nav: the Indent Frequency Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__ifmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("IndentFrequencyMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__ifmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("IndentFrequencyMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the IndentFrequencyMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("indentfrequency") || route.contains("indent")
    ...
    public String describeControls() {
        ...
        System.out.println("=== IndentFrequencyMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?indent|AddIndentFrequencyMaster|INDENTFREQ/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__ifmAdd'; }");
        try {
            page.locator("#__ifmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("IndentFrequencyMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__ifmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("IndentFrequencyMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    public String lastRequesting = "", lastIssuing = "", lastCycle = "", lastDay = "", lastGridCheck = "";
    /** The day that was ticked, e.g. {@code Monday} — used to read the saved row back. */
    public String tickedDayName = "";
    ...
    /**
     * Select the <b>Requesting Store</b> and the <b>Issuing Store</b>.
     *
     * <p>The two are deliberately DIFFERENT stores, and which pair is used varies per run: this screen
     * holds one record per store pair, so reusing a pair would be refused as a duplicate and the run
     * would report a defect where the screen is behaving correctly.</p>
     */
    public String selectStores(int seed) {
        java.util.List<Integer> req = realIndexes("dataObj.drpRequestingStore");
        java.util.List<Integer> iss = realIndexes("dataObj.drpIssuingStore");
        if (req.isEmpty() || iss.isEmpty()) {
            lastRequesting = "(the Requesting Store list is empty)";
            lastIssuing = "(the Issuing Store list is empty)";
            return lastRequesting + " | " + lastIssuing;
        }
        int a = Math.floorMod(seed, req.size());
        int b = Math.floorMod(seed / 7 + 1, iss.size());
        if (req.get(a).equals(iss.get(b))) b = (b + 1) % iss.size();

        lastRequesting = selectIndex("dataObj.drpRequestingStore", "__ifmReq", req.get(a));
        lastIssuing = selectIndex("dataObj.drpIssuingStore", "__ifmIss", iss.get(b));
        System.out.println("IndentFrequencyMaster: stores -> requesting=\"" + lastRequesting
                + "\" issuing=\"" + lastIssuing + "\" (from " + req.size() + " stores)");
        return "Requesting=" + lastRequesting + " | Issuing=" + lastIssuing;
    }

    public boolean storesSelected() {
        return chosen(lastRequesting) && chosen(lastIssuing) && !lastRequesting.equals(lastIssuing);
    }

    /**
     * Select the <b>Recurring Cycle</b>, preferring <b>Weekly</b>.
     *
     * <p>Weekly is the cycle the day checkboxes belong to — under Daily there is no day to tick, so
     * taking whichever option came first would make the next step untestable.</p>
     */
    public String selectRecurringCycle() {
        java.util.List<Integer> all = realIndexes("dataObj.RecurringCycle");
        if (all.isEmpty()) { lastCycle = "(the Recurring Cycle list is empty)"; return lastCycle; }
        Object wk = page.evaluate("() => {" + JS
                + " const e=[...document.querySelectorAll(\"select[ng-model='dataObj.RecurringCycle']\")]"
                + "   .filter(vis)[0]; if(!e) return -1;"
                + " return [...e.options].findIndex(o=>/weekly/i.test(norm(o.text))); }");
        int weekly = wk instanceof Number ? ((Number) wk).intValue() : -1;
        lastCycle = selectIndex("dataObj.RecurringCycle", "__ifmCycle", weekly >= 0 ? weekly : all.get(0));
        System.out.println("IndentFrequencyMaster: cycle -> " + lastCycle);
        return lastCycle;
    }

    public boolean cycleSelected() { return chosen(lastCycle); }

    /**
     * Tick the checkbox for a day ({@code dataObj.IsMonday} … {@code IsFriday}).
     *
     * <p>The box is CLICKED, never assigned: an Angular checkbox set by hand leaves the model untouched,
     * so the tick shows on screen while the record saves without the day. Both the control and the
     * ngModel are read back.</p>
     */
    public String tickDay() {
        Object r = page.evaluate("() => {" + JS
                + " const boxes=[...document.querySelectorAll(\"input[type='checkbox']\")].filter(vis)"
                + "   .filter(e=>/^dataObj\\.Is/.test(ngOf(e)));"
                + " if(!boxes.length) return '|(no day checkbox on this form)';"
                + " const b=boxes.find(e=>/monday/i.test(ngOf(e))) || boxes[0];"
                + " const ng=ngOf(b), day=ng.replace(/^dataObj\\.Is/,'');"
                ...
                + " return day+'|ticked '+day+' ['+ng+'] -> the control reads checked='+b.checked"
                + "   +' and its model reads '+model+' (the form offers '+boxes.length+' days)'; }");
        String s = r == null ? "" : r.toString();
        int bar = s.indexOf('|');
        tickedDayName = bar > 0 ? s.substring(0, bar) : "";
        lastDay = bar >= 0 ? s.substring(bar + 1) : s;
        waitForAngular(500);
        System.out.println("IndentFrequencyMaster: day -> " + lastDay);
        return lastDay;
    }

    /** The day box is ticked AND its model took the value. */
    public boolean dayTicked() {
        return lastDay != null && lastDay.contains("checked=true")
                && !lastDay.contains("its model reads false");
    }

    /**
     * Look for the saved record in the screen's own grid — the Requesting Store and the Issuing Store on
     * ONE row.
     *
     * <p>Matching the two names separately would pass on two unrelated rows, which is easy here: every
     * row carries a store pair drawn from the same 94 names.</p>
     */
    public String recordInGrid() {
        waitForAngular(2000);
        Object r = page.evaluate("(a) => {" + JS
                + " const rows=[...document.querySelectorAll('.ui-grid-row,table tbody tr')].filter(vis)"
                + "   .map(t=>norm(t.textContent)).filter(t=>t);"
                + " const mine=rows.filter(t=>t.includes(a.req) && t.includes(a.iss));"
                + " if(!mine.length) return 'the pair \"'+a.req+'\" -> \"'+a.iss+'\" is NOT in the grid ('"
                + "   +rows.length+' rows shown)';"
                + " return 'FOUND the row: '+mine[0].slice(0,160); }",
                java.util.Map.of("req", lastRequesting, "iss", lastIssuing));
        lastGridCheck = r == null ? "" : r.toString();
        System.out.println("IndentFrequencyMaster: grid check -> " + lastGridCheck);
        return lastGridCheck;
    }

    public boolean recordFound() { return lastGridCheck != null && lastGridCheck.startsWith("FOUND"); }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("IndentFrequencyMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("IndentFrequencyMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__ifmSeen = [];"
                    + " if(window.__ifmObs) window.__ifmObs.disconnect();"
                    + " window.__ifmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__ifmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__ifmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__ifmSeen||[];"
                    + " if(window.__ifmObs) window.__ifmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("IndentFrequencyMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("IndentFrequencyMaster: list check -> " + lastListCheck);
