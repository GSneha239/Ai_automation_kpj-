...
/**
 * Application Configuration &gt; Nursing &gt; <b>Eating Ability</b> — configuration screen
 * Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu) → <b>Eating Ability</b>
 * ({@code #/EatingAbility}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → toast.</p>
 *
 ...
 */
public class EatingAbility extends BasePage {

    public EatingAbility(Page page) { super(page); }

    public static final String ROUTE = "#/EatingAbility";
    ...
    private static final String[] REMARKS = {
            "Independent", "Needs Assistance", "Tube Feeding", "Unable To Swallow", "Supervised Feeding"};
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            // The menu label is "Eating Ability" — accept either spelling, or the exact href.
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*eating\\s*ability\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/EatingAbility'); if(!a) return ''; a.id='__eaMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("EatingAbility.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__eaMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("EatingAbility.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__eaMenu'); if(e) e.removeAttribute('id'); }");
            try {
            ...
            } catch (Exception ignore) { System.out.println("EatingAbility.nav: screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("eatingability")) return false;
    ...
    public String clickAddIfPresent() {
        Object tagged = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " if(!b) return ''; b.id='__eaAdd'; return b.getAttribute('ng-click')||'add'; }");
        String how = tagged == null ? "" : tagged.toString();
        if (how.isEmpty()) return "(inline-add — no Add button)";
        try { page.locator("#__eaAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__eaAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2500);
    ...
    public String fillDetails(int attempt) {
        String code = "EA" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        Object tagged = page.evaluate("() => { window.__eaToasts=[]; if(window.__eaObs) window.__eaObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__eaToasts.includes(t)) window.__eaToasts.push(t); }); };"
                + " window.__eaObs=new MutationObserver(grab); window.__eaObs.observe(document.body,{childList:true,subtree:true}); grab();"
                ...
                + " if(!b) return false; b.id='__eaSubmit'; return true; }");
        if (!Boolean.TRUE.equals(tagged)) { System.out.println("submitAndGetToast: Submit button not found"); return ""; }
        try { page.locator("#__eaSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(5000)); }
        catch (Exception e) { System.out.println("submitAndGetToast: Submit click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__eaSubmit'); if(e) e.removeAttribute('id'); }");
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|error/i.test(el.textContent||''))"
                    + " || (window.__eaToasts||[]).some(a=>/eating|ability|saved|success|added|updated|please|enter|select|required|exist|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(20000));
        } catch (Exception ignore) { page.waitForTimeout(2500); }
        captureToastShot();
        Object r = page.evaluate("() => { const a=window.__eaToasts||[]; return a.find(x=>/(eating|ability|record|master).*(saved|added)|saved success|added success|success/i.test(x)) || a.find(x=>x) || ''; }");
        waitForAngular(400);
