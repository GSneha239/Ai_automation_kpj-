...
/**
 * Application Configuration &gt; Investigation &gt; <b>LabSampleSuitability</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>LabSampleSuitability</b> → (Add, if the
 ...
 */
public class LabSampleSuitability extends BasePage {

    public LabSampleSuitability(Page page) { super(page); }

    public static final String ROUTE = "#/LabSampleSuitability";
    public String lastCode = "", lastRemark = "", lastRemark2 = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Sample-suitability reasons that read like genuine configuration rather than machine noise. */
    private static final String[] REMARKS = {
            "Suitable For Testing", "Haemolysed Sample", "Insufficient Volume",
            "Clotted Sample", "Incorrect Container", "Sample Past Stability"
    };

    /** Second Remark box — a short note alongside the reason. */
    private static final String[] REMARKS2 = {
            "Accepted for processing", "Reject and request a repeat",
            "Contact the ward before discarding", "Process with a comment on the report",
            "Recollect within 24 hours", "Refer to the lab supervisor"
    };
     ...
     * without that, a loose fallback hands the same input back twice and one field silently stays empty.
     */
    private static final String FIND_FIELDS =
            ...
            + " const remEl2 = pick(e=>/\\.(remark2|description2|note|comment)$/i.test(mdl(e)), e=>/remark|description|note|comment/i.test(mdl(e)), e=>/^\\s*(remark|description|note)\\s*$/i.test(plc(e)));";
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*lab\\s*sample\\s*suitability\\s*$/i.test(norm(x.textContent)) || /suitab/i.test(x.getAttribute('href')||'')); if(!a) return ''; a.id='__lsMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("LabSampleSuitability.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__lsMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("LabSampleSuitability.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__lsMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("LabSampleSuitability.nav: direct route " + route);
    ...
    /** On the LabSampleSuitability screen — URL plus something real on the page (a Code/Remark box or an Add button). */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("labsample")) return false;
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__lsAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__lsAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("LabSampleSuitability.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__lsAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "LS" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
        lastRemark = REMARKS[(int) (Math.abs(System.nanoTime() / 1000 + attempt) % REMARKS.length)];
        lastRemark2 = REMARKS2[(int) (Math.abs(System.nanoTime() / 1000 + attempt) % REMARKS2.length)];
        Object r = page.evaluate("(a) => { const A=window.angular;" + FIND_FIELDS
                ...
                java.util.Map.of("code", code, "remark", lastRemark, "remark2", lastRemark2));
        waitForAngular(500);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__lsToasts=[]; if(window.__lsObs) window.__lsObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__lsToasts.includes(t)) window.__lsToasts.push(t); }); };"
                + " window.__lsObs=new MutationObserver(grab); window.__lsObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("labsample") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__lsSubmit'; return 'click'; }"
        ...
        System.out.println("LabSampleSuitability submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__lsSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("LabSampleSuitability submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__lsSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__lsToasts||[]).some(a=>/suitab|sample|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("LabSampleSuitability save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__lsToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
