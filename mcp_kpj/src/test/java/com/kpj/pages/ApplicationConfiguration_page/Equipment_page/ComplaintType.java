...
/**
 * Application Configuration &gt; Equipment &gt; <b>Complaint Type</b> — Page Object.
 *
 * <p>Flow: list → <b>Add</b> → enter <b>Code</b>, <b>Remark</b> and <b>Expected Resolution</b> → select the
 * <b>Period</b> → <b>Submit</b> → success toast.</p>
 */
public class ComplaintType extends BasePage {

    public ComplaintType(Page page) { super(page); }

    public String lastBodyText = "", lastControls = "", lastMenu = "", lastEntry = "", lastPeriod = "",
    ...
    /**
     * Application Configuration &rarr; Equipment &rarr; Complaint Type.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*complaint\\s*type\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__ctMenu'; return a.getAttribute('href')||''; }");
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("ComplaintType.nav: " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__ctMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
        if (!onScreen()) {
            // The app's route is #/ComplaintListType — NOT #/ComplaintType. Guessing the obvious name
            // lands on the dashboard, so the menu href is used first and this is only the fallback.
            try { page.evaluate("() => { window.location.hash = '#/ComplaintListType'; }"); } catch (Exception ignore) { }
            waitForAngular(3000);
        }
        if (!onScreen()) {
            try { page.navigate(baseUrl + "#/ComplaintListType"); } catch (Exception ignore) { }
            waitForAngular(3500);
        }
    ...
    /**
     * On the Complaint Type screen.
     *
     * <p>Its route is {@code #/ComplaintListType}, so a check for "complainttype" rejects the very page it
     * is meant to accept — and the run then falls back to a route that does not exist. Both spellings are
     * accepted, and the sibling {@code ComplaintCategory} / {@code ComplaintStatus} screens are excluded.</p>
     */
    public boolean onScreen() {
        String u = page.url().toLowerCase().replace(" ", "");
        return u.contains("complaintlisttype") || u.contains("complainttype");
    }
    ...
    public String describeControls() {
        ...
        System.out.println("=== ComplaintType CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public boolean clickAdd() {
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?complainttype|AddComplaintType/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__ctAdd'; }");
        try {
            page.locator("#__ctAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ComplaintType.clickAdd: " + e.getMessage());
            try { page.evaluate("() => { window.location.hash = '#/add-ComplaintType'; }"); } catch (Exception ignore) { }
        }
        ...
        } catch (Exception ignore) { System.out.println("ComplaintType.clickAdd: Code field never appeared"); }
        waitForAngular(900);
        return formOpen();
    }
    ...
    /**
     * Enter <b>Code</b>, <b>Remark</b> and <b>Expected Resolution</b>.
     *
     ...
     */
    public String enterDetails(String code, String remark, String expectedResolution) {
        Object r = page.evaluate("(a) => {" + JS
                ...
                + " const re2 = resEl? setEl(resEl,res)+' ['+(resEl.getAttribute('ng-model')||'?')+']' : '(no-expected-resolution-field)';"
                + " return 'Code='+rc+' | Remark='+rr+' | ExpectedResolution='+re2; }",
                java.util.List.of(code, remark, expectedResolution));
        lastEntry = r == null ? "" : r.toString();
        waitForAngular(500);
        System.out.println("ComplaintType: " + lastEntry);
        return lastEntry;
    }
    ...
    public String selectPeriod() {
        ...
        try {
            ...
            System.out.println("ComplaintType.selectPeriod: no populated dropdown within 15s");
        }

        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__ctPeriod').forEach(e=>e.removeAttribute('id'));"
                + " const sels=[...document.querySelectorAll('select')].filter(vis)"
                + "   .filter(e=>!/pagination/i.test(e.getAttribute('ng-model')||''));"
                + " const e=sels.find(s=>/period/i.test((s.getAttribute('ng-model')||'')+' '+labelOf(s)))"
                + "   || sels[0];"
                + " if(!e) return null; e.id='__ctPeriod';"
        ...
        if (info == null) { lastPeriod = "(no-option)"; return lastPeriod; }
        @SuppressWarnings("unchecked")
        java.util.Map<String, Object> m = (java.util.Map<String, Object>) info;
        try {
            page.locator("#__ctPeriod").selectOption(
                    new com.microsoft.playwright.options.SelectOption()
                            .setIndex(((Number) m.get("index")).intValue()));
        } catch (Exception e) { lastPeriod = "(select failed: " + e.getMessage() + ")"; return lastPeriod; }
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__ctPeriod');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastPeriod = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("ComplaintType: Period = " + lastPeriod);
        return lastPeriod;
    }

    public boolean periodSelected() {
        return lastPeriod != null && !lastPeriod.startsWith("(");
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ComplaintType: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code) {
        ...
        System.out.println("ComplaintType: list check -> " + lastListCheck);
