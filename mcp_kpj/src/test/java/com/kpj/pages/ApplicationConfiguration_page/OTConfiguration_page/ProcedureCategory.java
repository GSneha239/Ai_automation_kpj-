...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>Type of Return</b> — Page Object.
 *
 ...
 */
public class ProcedureCategory extends BasePage {

    public ProcedureCategory(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; ProcedureCategory.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*procedure\\s*category\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__pcgMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__pcgMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ProcedureCategory.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__pcgMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ProcedureCategory.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ProcedureCategory screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("procedurecategory") || route.contains("proccategory")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ProcedureCategory CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?procedure|AddProcedureCategory|PROCCATEGORY/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__pcgAdd'; }");
        try {
            page.locator("#__pcgAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ProcedureCategory.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__pcgAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ProcedureCategory: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("ProcedureCategory: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__pcgMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__pcgMims';"
        ...
        try {
            page.locator("#__pcgMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__pcgMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("ProcedureCategory: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("ProcedureCategory: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        try {
            ...
            if (hit != null && !hit.toString().contains("FOUND:")) {
                ...
                // Still nothing. The grid is loaded once when the screen opens and is NOT refreshed by
                // the save, so what was read is the list as it stood BEFORE the record was written.
                // Reload the screen and read it again - that is the difference between "the save did
                // not happen" and "the flow was looking at a stale grid".
                if (unfiltered != null && !unfiltered.toString().contains("FOUND:")
                        && lastRoute != null && !lastRoute.isEmpty()) {
                    String hash = lastRoute.startsWith("#") ? lastRoute.substring(1) : lastRoute;
                    try { page.evaluate("(h) => { window.location.hash = h; }", hash); }
                    catch (Exception ignore) { }
                    waitForAngular(4000);
                    Object reloaded = page.evaluate("(a) => {" + JS
                            + " const rows=[...document.querySelectorAll('tr,.ui-grid-row')].filter(vis)"
                            + "   .map(t=>norm(t.textContent)).filter(t=>t);"
                            + " const withCode=rows.filter(t=>a.c && t.includes(a.c));"
                            + " const mine=a.v? withCode.find(t=>t.includes(a.v)) : withCode[0];"
                            + " if(mine) return 'FOUND: '+mine.slice(0,140);"
                            + " return 'still not there ('+rows.length+' rows after reloading)'; }",
                            java.util.Map.of("c", code, "v", remark == null ? "" : remark));
                    lastListCheck += "  ||  the grid had not been refreshed by the save, so the screen "
                            + "was RELOADED and read again -> " + (reloaded == null ? "" : reloaded);
                }
            }
        } catch (Exception e) {
            lastListCheck += "could not read the list: " + e.getMessage().split("\n")[0];
        }
        System.out.println("ProcedureCategory: list check -> " + lastListCheck);
