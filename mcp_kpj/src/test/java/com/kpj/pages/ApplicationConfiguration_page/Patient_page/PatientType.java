...
/**
 * Application Configuration &gt; Patient &gt; <b>Patient Type</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Patient Type</b> (route
 * {@code #/PATIENTSRC}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 * <b>Code*</b> + <b>Remark*</b>, select <b>Concession Template</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.PatientRelation} (this screen's exact ng-model
 * names were not yet confirmed live; sibling masters can live at routes that do not literally match their
 * menu label, so {@link #onScreen()} verifies primarily by field presence + URL, not header text alone — the
 * CommonMaster-family header can show the pre-selected Form Name rather than the menu label, per the
 * {@code Patient_page.Payor} / {@code Patient_page.CampType} lesson). The Concession Template SELECT is
 * matched by its own LABEL text. Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are
 * excluded.</p>
 */
public class PatientType extends BasePage {

    public PatientType(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/PATIENTSRC";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    public String lastConcessionTemplate = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks are real patient-type names, not generic filler text. */
    private static final String[] REMARKS = {
            "Walk-in Patient", "Referred Patient", "Follow-up Patient", "VIP Patient", "Corporate Patient", "Insurance Patient"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("PatientType.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*patient\\s*type\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("PatientType.nav DEBUG: visible links => " + dbg);
                    System.out.println("PatientType.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__ptMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("PatientType.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__ptMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__ptMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("PatientType.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("PatientType.nav: direct route " + route);
    ...
    /**
     * On the Patient Type screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/patientsrc/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/patient\\s*type/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__ptAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__ptAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("PatientType.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__ptAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    /** Select the first real option of the <b>Concession Template</b> dropdown, matched by its own LABEL text. */
    public String selectConcessionTemplate() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const e=sels.find(x=>/concession\\s*template/i.test(labelOf(x)) || /concessiontemplate/i.test(x.getAttribute('ng-model')||''));"
                + " if(!e) return '(not found)'; e.id='__ptConcSel';"
        ...
        if (idxM.find()) {
            ...
            try {
                page.locator("#__ptConcSel").selectOption(new com.microsoft.playwright.options.SelectOption().setIndex(idx),
                        new com.microsoft.playwright.Locator.SelectOptionOptions().setTimeout(6000));
            } catch (Exception e) { System.out.println("selectConcessionTemplate: real select failed - " + e.getMessage()); }
            waitForAngular(400);
            Object t = page.evaluate("() => { const e=document.getElementById('__ptConcSel'); return (e && e.selectedIndex>=0)?(e.options[e.selectedIndex].textContent||'').trim():''; }");
            res = t == null ? "" : t.toString();
        }
        page.evaluate("() => { const e=document.getElementById('__ptConcSel'); if(e) e.removeAttribute('id'); }");
        lastConcessionTemplate = res;
        return lastConcessionTemplate;
    }
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("patientsrc") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__ptSubmit'; return 'click'; }"
        ...
        System.out.println("PatientType submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__ptSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("PatientType submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ptToasts||[]).some(a=>/patient|type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("PatientType save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__ptToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
