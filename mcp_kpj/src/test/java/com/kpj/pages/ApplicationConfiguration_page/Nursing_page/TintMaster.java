...
/**
 * Application Configuration &gt; Nursing &gt; <b>Tint</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu) → <b>Tint</b>
 * ({@code #/Tint}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 ...
 * / Body Symptoms / Tint / … all share this exact generic commonmaster form, differing only in route, menu
 * text and code prefix).</p>
 *
 * <p><b>Known live risk (2026-08-20):</b> the sibling screens {@code Opthal Advice} and {@code Severity} both
 * hit an identical HTTP 500 {@code MappingException} on the SHARED {@code /api/CommonMaster/IUD} save
 * endpoint — a backend defect, not a per-screen one. If Tint fails the same way, {@link #lastSaveHttp} carries
 * the diagnostic HTTP body confirming it is the same systemic issue.</p>
 *
 ...
 */
public class TintMaster extends BasePage {

    public TintMaster(Page page) { super(page); }

    public static final String ROUTE = "#/Tint";
    ...
    /** Real lens-tint names, not generic filler text. */
    private static final String[] REMARKS = {"Grey", "Brown", "Green", "Blue", "Amber", "Rose", "Yellow", "Clear"};
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            // EXACT "Tint" — a loose match could pick up an unrelated word containing "tint".
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*tint(\\s*master)?\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/Tint'); if(!a) return ''; a.id='__tnMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("TintMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__tnMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("TintMaster.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__tnMenu'); if(e) e.removeAttribute('id'); }");
            try {
            ...
            } catch (Exception ignore) { System.out.println("TintMaster.nav: Tint screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    /**
     * On the REAL Tint screen. Checked by the screen's own <b>Remark</b> field
     * ({@code commonmaster.Description}) — NOT just the URL: the SPA can leave a previous screen mounted while the
     * hash reads {@code #/Tint}, and the leftover markup carries a Code + Store commonmaster form that a
     * Code-only check would happily accept.
     */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("tint")) return false;
    ...
    public String fillDetails() {
        String code = "TN" + String.format("%05d", Math.abs(System.nanoTime() % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__tnToasts=[]; if(window.__tnObs) window.__tnObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__tnToasts.includes(t)) window.__tnToasts.push(t); }); };"
                + " window.__tnObs=new MutationObserver(grab); window.__tnObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            Object res = page.evaluate("(a) => { const A=window.angular; const code=a.code, desc=a.remark;"
                    ...
                    + " const b=[...document.querySelectorAll('button[type=submit],input[type=submit]')].find(x=>/^\\s*submit\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(b){ b.id='__tnSubmit'; return 'click'; } return 'none'; }",
                    java.util.Map.of("code", lastCode, "remark", lastRemark));
            System.out.println("TintMaster submit[" + attempt + "] => " + res);
            if ("click".equals(String.valueOf(res))) {
                try { page.locator("#__tnSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(5000)); } catch (Exception ignore) { }
                page.evaluate("() => { const e=document.getElementById('__tnSubmit'); if(e) e.removeAttribute('id'); }");
            }
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|required|exist|error/i.test(el.textContent||''))"
                        + " || (window.__tnToasts||[]).some(a=>/tint|master|saved|success|added|updated|please|enter|required|exist|error/i.test(a))",
            ...
            System.out.println("TintMaster save HTTP => " + lastSaveHttp);
            Object t = page.evaluate("() => { const a=window.__tnToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>x) || ''; }");
            ...
            if (last.toLowerCase().matches(".*(exist|already).*")) {
                ...
                // unique, while still reading like configuration ("Grey 4821", not machine noise).
                lastRemark = lastRemark + " " + (Math.abs(System.nanoTime()) % 10000);
            }
            page.evaluate("() => { window.__tnToasts=[]; document.querySelectorAll('.toast-message,.toast').forEach(t=>t.remove()); }");
        }
