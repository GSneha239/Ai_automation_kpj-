...
/**
 * Application Configuration &gt; Patient &gt; <b>Mother Tongue</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Mother Tongue</b> (route
 * {@code #/MOTHERTONG}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.Language} (this screen's exact ng-model names were
 ...
 */
public class MotherTongue extends BasePage {

    public MotherTongue(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/MOTHERTONG";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks are real language names (mother tongues), not generic filler text. */
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("MotherTongue.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*mother\\s*tongue\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("MotherTongue.nav DEBUG: visible links => " + dbg);
                    System.out.println("MotherTongue.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                ...
                try { page.locator("#__mtMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("MotherTongue.nav: real click failed, dispatching in-page click - " + e.getMessage());
                ...
                System.out.println("MotherTongue.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("MotherTongue.nav: direct route " + route);
    ...
    /**
     * On the Mother Tongue screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/mothertong/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/mother\\s*tongue/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("MotherTongue.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("mothertong") || resp.status() >= 400)) return;
                String body = "";
        ...
        System.out.println("MotherTongue submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("MotherTongue submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__mtToasts||[]).some(a=>/mother|tongue|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("MotherTongue save HTTP => " + lastSaveHttp);
