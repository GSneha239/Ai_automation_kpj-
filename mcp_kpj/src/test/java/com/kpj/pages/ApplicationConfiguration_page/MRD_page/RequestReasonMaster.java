...
/**
 * Application Configuration &gt; MRD &gt; <b>Request Reason Master</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>MRD</b> (submenu) → <b>Request Reason Master</b>
 * ({@code #/RequestReasonMaster}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.
 ...
 */
public class RequestReasonMaster extends BasePage {

    public RequestReasonMaster(Page page) { super(page); }

    public static final String ROUTE = "#/RequestReasonMaster";
    ...
    private static final String[] REMARKS = {
            "Follow Up Consultation", "Medico Legal Request", "Insurance Claim",
            "Research Study", "Continuity Of Care", "Audit Review"};
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); document.querySelectorAll('#__rrMenu').forEach(e=>e.removeAttribute('id')); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*request\\s*reason(\\s*master)?\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/RequestReasonMaster'); if(!a) return ''; a.id='__rrMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("RequestReasonMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__rrMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(4000)); }
            catch (Exception e) { page.evaluate("() => { const a=document.getElementById('__rrMenu'); if(a) a.click(); }"); }
            page.evaluate("() => { const e=document.getElementById('__rrMenu'); if(e) e.removeAttribute('id'); }");
            try {
            ...
            } catch (Exception ignore) { System.out.println("RequestReasonMaster.nav: screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    /**
     * On the REAL Request Reason Master screen — verified by its own <b>Remark</b> field, NOT the URL (see the
     * class note on the leftover Code + Store commonmaster form).
     */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("requestreason")) return false;
    ...
    public String fillDetails() {
        String code = "RR" + String.format("%05d", Math.abs(System.nanoTime() % 100000));
        lastCode = code;
    ...
    /**
     * Submit the inline commonmaster form and return the toast. The Submit button has no {@code ng-click} — the
     * save is the form's {@code ng-submit} — so the handler is read off the form and invoked from its own scope,
     * with Code/Remark re-applied to EVERY scope's {@code commonmaster} first (the inline form re-renders on save
     * and can blank the Code), and retried: the async save occasionally yields no toast at all.
     */
    public String submitAndGetToast() {
        page.evaluate("() => { window.__rrToasts=[]; if(window.__rrObs) window.__rrObs.disconnect();"
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            Object res = page.evaluate("(a) => { const A=window.angular; const code=a.code, desc=a.remark;"
                    ...
                    + " const b=[...document.querySelectorAll('button[type=submit],input[type=submit],button')].find(x=>/^\\s*submit\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(b){ b.id='__rrSubmit'; return 'click'; } return 'none'; }",
                    java.util.Map.of("code", lastCode, "remark", lastRemark));
            System.out.println("RequestReasonMaster submit[" + attempt + "] => " + res);
            if ("click".equals(String.valueOf(res))) {
                try { page.locator("#__rrSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(5000)); } catch (Exception ignore) { }
                page.evaluate("() => { const e=document.getElementById('__rrSubmit'); if(e) e.removeAttribute('id'); }");
            }
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(el.textContent||''))"
                        + " || (window.__rrToasts||[]).some(a=>/request|reason|master|saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(a))",
                        null, new Page.WaitForFunctionOptions().setTimeout(9000));
            } catch (Exception ignore) { page.waitForTimeout(1500); }
            captureToastShot();
            Object t = page.evaluate("() => { const a=window.__rrToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>x) || ''; }");
            ...
            page.evaluate("() => { window.__rrToasts=[]; document.querySelectorAll('.toast-message,.toast').forEach(t=>t.remove()); }");
        }
        return last;
    }
}
