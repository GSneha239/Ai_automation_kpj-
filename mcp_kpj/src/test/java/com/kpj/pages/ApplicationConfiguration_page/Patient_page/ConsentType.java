...
/**
 * Application Configuration &gt; Patient &gt; <b>Consent Type</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Consent Type</b> → (Add, if
 * the screen has one) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>Confirmed a SEPARATE screen from its siblings on the same submenu, per {@code Patient_page.Consent}'s own
 * notes: <b>Consent Master</b> ({@code #/Consent}), <b>ConsentTXT</b> ({@code #/ConsentTXT}) and
 * <b>Consent Type</b> ({@code #/CONSNTTYP}) are three distinct screens — the menu link is matched on EXACT
 * text so "Consent Type" is never confused with the plain "Consent" link.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.CampType} (this screen's exact ng-model names were
 * not yet confirmed live; sibling masters can live at routes that do not literally match their menu label, so
 * {@link #onScreen()} verifies by the rendered HEADER text via a substring match, not an over-strict anchor —
 * see the lesson from {@code Patient_page.CampType}). Grid filter boxes ({@code colFilter.term}) and the
 * textAngular artefact are excluded.</p>
 */
public class ConsentType extends BasePage {

    public ConsentType(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/CONSNTTYP";
    ...
    private static final String[] REMARKS = {
            "Surgical Consent", "Anaesthesia Consent", "Blood Transfusion Consent",
            "Admission Consent", "Data Sharing Consent", "Treatment Consent"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                // poll for ANY visible link before trusting the "not found" result (learned from BlackListReason /
                // BloodGroup, where a query issued too early only saw 2 leftover top-level links).
                try {
                ...
                } catch (Exception ignore) { System.out.println("ConsentType.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT text match on a VISIBLE link ONLY — "Consent Type" must not match the sibling "Consent"
                // (Consent Master) or "ConsentTXT" links on the same submenu (learned from the Payor mix-up:
                // an unfiltered/loose match can pick up the WRONG sibling screen).
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*consent\\s*type\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__ctyMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("ConsentType.nav DEBUG: visible links => " + dbg);
                    System.out.println("ConsentType.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__ctyMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("ConsentType.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__ctyMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__ctyMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("ConsentType.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("ConsentType.nav: direct route " + route);
    ...
    /**
     * On the Consent Type screen — verified by a Code/Remark box actually present plus the URL (CommonMaster
     * header text on this app can show the pre-selected Form Name rather than the menu label — see the
     * Patient_page.Payor / Patient_page.CampType lesson — so field presence + URL are the real signals).
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const urlOk=/consnttyp|consenttype/i.test(location.hash||'');" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__ctyAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__ctyAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("ConsentType.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__ctyAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "CY" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__ctyToasts=[]; if(window.__ctyObs) window.__ctyObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__ctyToasts.includes(t)) window.__ctyToasts.push(t); }); };"
                + " window.__ctyObs=new MutationObserver(grab); window.__ctyObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("consnttyp") || resp.url().toLowerCase().contains("consenttype") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__ctySubmit'; return 'click'; }"
        ...
        System.out.println("ConsentType submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__ctySubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ConsentType submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__ctySubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ctyToasts||[]).some(a=>/consent|type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("ConsentType save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__ctyToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
