...
public class Rack extends BasePage {
    ...
    public Rack(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; Rack.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*rack\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__rckMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__rckMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Rack.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__rckMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("Rack.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the Rack screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("rack") || route.contains("rackmst")
    ...
    public String describeControls() {
        ...
        System.out.println("=== Rack CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?rack|AddRack|RACK/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__rckAdd'; }");
        try {
            page.locator("#__rckAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("Rack.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__rckAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("Rack: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("Rack: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__rckMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__rckMims';"
        ...
        try {
            page.locator("#__rckMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__rckMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("Rack: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("Rack: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("Rack: list check -> " + lastListCheck);
