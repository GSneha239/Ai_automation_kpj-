...
/**
 * Application Configuration &gt; Nursing &gt; <b>Tag Creation</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu, EXACT text — "Nursing Station" is a
 * different module and a prefix match hits it first) → <b>Tag Creation</b> (route {@code #/TagCreation},
 * best-guess fallback — navigation is primarily by the live menu link) → (Add, if the screen has one) → enter
 * <b>Code*</b> + <b>Description*</b> → <b>Save</b> → success toast.</p>
 *
 * <p>The Code / Description fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Nursing_page.Diagnosis} (this screen's exact ng-model names were
 * not yet confirmed live). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are
 * excluded.</p>
 *
 * <p><b>Known live risk (2026-08-20):</b> several sibling {@code commonmaster}-family screens (Opthal Advice,
 * Severity, Tint) all hit an identical HTTP 500 {@code MappingException} on the SHARED
 * {@code /api/CommonMaster/IUD} save endpoint — a backend defect, not a per-screen one (see
 * {@code devhis-commonmaster-iud-500} project memory). If Tag Creation fails the same way,
 * {@link #lastSaveHttp} carries the diagnostic HTTP body confirming it is the same systemic issue.</p>
 */
public class TagCreation extends BasePage {

    public TagCreation(Page page) { super(page); }

    /** Best-guess fallback — navigation is primarily by the live menu link, not this hardcoded route. */
    public static final String ROUTE = "#/TagCreation";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Realistic tag names, not "Auto ... <code>" filler. */
    private static final String[] REMARKS = {
            "High Risk", "Follow Up Required", "VIP Patient", "Fall Risk", "Isolation Required",
            "Allergy Alert", "Do Not Resuscitate", "Frequent Flyer", "Chronic Condition", "Palliative Care",
            "Interpreter Needed", "Wheelchair Access"
    };

    private static final int TOAST_SHOT_WAIT_MS = 5000;

    /** JS helper: the visible Code / Description inputs, whatever they are called on this screen. */
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("TagCreation.nav: Nursing submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*tag\\s*creation\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null)"
                        + "        || links.find(x=>/tagcreation/i.test(x.getAttribute('href')||'') && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__tgMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("TagCreation.nav DEBUG: visible links => " + dbg);
                    System.out.println("TagCreation.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__tgMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("TagCreation.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__tgMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__tgMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("TagCreation.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("TagCreation.nav: direct route " + route);
    ...
    /** On the Tag Creation screen — verified by a Code/Description box actually present plus the URL. */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/tagcreation/i.test((location.hash||'').replace(/\\s+/g,''));" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/tag\\s*creation/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__tgAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__tgAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("TagCreation.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__tgAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    /**
     * Enter <b>Code*</b> and <b>Description*</b>. {@code attempt} shifts BOTH values so a retry after an
    ...
    public String fillDetails(int attempt) {
        String code = "TG" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    /**
     * Click <b>Save</b> (accepts "Submit" too) and return the toast. Re-asserts Code/Description first — these
    ...
    public String submitAndGetToast() {
        ...
        // (See devhis-commonmaster-iud-500 project memory: several sibling screens hit the SHARED
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("tag") || resp.url().toLowerCase().contains("commonmaster") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__tgSave'; return 'click'; }"
        ...
        System.out.println("TagCreation submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__tgSave").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("TagCreation submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__tgSave'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__tgToasts||[]).some(a=>/tag|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("TagCreation save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__tgToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
