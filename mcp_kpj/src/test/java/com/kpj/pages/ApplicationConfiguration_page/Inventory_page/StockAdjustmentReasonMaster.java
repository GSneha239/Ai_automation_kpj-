 ...
 */
public class StockAdjustmentReasonMaster extends BasePage {

    public StockAdjustmentReasonMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; StockAdjustmentReasonMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*stock\\s*adjustment\\s*reason\\s*master\\s*$/i.test(norm(x.textContent)));"
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("StockAdjustmentReasonMaster.nav: " + e.getMessage().split("\n")[0]); }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("StockAdjustmentReasonMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the StockAdjustmentReasonMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("stockadjustment") || route.contains("adjustmentreason")
    ...
    public String describeControls() {
        ...
        System.out.println("=== StockAdjustmentReasonMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?stock|AddStockAdjustmentReasonMaster|STOCKADJ/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__sarAdd'; }");
        try {
            page.locator("#__sarAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("StockAdjustmentReasonMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__sarAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("StockAdjustmentReasonMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("StockAdjustmentReasonMaster: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__sarMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__sarMims';"
        ...
        try {
            page.locator("#__sarMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__sarMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("StockAdjustmentReasonMaster: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("StockAdjustmentReasonMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("StockAdjustmentReasonMaster: list check -> " + lastListCheck);
