...
/**
 * Application Configuration &gt; Diet and Nutrition &gt; <b>Diet Type Master</b> — configuration screen Page
 * Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Diet and Nutrition</b> (submenu) → <b>Diet Type Master</b> →
 ...
 * {@code Investigation_page.Agency} / {@code DietNutrition_page.MealTypeMaster} (this screen's exact route and
 ...
 */
public class DietTypeMaster extends BasePage {

    public DietTypeMaster(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/DietTypeMaster";
    ...
    private static final String[] REMARKS = {
            "Regular Diet", "Diabetic Diet", "Low Sodium Diet", "Renal Diet", "Soft Diet", "Liquid Diet"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                waitForAngular(1000);
                // EXACT text match ONLY, ANCHORED — "Diet Type Master" must not match any sibling diet-related link.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*diet\\s*type\\s*master\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__dtmMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("DietTypeMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__dtmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("DietTypeMaster.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__dtmMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__dtmMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("DietTypeMaster.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("DietTypeMaster.nav: direct route " + route);
    ...
    /**
     * On the Diet Type Master screen — verified by the rendered HEADER text rather than a guessed URL
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/diettype/i.test(location.hash||'');"
                + " if(!/diet\\s*type/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__dtmAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__dtmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("DietTypeMaster.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__dtmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "DTM" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__dtmToasts=[]; if(window.__dtmObs) window.__dtmObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__dtmToasts.includes(t)) window.__dtmToasts.push(t); }); };"
                + " window.__dtmObs=new MutationObserver(grab); window.__dtmObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("diettype") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__dtmSubmit'; return 'click'; }"
        ...
        System.out.println("DietTypeMaster submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__dtmSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("DietTypeMaster submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__dtmSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__dtmToasts||[]).some(a=>/diet|type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("DietTypeMaster save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__dtmToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
