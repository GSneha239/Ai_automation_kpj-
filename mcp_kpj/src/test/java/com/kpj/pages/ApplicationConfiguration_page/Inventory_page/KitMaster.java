...
/**
 * Application Configuration &gt; Inventory &gt; <b>KitMaster</b> — Page Object.
 *
 ...
 */
public class KitMaster extends BasePage {

    public KitMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; KitMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*kit\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__kmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("KitMaster.nav: the KIT Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__kmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("KitMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__kmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("KitMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the KitMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("kitmaster") || route.contains("kit")
    ...
    public String describeControls() {
        ...
        System.out.println("=== KitMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?kit|AddKitMaster|KITMASTER/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__kmAdd'; }");
        try {
            page.locator("#__kmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("KitMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__kmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("KitMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    // ---- KIT Master's own fields ----------------------------------------

    public String lastKitEntry = "", lastDrug = "", lastQty = "", lastItemAdd = "", lastItemRowText = "";
    ...
    /** Enter the <b>KIT Code</b> and <b>KIT Name</b> ({@code kit.code} / {@code kit.name}). */
    public String enterKitDetails(String code, String name) {
        String r1 = put("kit.code", code);
        String r2 = put("kit.name", name);
        lastKitEntry = "KITCode=" + r1 + " | KITName=" + r2;
        waitForAngular(500);
        System.out.println("KitMaster: " + lastKitEntry);
        return lastKitEntry;
    }

    public boolean kitDetailsEntered(String code, String name) {
        return lastKitEntry != null && !lastKitEntry.contains("(no field")
                && lastKitEntry.contains("KITCode=" + code) && lastKitEntry.contains("KITName=" + name);
    }

    /**
     * Select a <b>Drug Name</b> in the KIT Information list.
     *
     * <p>{@code kit.itemPicker} is a uib-typeahead over {@code drpItemList}. It opens only for REAL
     * keystrokes — writing the model directly, as every other field on these screens allows, leaves the
     * typeahead closed and the model holding a plain string instead of the item it must bind to, and Add
     * then has nothing to add. So an existing drug is read out of the list first, its opening characters
     * are typed key by key, and the matching suggestion is clicked.</p>
     */
    public String selectDrug() {
        // The list is fetched after the form renders, so it is polled rather than read once: a single
        // early read reports "no drugs" for a list that simply had not arrived.
        Object info = null;
        int count = 0;
        String wanted = "";
        ...
        while (System.currentTimeMillis() < deadline) {
            info = page.evaluate("() => {" + JS
                    + " const e=[...document.querySelectorAll(\"[ng-model='kit.itemPicker']\")]"
                    + "   .filter(vis)[0];"
                    + " if(!e) return null;"
                    + " let list=[];"
                    + " try{ list=angular.element(e).scope().$eval('drpItemList')||[]; }catch(err){}"
                    + " const texts=list.slice(0,400).map(i=>(i&&(i.text||i.Text||i.name))||'')"
                    + "   .filter(t=>t);"
                    + " return {count:list.length, first:(texts[0]||'')}; }");
            if (info == null) break;
            @SuppressWarnings("unchecked")
            java.util.Map<String, Object> probe = (java.util.Map<String, Object>) info;
            count = ((Number) probe.get("count")).intValue();
            wanted = String.valueOf(probe.get("first"));
            if (count > 0 && !wanted.isEmpty()) break;
            page.waitForTimeout(500);
        }
        if (info == null) { lastDrug = "(no drug picker on this form)"; return lastDrug; }

        if (count == 0 || wanted.isEmpty()) {
            // Empty after polling. Type anyway with real keys — a picker that searches the SERVER on each
            // keystroke would still open, and that tells apart "no data" from "the list never loaded".
            String probeTyped = typeAndDescribePopup("a");
            lastDrug = "(the drug list drpItemList was still EMPTY after 20s — " + count + " items. "
                    + "Typing a real keystroke into the box then gave: " + probeTyped + ")";
            System.out.println("KitMaster: drug -> " + lastDrug);
            return lastDrug;
        }

        // Type enough of the name to narrow the list, with real keys so the typeahead opens.
        String prefix = wanted.length() > 6 ? wanted.substring(0, 6) : wanted;
        com.microsoft.playwright.Locator box = page.locator("input[ng-model='kit.itemPicker']").first();
        try {
            box.click();
            box.fill("");
            box.type(prefix, new com.microsoft.playwright.Locator.TypeOptions().setDelay(120));
        } catch (Exception e) {
            lastDrug = "(typing into the drug box failed: " + e.getMessage().split("\n")[0] + ")";
            return lastDrug;
        }
        page.waitForTimeout(1500);

        Object picked = page.evaluate("(want) => {" + JS
                + " const pops=[...document.querySelectorAll("
                + "   'ul.dropdown-menu,[uib-typeahead-popup],ul.typeahead')].filter(vis);"
                + " for(const p of pops){"
                + "   const items=[...p.querySelectorAll('li')].filter(vis);"
                + "   if(!items.length) continue;"
                + "   const hit=items.find(li=>norm(li.textContent)===want) || items[0];"
                + "   const t=norm(hit.textContent);"
                + "   const a=hit.querySelector('a')||hit; a.click();"
                + "   return 'the suggestion list opened with '+items.length+' item(s); clicked \"'+t+'\"'; }"
                + " return ''; }", wanted);
        String how = picked == null ? "" : picked.toString();
        waitForAngular(1000);

        Object back = page.evaluate("() => {" + JS
                + " const e=[...document.querySelectorAll(\"[ng-model='kit.itemPicker']\")].filter(vis)[0];"
                + " if(!e) return '(gone)';"
                + " let model='(unreadable)';"
                + " try{ const c=angular.element(e).controller('ngModel');"
                + "      const v=c? c.$modelValue : null;"
                + "      model=(v&&typeof v==='object')"
                + "        ? ('the item object, text=\"'+(v.text||v.Text||'')+'\"') : ('a plain '+typeof v"
                + "          +' : '+String(v)); }catch(err){}"
                + " return 'the box now reads \"'+(e.value||'')+'\" and its model holds '+model; }");
        lastDrug = (how.isEmpty() ? "NO suggestion list opened after typing \"" + prefix + "\"" : how)
                + "; " + (back == null ? "" : back.toString())
                + " (chosen from the " + count + " drugs the screen offers)";
        System.out.println("KitMaster: drug -> " + lastDrug);
        return lastDrug;
    }

    /** Type real keys into the drug box and say what, if anything, the picker offered. */
    private String typeAndDescribePopup(String text) {
        try {
            com.microsoft.playwright.Locator box = page.locator("input[ng-model='kit.itemPicker']").first();
            box.click();
            box.fill("");
            box.type(text, new com.microsoft.playwright.Locator.TypeOptions().setDelay(150));
        } catch (Exception e) {
            return "typing failed: " + e.getMessage().split("\n")[0];
        }
        page.waitForTimeout(2500);
        Object r = page.evaluate("() => {" + JS
                + " const pops=[...document.querySelectorAll("
                + "   'ul.dropdown-menu,[uib-typeahead-popup],ul.typeahead')].filter(vis);"
                + " const items=pops.flatMap(p=>[...p.querySelectorAll('li')].filter(vis));"
                + " return items.length? ('the picker opened with '+items.length+' suggestion(s): '"
                + "   +items.slice(0,5).map(li=>norm(li.textContent)).join(', '))"
                + "   : 'the picker offered NOTHING — no suggestion list opened at all'; }");
        return r == null ? "" : r.toString();
    }

    /** A drug is really bound — the box shows it AND the model holds the item, not a typed string. */
    public boolean drugSelected() {
        return lastDrug != null && lastDrug.contains("model holds the item object")
                && !lastDrug.startsWith("(");
    }

    /** Enter the <b>Quantity</b> ({@code kit.quantity}). */
    public String enterQuantity(String qty) {
        lastQty = put("kit.quantity", qty);
        waitForAngular(400);
        System.out.println("KitMaster: quantity = " + lastQty);
        return lastQty;
    }

    public boolean quantityEntered(String qty) {
        return lastQty != null && !lastQty.startsWith("(no field") && lastQty.startsWith(qty);
    }

    /**
     * Click <b>Add</b> ({@code fnAddKitItem}) and count the KIT Information rows either side.
     *
     * <p>The click is never credited on its own — the row count before and after is what says the drug
     * was added to the kit, and the row is read back so it can be checked against what was entered.</p>
     */
    public String addKitItem() {
        itemRowsBefore = itemGridRows();
        page.evaluate("() => {" + JS
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].filter(vis)"
                + "   .find(x=>/fnAddKitItem/i.test(x.getAttribute('ng-click')||''));"
                + " if(b) b.click(); }");
        waitForAngular(2000);
        itemRowsAfter = itemGridRows();
        ...
        lastItemAdd = "KIT Information rows " + itemRowsBefore + " -> " + itemRowsAfter
                + (itemRowsAfter > itemRowsBefore
                    ? "; the new row reads: " + lastItemRowText
                    : "; NO row was added (rows shown: " + lastItemRowText + ")");
        System.out.println("KitMaster: add item -> " + lastItemAdd);
        return lastItemAdd;
    }
    ...
    /** The added row carries the quantity that was entered. */
    public boolean itemRowShows(String qty) {
        return lastItemRowText != null && lastItemRowText.contains(qty);
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("KitMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("KitMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__kmSeen = [];"
                    + " if(window.__kmObs) window.__kmObs.disconnect();"
                    + " window.__kmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__kmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__kmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__kmSeen||[];"
                    + " if(window.__kmObs) window.__kmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("KitMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("KitMaster: list check -> " + lastListCheck);
