...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>AnesthesiaNotesMaster</b> — Page Object.
 *
 ...
 */
public class AnesthesiaNotesMaster extends BasePage {

    public AnesthesiaNotesMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; AnesthesiaNotesMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*anesthesia\\s*notes\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__anmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("AnesthesiaNotesMaster.nav: the Anesthesia Notes Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__anmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("AnesthesiaNotesMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__anmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("AnesthesiaNotesMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the AnesthesiaNotesMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("anesthesia") || route.contains("anaesthesia")
    ...
    public String describeControls() {
        ...
        System.out.println("=== AnesthesiaNotesMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?anes|AddAnesthesiaNotesMaster|ANESTHESIA/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__anmAdd'; }");
        try {
            page.locator("#__anmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("AnesthesiaNotesMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__anmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String shortDescription) {
        Object r = page.evaluate("(a) => {" + JS
                + " const get=m=>[...document.querySelectorAll(\"[ng-model='AnesthesiaNotesMaster.\"+m+\"']\")]"
                + "   .filter(vis)[0];"
                + " const c=get('code'), d=get('shortdescription');"
                + " const rc = c? setEl(c,a[0])+' [AnesthesiaNotesMaster.code]' : '(no-code-field)';"
                + " const rd = d? setEl(d,a[1])+' [AnesthesiaNotesMaster.shortdescription]'"
        ...
        waitForAngular(500);
        System.out.println("AnesthesiaNotesMaster: " + lastEntry);
        return lastEntry;
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("AnesthesiaNotesMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("AnesthesiaNotesMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__anmSeen = [];"
                    + " if(window.__anmObs) window.__anmObs.disconnect();"
                    + " window.__anmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__anmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__anmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__anmSeen||[];"
                    + " if(window.__anmObs) window.__anmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("AnesthesiaNotesMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("AnesthesiaNotesMaster: list check -> " + lastListCheck);
