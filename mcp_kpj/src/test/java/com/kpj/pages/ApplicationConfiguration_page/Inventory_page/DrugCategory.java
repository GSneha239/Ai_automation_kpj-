...
/**
 * Application Configuration &gt; Inventory &gt; <b>DrugCategory</b> — Page Object.
 *
 ...
 */
public class DrugCategory extends BasePage {

    public DrugCategory(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Inventory &rarr; DrugCategory.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*drug\\s*category\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__dcMenu'; return a.getAttribute('href')||''; }");
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("DrugCategory.nav: " + e.getMessage().split("\n")[0]); }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("DrugCategory.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the DrugCategory screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("drugcategory") || route.contains("drugcat")
    ...
    public String describeControls() {
        ...
        System.out.println("=== DrugCategory CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?drug|AddDrugCategory|DRUGCAT/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__dcAdd'; }");
        try {
            page.locator("#__dcAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("DrugCategory.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("DrugCategory: " + lastEntry);
        return lastEntry;
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("DrugCategory: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("DrugCategory: confirm -> no confirmation dialog appeared within 10s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__dcSeen = [];"
                    + " if(window.__dcObs) window.__dcObs.disconnect();"
                    + " window.__dcObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__dcSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__dcObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__dcSeen||[];"
                    + " if(window.__dcObs) window.__dcObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("DrugCategory: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("DrugCategory: list check -> " + lastListCheck);
