...
/**
 * Application Configuration &gt; Patient &gt; <b>Patient Relation</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Patient Relation</b> (route
 * {@code #/PATIENTREL}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.PatientInstruction} (this screen's exact ng-model
 ...
 */
public class PatientRelation extends BasePage {

    public PatientRelation(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/PATIENTREL";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks are real relationship names, not generic filler text. */
    private static final String[] REMARKS = {
            "Father", "Mother", "Spouse", "Sibling", "Son", "Daughter", "Guardian", "Friend",
            "Grandfather", "Grandmother", "Uncle", "Aunt", "Cousin", "Nephew", "Niece",
            "Brother-in-law", "Sister-in-law", "Employer", "Colleague", "Legal Representative"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("PatientRelation.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*patient\\s*relation\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("PatientRelation.nav DEBUG: visible links => " + dbg);
                    System.out.println("PatientRelation.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                ...
                try { page.locator("#__prMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("PatientRelation.nav: real click failed, dispatching in-page click - " + e.getMessage());
                ...
                System.out.println("PatientRelation.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("PatientRelation.nav: direct route " + route);
    ...
    /**
     * On the Patient Relation screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/patientrel/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/patient\\s*relation/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("PatientRelation.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String fillDetails(int attempt) {
        String code = "PR" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("patientrel") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__prSubmit'; return 'click'; }"
        ...
        System.out.println("PatientRelation submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("PatientRelation submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__prToasts||[]).some(a=>/patient|relation|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("PatientRelation save HTTP => " + lastSaveHttp);
