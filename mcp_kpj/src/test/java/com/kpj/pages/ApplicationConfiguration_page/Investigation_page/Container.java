...
/**
 * Application Configuration &gt; Investigation &gt; <b>Container</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Container</b> → (Add, if
 * the screen has one) → enter <b>Code*</b> + <b>Remark*</b> → select <b>Colour</b> (enter R/G/B values) →
 ...
 * {@code Investigation_page.Agency} (this screen's exact route and ng-model names were not yet confirmed
 * live). The Colour control is also discovered: this app family typically exposes either three separate
 * R/G/B numeric boxes or a single native {@code input[type=color]} swatch — {@link #selectColour()} tries
 * the R/G/B boxes first (matching this task's "enter rgb values" wording) and falls back to a colour swatch
 * input if that is what the screen actually renders. {@link #describeForm()} dumps every visible field/button
 * so a naming mismatch is diagnosable from the report without a second live session.</p>
 */
public class Container extends BasePage {

    public Container(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/Container";
    ...
    private static final String[] REMARKS = {
            "Red Top Tube", "Purple Top Tube (EDTA)", "Blue Top Tube (Citrate)",
            "Green Top Tube (Heparin)", "Grey Top Tube (Fluoride)", "Yellow Top Tube (SST)"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    ...
                    + " const a = links.find(x=>/^\\s*container\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__ctnMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("Container.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__ctnMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Container.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__ctnMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Container.nav: direct route " + route);
    ...
    /**
     * On the Container screen — verified by the rendered HEADER text rather than a guessed URL substring
     * (sibling masters can live at routes that do not literally match their menu label), plus a Code/Remark
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/container/i.test(location.hash||'');"
                + " if(!/container/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__ctnAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__ctnAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Container.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__ctnAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    /**
     * Select <b>Colour</b> by entering R/G/B values. Tries, in order: (1) three separate numeric/text boxes
     * whose ng-model/placeholder/label match red/green/blue or a trailing .R/.G/.B, (2) a single native
     * {@code input[type=color]} swatch (set via hex, since that is the only value such a control accepts).
     */
    public String selectColour() {
        int red = 30 + (int) (Math.abs(System.nanoTime() / 7) % 200);
        int green = 30 + (int) (Math.abs(System.nanoTime() / 11) % 200);
        int blue = 30 + (int) (Math.abs(System.nanoTime() / 13) % 200);
        ...
        Object r = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const boxes=[...document.querySelectorAll('input')].filter(e=>e.offsetParent!==null && (e.type==='number'||e.type==='text'));"
                + " const byRe=(re)=>boxes.find(e=>re.test(e.getAttribute('ng-model')||'') || re.test(e.placeholder||'') || re.test(labelOf(e)));"
                + " const rEl=byRe(/\\bred\\b|\\.r$|^r$/i), gEl=byRe(/\\bgreen\\b|\\.g$|^g$/i), bEl=byRe(/\\bblue\\b|\\.b$|^b$/i);"
                + " const setVal=(e,v)=>{ if(!e) return null; const c=A.element(e).controller('ngModel'); e.value=v; if(c){c.$setViewValue(String(v));c.$render();} e.dispatchEvent(new Event('input',{bubbles:true})); e.dispatchEvent(new Event('change',{bubbles:true})); return v; };"
                + " if(rEl && gEl && bEl){ const rv=setVal(rEl,a.r), gv=setVal(gEl,a.g), bv=setVal(bEl,a.b);"
                + "   return 'R='+rv+' G='+gv+' B='+bv+' [RGB boxes: '+(rEl.getAttribute('ng-model')||'?')+' / '+(gEl.getAttribute('ng-model')||'?')+' / '+(bEl.getAttribute('ng-model')||'?')+']'; }"
                + " const colorEl=[...document.querySelectorAll('input[type=color]')].find(e=>e.offsetParent!==null);"
                + " if(colorEl){ const c=A.element(colorEl).controller('ngModel'); colorEl.value=a.hex; if(c){c.$setViewValue(a.hex);c.$render();} colorEl.dispatchEvent(new Event('input',{bubbles:true})); colorEl.dispatchEvent(new Event('change',{bubbles:true}));"
                + "   return 'Colour(hex)='+a.hex+' [swatch: '+(colorEl.getAttribute('ng-model')||'?')+']'; }"
                + " return '(not found)'; }",
                java.util.Map.of("r", red, "g", green, "b", blue, "hex", hex));
        waitForAngular(400);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__ctnToasts=[]; if(window.__ctnObs) window.__ctnObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__ctnToasts.includes(t)) window.__ctnToasts.push(t); }); };"
                + " window.__ctnObs=new MutationObserver(grab); window.__ctnObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("container") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__ctnSubmit'; return 'click'; }"
        ...
        System.out.println("Container submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__ctnSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Container submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__ctnSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ctnToasts||[]).some(a=>/container|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Container save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__ctnToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
