...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>ProcedureSubcategoryMaster</b> — Page Object.
 *
 ...
 */
public class ProcedureSubcategoryMaster extends BasePage {

    public ProcedureSubcategoryMaster(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; ProcedureSubcategoryMaster.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*procedure\\s*sub\\s*category\\s*master\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__psmMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("ProcedureSubcategoryMaster.nav: the Procedure Subcategory Master link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__psmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ProcedureSubcategoryMaster.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__psmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ProcedureSubcategoryMaster.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ProcedureSubcategoryMaster screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("proceduresubcategory") || route.contains("subcategory")
                || route.contains("procedure")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ProcedureSubcategoryMaster CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?proc|AddProcedureSubcategoryMaster|SUBCATEGORY/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__psmAdd'; }");
        try {
            page.locator("#__psmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ProcedureSubcategoryMaster.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__psmAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterCodeAndRemark(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ProcedureSubcategoryMaster: " + lastEntryPinned);
    ...
    public String selectCategory(int seed) {
        Object info = page.evaluate("() => {" + JS
                ...
                + "   || sels[0];"
                + " if(!e) return null; e.id='__psmCategory';"
        ...
        try {
            page.locator("#__psmCategory").selectOption(
        ...
        waitForAngular(800);
        Object t = page.evaluate("() => { const e=document.getElementById('__psmCategory');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastCategory = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count")
                + " categories]";
        System.out.println("ProcedureSubcategoryMaster: category -> " + lastCategory);
        return lastCategory;
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("ProcedureSubcategoryMaster: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("ProcedureSubcategoryMaster: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__psmSeen = [];"
                    + " if(window.__psmObs) window.__psmObs.disconnect();"
                    + " window.__psmObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__psmSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__psmObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__psmSeen||[];"
                    + " if(window.__psmObs) window.__psmObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("ProcedureSubcategoryMaster: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ProcedureSubcategoryMaster: list check -> " + lastListCheck);
