...
/**
 * Application Configuration &gt; Nursing &gt; <b>Allergy</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu) → <b>Allergy</b>
 * ({@code #/Allergy}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 ...
 */
public class AllergyMaster extends BasePage {

    public AllergyMaster(Page page) { super(page); }

    public static final String ROUTE = "#/Allergy";
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*allergy(\\s*master)?\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/Allergy'); if(!a) return ''; a.id='__alMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("AllergyMaster.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__alMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("AllergyMaster.nav: click failed - " + e.getMessage()); }
            ...
            } catch (Exception ignore) { System.out.println("AllergyMaster.nav: Allergy screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
     ...
     * hash reads {@code #/Allergy}, and the leftover markup carries a Code + Store commonmaster form that a
     * Code-only check would happily accept.
     */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("allergy")) return false;
    ...
    public String fillDetails() {
        ...
        lastCode = code;
        lastRemark = new String[]{"Peanut","Penicillin","Dust Mite","Seafood","Latex","Pollen"}[(int)(Math.abs(System.nanoTime())%6)];
    ...
    public String submitAndGetToast() {
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            Object res = page.evaluate("(a) => { const A=window.angular; const code=a.code, desc=a.remark;"
                    ...
                    + " const b=[...document.querySelectorAll('button[type=submit],input[type=submit]')].find(x=>/^\\s*submit\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(b){ b.id='__alSubmit'; return 'click'; } return 'none'; }",
                    java.util.Map.of("code", lastCode, "remark", lastRemark));
            System.out.println("AllergyMaster submit[" + attempt + "] => " + res);
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|required|exist|error/i.test(el.textContent||''))"
                        + " || (window.__alToasts||[]).some(a=>/allergy|master|saved|success|added|updated|please|enter|required|exist|error/i.test(a))",
