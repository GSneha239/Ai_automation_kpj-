...
/**
 * Application Configuration &gt; Investigation &gt; <b>HistoTemplate</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>HistoTemplate</b> → (Add, if the
 ...
 */
public class HistoTemplate extends BasePage {

    public HistoTemplate(Page page) { super(page); }

    public static final String ROUTE = "#/HistoTemplate";
    ...
    private static final String[] NAMES = {
            "Biopsy Report", "Frozen Section Report", "Cytology Report",
            "Immunohistochemistry Report", "Bone Marrow Report", "Surgical Pathology Report"
    };

    /** Remarks — a short note a person would actually type on this screen. */
    private static final String[] REMARKS = {
            "Routine histopathology reporting", "Used for urgent frozen sections",
            "Standard cytology reporting format", "For staged tumour reporting",
            "Haematology referral template", "General surgical specimen reporting"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                waitForAngular(1500);
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*histo\\s*template\\s*$/i.test(norm(x.textContent)) || (/histo/i.test(x.getAttribute('href')||'') && /template/i.test(x.getAttribute('href')||''))); if(!a) return ''; a.id='__htMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("HistoTemplate.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__htMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    ...
                    System.out.println("HistoTemplate.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__htMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__htMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("HistoTemplate.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("HistoTemplate.nav: direct route " + route);
    ...
    /** On the HistoTemplate screen — URL plus something real on the page (a Code/Remark box or an Add button). */
    public boolean onScreen() {
        // BOTH words: a bare "histo" test also accepts #/HistoDashboard (the Histopath Dashboard), which is a
        // real, working screen sitting earlier in the menu — that is what a loose match landed on first.
        String u = page.url().toLowerCase();
        if (!(u.contains("histo") && u.contains("template"))) return false;
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__htAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__htAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("HistoTemplate.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__htAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "HT" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__htToasts=[]; if(window.__htObs) window.__htObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__htToasts.includes(t)) window.__htToasts.push(t); }); };"
                + " window.__htObs=new MutationObserver(grab); window.__htObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("histo") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS + SET_DESC
                ...
                + " if(b){ b.id='__htSubmit'; return 'click'; }"
        ...
        System.out.println("HistoTemplate submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__htSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("HistoTemplate submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__htSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__htToasts||[]).some(a=>/histo|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("HistoTemplate save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__htToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
