...
/**
 * Application Configuration &gt; Investigation &gt; <b>Lab Unit</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Lab Unit</b> → (Add, if the
 ...
 * {@code Investigation_page.Agency} / {@code Investigation_page.Film} (this screen's exact route and ng-model
 * names were not yet confirmed live; sibling masters can live at routes that do not literally match their menu
 * label, so {@link #onScreen()} verifies by the rendered HEADER text, not a guessed URL substring). Grid filter
 ...
 */
public class LabUnit extends BasePage {

    public LabUnit(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/LabUnit";
    ...
    private static final String[] REMARKS = {
            "mg/dL", "mmol/L", "g/dL", "IU/L", "mIU/mL", "ng/mL", "U/L", "%"
    };
        ...
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*lab\\s*unit\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__luMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("LabUnit.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__luMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    ...
                    System.out.println("LabUnit.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__luMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__luMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("LabUnit.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("LabUnit.nav: direct route " + route);
    ...
    /**
     * On the Lab Unit screen — verified by the rendered HEADER text rather than a guessed URL substring
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/labunit/i.test(location.hash||'');"
                + " if(!/lab\\s*unit/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__luAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__luAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("LabUnit.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__luAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "LU" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__luToasts=[]; if(window.__luObs) window.__luObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__luToasts.includes(t)) window.__luToasts.push(t); }); };"
                + " window.__luObs=new MutationObserver(grab); window.__luObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("labunit") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__luSubmit'; return 'click'; }"
        ...
        System.out.println("LabUnit submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__luSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("LabUnit submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__luSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__luToasts||[]).some(a=>/lab|unit|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("LabUnit save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__luToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
