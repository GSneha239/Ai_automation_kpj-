...
/**
 * Application Configuration &gt; Blood Bank &gt; <b>Red Cell Serology Group</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Red Cell Serology Group</b> → (Add, if
 ...
 * / {@code BloodBank_page.DonorRejectionReason} (this screen's exact route and ng-model names were not yet
 * confirmed live; sibling masters can live at routes that do not literally match their menu label — e.g.
 * Cancellation Reason is {@code #/CANCELLATION} — so {@link #onScreen()} verifies by the rendered HEADER
 * text, not a guessed URL substring). Grid filter boxes ({@code colFilter.term}) and the textAngular
 * artefact are excluded.</p>
 */
public class RedCellSerologyGroup extends BasePage {

    public RedCellSerologyGroup(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/RedCellSerologyGroup";
    ...
    private static final String[] REMARKS = {
            "Rh Group", "ABO Group", "Kell Group", "Duffy Group", "Kidd Group"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    ...
                    + " const a = links.find(x=>/^\\s*red\\s*cell\\s*serology\\s*group\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__rcsMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("RedCellSerologyGroup.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__rcsMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("RedCellSerologyGroup.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__rcsMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("RedCellSerologyGroup.nav: direct route " + route);
    ...
    /**
     * On the Red Cell Serology Group screen — verified by the rendered HEADER text rather than a guessed URL
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/redcellserology/i.test(location.hash||'');"
                + " if(!/red\\s*cell\\s*serology/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__rcsAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__rcsAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("RedCellSerologyGroup.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__rcsAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "RC" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__rcsToasts=[]; if(window.__rcsObs) window.__rcsObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__rcsToasts.includes(t)) window.__rcsToasts.push(t); }); };"
                + " window.__rcsObs=new MutationObserver(grab); window.__rcsObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__rcsSubmit'; return 'click'; }"
        ...
        System.out.println("RedCellSerologyGroup submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__rcsSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("RedCellSerologyGroup submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__rcsSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__rcsToasts||[]).some(a=>/red\\s*cell|serology|group|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("RedCellSerologyGroup save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__rcsToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
