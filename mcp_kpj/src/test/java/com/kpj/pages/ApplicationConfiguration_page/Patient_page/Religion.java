...
/**
 * Application Configuration &gt; Patient &gt; <b>Religion</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Religion</b> (route
 * {@code #/Religion}, best-guess fallback — navigation is primarily by the live menu link) →
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.Race} (this screen's exact ng-model names were
 * not yet confirmed live; sibling masters can live at routes that do not literally match their menu label,
 * so {@link #onScreen()} verifies primarily by field presence + URL, not header text alone — the
 ...
 */
public class Religion extends BasePage {

    public Religion(Page page) { super(page); }

    /** Best-guess fallback — navigation is primarily by the live menu link, not this hardcoded route. */
    public static final String ROUTE = "#/Religion";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /**
     * Remarks are real religion names, not generic filler text. Core demographic masters (Nationality,
     * Patient Relation, Language, Race) turned out to be comprehensively pre-loaded, so this pool is
     * intentionally larger to reliably find one not yet used.
     */
    private static final String[] REMARKS = {
            "Islam", "Buddhism", "Christianity", "Hinduism", "Sikhism", "Taoism", "Confucianism",
            "Judaism", "Bahai", "Animism", "Atheist", "Free Thinker"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("Religion.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*religion\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__relMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("Religion.nav DEBUG: visible links => " + dbg);
                    System.out.println("Religion.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__relMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("Religion.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__relMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__relMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("Religion.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Religion.nav: direct route " + route);
    ...
    /**
     * On the Religion screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/religion/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/^\\s*religion\\s*$/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__relAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__relAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Religion.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__relAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__relToasts=[]; if(window.__relObs) window.__relObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__relToasts.includes(t)) window.__relToasts.push(t); }); };"
                + " window.__relObs=new MutationObserver(grab); window.__relObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("religion") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__relSubmit'; return 'click'; }"
        ...
        System.out.println("Religion submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__relSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Religion submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__relSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__relToasts||[]).some(a=>/religion|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Religion save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__relToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
