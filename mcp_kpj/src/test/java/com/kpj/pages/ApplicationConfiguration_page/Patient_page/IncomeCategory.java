...
/**
 * Application Configuration &gt; Patient &gt; <b>Income Category</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Income Category</b> (route
 * {@code #/INCOMECAT}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 ...
 * {@code Investigation_page.Agency} / {@code Patient_page.Gender} (this screen's exact ng-model names were
 ...
 */
public class IncomeCategory extends BasePage {

    public IncomeCategory(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/INCOMECAT";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Remarks are real income-band names, not generic filler text. */
    private static final String[] REMARKS = {
            "Low Income", "Middle Income", "High Income", "Below Poverty Line", "Fixed Income", "Variable Income"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("IncomeCategory.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*income\\s*category\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                        + " if(!a) return ''; a.id='__icMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("IncomeCategory.nav DEBUG: visible links => " + dbg);
                    System.out.println("IncomeCategory.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__icMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("IncomeCategory.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__icMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__icMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("IncomeCategory.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("IncomeCategory.nav: direct route " + route);
    ...
    /**
     * On the Income Category screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/incomecat/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/income\\s*cat/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__icAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__icAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("IncomeCategory.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__icAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "IC" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__icToasts=[]; if(window.__icObs) window.__icObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__icToasts.includes(t)) window.__icToasts.push(t); }); };"
                + " window.__icObs=new MutationObserver(grab); window.__icObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("incomecat") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__icSubmit'; return 'click'; }"
        ...
        System.out.println("IncomeCategory submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__icSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("IncomeCategory submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__icSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__icToasts||[]).some(a=>/income|category|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("IncomeCategory save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__icToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
