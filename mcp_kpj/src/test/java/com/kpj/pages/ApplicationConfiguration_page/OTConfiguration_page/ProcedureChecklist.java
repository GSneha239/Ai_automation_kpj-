...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>ProcedureChecklist</b> — Page Object.
 *
 ...
 */
public class ProcedureChecklist extends BasePage {

    public ProcedureChecklist(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; ProcedureChecklist.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*procedure\\s*checklist\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__pchMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("ProcedureChecklist.nav: the Procedure Checklist link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__pchMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("ProcedureChecklist.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__pchMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("ProcedureChecklist.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the ProcedureChecklist screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("procedurechecklist") || route.contains("procchecklist")
    ...
    public String describeControls() {
        ...
        System.out.println("=== ProcedureChecklist CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?proc|AddProcedureChecklist|CHECKLIST/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__pchAdd'; }");
        try {
            page.locator("#__pchAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("ProcedureChecklist.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__pchAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterCodeAndRemark(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("ProcedureChecklist: " + lastEntryPinned);
    ...
    public String selectCategory(int seed) {
        Object info = page.evaluate("() => {" + JS
                ...
                + "   || sels[0];"
                + " if(!e) return null; e.id='__pchCategory';"
        ...
        try {
            page.locator("#__pchCategory").selectOption(
        ...
        waitForAngular(800);
        Object t = page.evaluate("() => { const e=document.getElementById('__pchCategory');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastCategory = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count")
                + " categories]";
        System.out.println("ProcedureChecklist: category -> " + lastCategory);
        return lastCategory;
    }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("ProcedureChecklist: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("ProcedureChecklist: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__pchSeen = [];"
                    + " if(window.__pchObs) window.__pchObs.disconnect();"
                    + " window.__pchObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__pchSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__pchObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__pchSeen||[];"
                    + " if(window.__pchObs) window.__pchObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("ProcedureChecklist: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("ProcedureChecklist: list check -> " + lastListCheck);
