 ...
 */
public class FunctionalChecklist extends BasePage {

    public FunctionalChecklist(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Equipment &rarr; FunctionalChecklist.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*functional\\s*check\\s*list\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__fncMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__fncMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("FunctionalChecklist.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__fncMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("FunctionalChecklist.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the FunctionalChecklist screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("functionalchecklist") || route.contains("functional")
    ...
    public String describeControls() {
        ...
        System.out.println("=== FunctionalChecklist CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?functional|AddFunctionalChecklist|FUNCTIONAL/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__fncAdd'; }");
        try {
            page.locator("#__fncAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("FunctionalChecklist.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__fncAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("FunctionalChecklist: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("FunctionalChecklist: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__fncMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__fncMims';"
        ...
        try {
            page.locator("#__fncMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__fncMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("FunctionalChecklist: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("FunctionalChecklist: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("FunctionalChecklist: list check -> " + lastListCheck);
