...
/**
 * Application Configuration &gt; Patient &gt; <b>Nationality</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Nationality</b> (route
 * {@code #/NATIONALITY}, confirmed live in a prior menu dump) → (Add, if the screen has one) → enter
 * <b>Code*</b> + <b>Nationality*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>Unlike its sibling CommonMaster screens, the second field's LABEL here reads "Nationality*" rather than
 * "Remark*" (the underlying ng-model is still expected to be {@code commonmaster.Description}) — the field
 * discovery below matches on that label too, in addition to the usual remark/description patterns.</p>
 *
 * <p>The Code / Nationality fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.MotherTongue} (this screen's exact ng-model names
...
public class Nationality extends BasePage {
    ...
    public Nationality(Page page) { super(page); }

    /** Confirmed live in a prior menu dump. */
    public static final String ROUTE = "#/NATIONALITY";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /**
     * Remarks are real nationality names, not generic filler text. A comprehensive-looking Nationality master
     * (confirmed live: it already has Malaysian/Singaporean/Indonesian/Filipino/Bangladeshi pre-loaded) needs
     * a longer pool than the other simple masters to reliably find one not yet used.
     */
    private static final String[] REMARKS = {
            "Australian", "British", "Canadian", "New Zealander", "South African", "Bruneian",
            "Cambodian", "Laotian", "Mongolian", "Fijian", "Papua New Guinean", "Tongan"
    };

    private static final int TOAST_SHOT_WAIT_MS = 5000;

    /**
     * JS helper: the visible Code / Nationality inputs, whatever they are called on this screen. The second
     * field's label can read "Nationality*" instead of "Remark*"/"Description*" — matched as a fallback.
     */
    private static final String FIND_FIELDS =
            ...
            + " const byLabel=re=>boxes.find(e=>re.test(labelOf(e)));"
            ...
            + "   || byLabel(/^\\s*nationality\\s*\\*?\\s*$/i) || byPh(/^\\s*nationality\\s*$/i);";
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("Nationality.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*nationality\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("Nationality.nav DEBUG: visible links => " + dbg);
                    System.out.println("Nationality.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                ...
                try { page.locator("#__ntMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("Nationality.nav: real click failed, dispatching in-page click - " + e.getMessage());
                ...
                System.out.println("Nationality.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Nationality.nav: direct route " + route);
    ...
    /**
     * On the Nationality screen — verified by a Code/Nationality box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/nationality/i.test(location.hash||'');" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/^\\s*nationality\\s*$/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("Nationality.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    /**
     * Enter <b>Code*</b> and <b>Nationality*</b>. {@code attempt} shifts BOTH values so a retry after an
     * "already exists" toast submits genuinely different details.
     */
    public String fillDetails(int attempt) {
        String code = "NT" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    /**
     * Click <b>Submit</b> (accepts "Save" too) and return the toast. Re-asserts Code/Nationality first — these
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("nationality") || resp.status() >= 400)) return;
                String body = "";
        ...
        System.out.println("Nationality submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("Nationality submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ntToasts||[]).some(a=>/nationality|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Nationality save HTTP => " + lastSaveHttp);
