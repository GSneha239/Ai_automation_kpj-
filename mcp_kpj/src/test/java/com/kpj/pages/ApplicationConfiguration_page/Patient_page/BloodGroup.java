...
/**
 * Application Configuration &gt; Patient &gt; <b>Blood Group</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Blood Group</b> → (Add, if
 ...
 */
public class BloodGroup extends BasePage {

    public BloodGroup(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/BloodGroup";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Real blood group names — the Remark IS the blood group itself, not generic "Auto ..." noise. */
    private static final String[] REMARKS = {
            "A Positive", "A Negative", "B Positive", "B Negative", "O Positive", "O Negative", "AB Positive", "AB Negative"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                // poll for ANY visible link before trusting the "not found" result (a previous BlackListReason
                // run saw only 2 leftover top-level links because it queried before the submenu had expanded).
                try {
                ...
                } catch (Exception ignore) { System.out.println("BloodGroup.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                ...
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        ...
                        + " const a = links.find(x=>/^\\s*blood\\s*group\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__bgMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("BloodGroup.nav DEBUG: visible links => " + dbg);
                    System.out.println("BloodGroup.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__bgMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("BloodGroup.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__bgMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__bgMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("BloodGroup.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("BloodGroup.nav: direct route " + route);
    ...
    /**
     * On the Blood Group screen — verified by the rendered HEADER text rather than a guessed URL substring
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/bloodgroup/i.test(location.hash||'');"
                + " if(!/blood\\s*group/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__bgAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__bgAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("BloodGroup.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__bgAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
     ...
     * "already exists" toast submits genuinely different details. The Remark is a real blood group name
     * ("A Positive", "O Negative", …), not generic filler text.
     */
    public String fillDetails(int attempt) {
        String code = "BG" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__bgToasts=[]; if(window.__bgObs) window.__bgObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__bgToasts.includes(t)) window.__bgToasts.push(t); }); };"
                + " window.__bgObs=new MutationObserver(grab); window.__bgObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("bloodgroup") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__bgSubmit'; return 'click'; }"
        ...
        System.out.println("BloodGroup submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__bgSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("BloodGroup submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__bgSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__bgToasts||[]).some(a=>/blood|group|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("BloodGroup save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__bgToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
