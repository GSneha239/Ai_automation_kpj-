...
/**
 * Application Configuration &gt; Patient &gt; <b>Patient Status</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Allergy</b>
 * ({@code #/PatientStatus}) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
...
public class PatientStatus extends BasePage {
    ...
    public PatientStatus(Page page) { super(page); }

    public static final String ROUTE = "#/PatientStatus";
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            // The menu label is "PatientStatus" (no space) — accept either spelling, or the exact href.
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*patient\\s*status\\s*$/i.test(norm(x.textContent)) || (x.getAttribute('href')||'')==='#/PatientStatus'); if(!a) return ''; a.id='__psMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("PatientStatus.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__psMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("PatientStatus.nav: click failed - " + e.getMessage()); }
            ...
            } catch (Exception ignore) { System.out.println("PatientStatus.nav: Allergy screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    /**
     * On the REAL Allergy screen. Checked by the screen's own <b>Remark</b> field
     * ({@code commonmaster.Description}) — NOT just the URL: the SPA can leave a previous screen mounted while the
     * hash reads {@code #/PatientStatus}, and the leftover markup carries a Code + Store commonmaster form that a
     * Code-only check would happily accept.
     */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("patientstatus")) return false;
    ...
    public String fillDetails() {
        String code = "PS" + String.format("%05d", Math.abs(System.nanoTime() % 100000));
        lastCode = code;
        lastRemark = new String[]{"Admitted","Discharged","Under Observation","Stable","Critical","Referred Out"}[(int)(Math.abs(System.nanoTime())%6)];
    ...
    public String submitAndGetToast() {
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            Object res = page.evaluate("(a) => { const A=window.angular; const code=a.code, desc=a.remark;"
                    ...
                    + " const b=[...document.querySelectorAll('button[type=submit],input[type=submit]')].find(x=>/^\\s*submit\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(b){ b.id='__psSubmit'; return 'click'; } return 'none'; }",
                    java.util.Map.of("code", lastCode, "remark", lastRemark));
            System.out.println("PatientStatus submit[" + attempt + "] => " + res);
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|required|exist|error/i.test(el.textContent||''))"
                        + " || (window.__psToasts||[]).some(a=>/status|master|saved|success|added|updated|please|enter|required|exist|error/i.test(a))",
