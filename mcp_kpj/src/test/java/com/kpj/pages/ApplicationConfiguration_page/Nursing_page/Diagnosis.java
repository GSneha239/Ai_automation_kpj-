...
/**
 * Application Configuration &gt; Nursing &gt; <b>Diagnosis</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Nursing</b> (submenu, EXACT text — "Nursing Station" is a
 * different module and a prefix match hits it first) → <b>Diagnosis</b> (route {@code #/Diagnosis}, best-guess
 * fallback — navigation is primarily by the live menu link) → (Add, if the screen has one — a no-op on
 * inline-add screens) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.Race} / {@code Nursing_page.AdviseMaster} (this
 ...
 * header text alone). Grid filter boxes ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class Diagnosis extends BasePage {

    public Diagnosis(Page page) { super(page); }

    /** Best-guess fallback — navigation is primarily by the live menu link, not this hardcoded route. */
    public static final String ROUTE = "#/Diagnosis";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /** Realistic diagnosis names, not "Auto diagnosis <code>" filler — cycled per retry attempt. */
    private static final String[] REMARKS = {
            "Hypertension", "Type 2 Diabetes Mellitus", "Acute Gastroenteritis", "Community Acquired Pneumonia",
            "Bronchial Asthma", "Chronic Kidney Disease", "Urinary Tract Infection", "Acute Appendicitis",
            "Migraine", "Osteoarthritis", "Dengue Fever", "Anaemia"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("Diagnosis.nav: Nursing submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT text match on a VISIBLE link ONLY — a loose match can pick up a sibling screen like
                // "Diagnosis Set" or "Diagnosis Complexity Category" instead.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*diagnosis\\s*$/i.test(norm(x.textContent)) && x.offsetParent!==null);"
                ...
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("Diagnosis.nav DEBUG: visible links => " + dbg);
                    System.out.println("Diagnosis.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                if (href.toString().startsWith("#")) menuHref = href.toString();
                try { page.locator("#__dgMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                catch (Exception e) {
                    System.out.println("Diagnosis.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__dgMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__dgMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("Diagnosis.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Diagnosis.nav: direct route " + route);
    ...
    /**
     * On the Diagnosis screen — verified by a Code/Remark box actually present plus the URL (the app's
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/diagnosis/i.test((location.hash||'').replace(/\\s+/g,''));" + FIND_FIELDS
                + " if(codeEl && remEl) return true;"
                + " if(/^\\s*diagnosis\\s*$/i.test(hdr)) return true;"
    ...
    public String clickAddIfPresent() {
        ...
        try { page.locator("#__dgAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("Diagnosis.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String fillDetails(int attempt) {
        String code = "DG" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__dgToasts=[]; if(window.__dgObs) window.__dgObs.disconnect();"
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("diagnosis") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__dgSubmit'; return 'click'; }"
        ...
        System.out.println("Diagnosis submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__dgSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Diagnosis submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__dgSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__dgToasts||[]).some(a=>/diagnosis|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Diagnosis save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__dgToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
