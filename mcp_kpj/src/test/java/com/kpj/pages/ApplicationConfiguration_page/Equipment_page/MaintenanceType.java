 ...
 */
public class MaintenanceType extends BasePage {

    public MaintenanceType(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; Equipment &rarr; MaintenanceType.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*maintenance\\s*type\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__mtMenu'; return a.getAttribute('href')||''; }");
        ...
        if (!lastRoute.isEmpty()) {
            ...
            catch (Exception e) { System.out.println("MaintenanceType.nav: " + e.getMessage().split("\n")[0]); }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("MaintenanceType.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the MaintenanceType screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("maintenancetype") || route.contains("maintenance")
    ...
    public String describeControls() {
        ...
        System.out.println("=== MaintenanceType CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?maintenance|AddMaintenanceType|MAINTENANCE/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__mtAdd'; }");
        try {
            page.locator("#__mtAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("MaintenanceType.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
    ...
    public String enterDetails(String code, String remark) {
        ...
        waitForAngular(500);
        System.out.println("MaintenanceType: " + lastEntry);
        return lastEntry;
    }
    ...
    public String enterMims(String guid, String description) {
        ...
        waitForAngular(500);
        System.out.println("MaintenanceType: " + lastMims);
        return lastMims;
    }
    ...
    public String selectMimsType() {
        ...
        Object info = page.evaluate("() => {" + JS
                + " document.querySelectorAll('#__mtMims').forEach(e=>e.removeAttribute('id'));"
                + " const e=[...document.querySelectorAll('select')].filter(vis)"
                + "   .find(s=>/mimstype/i.test(tail(s)));"
                + " if(!e) return null; e.id='__mtMims';"
        ...
        try {
            page.locator("#__mtMims").selectOption(
        ...
        waitForAngular(900);
        Object t = page.evaluate("() => { const e=document.getElementById('__mtMims');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastMimsType = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count") + " options]";
        System.out.println("MaintenanceType: MIMS Type = " + lastMimsType);
        return lastMimsType;
    }
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("MaintenanceType: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("MaintenanceType: list check -> " + lastListCheck);
