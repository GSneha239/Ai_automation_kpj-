...
/**
 * Application Configuration &gt; OT Configuration &gt; <b>SurgeryProcedureChecklist</b> — Page Object.
 *
 ...
 */
public class SurgeryProcedureChecklist extends BasePage {

    public SurgeryProcedureChecklist(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; OT Configuration &rarr; SurgeryProcedureChecklist.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            href = page.evaluate("() => {" + JS
                    ...
                    + "   .find(x=>/^\\s*surg(ery|ical)\\s*procedure\\s*check\\s*list\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__spcMenu'; return a.getAttribute('href')||''; }");
            if (href != null && !href.toString().isEmpty()) break;
            System.out.println("SurgeryProcedureChecklist.nav: the Surgery Procedure Checklist link was not in the menu on attempt "
                    + attempt + " — reopening the menu");
            page.waitForTimeout(1500);
        }
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__spcMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("SurgeryProcedureChecklist.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__spcMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    public boolean waitForScreenControls(int timeoutMs) {
        try {
            ...
            System.out.println("SurgeryProcedureChecklist.waitForScreenControls: no Code box or grid within "
    ...
    /**
     * On the SurgeryProcedureChecklist screen — matched on the hash route, never on page text.
     *
    ...
    public boolean onScreen() {
        ...
        if (route.contains("surgicalprocedure") || route.contains("surgeryprocedure")
                || route.contains("checklist")
    ...
    public String describeControls() {
        ...
        System.out.println("=== SurgeryProcedureChecklist CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public String openFormIfNeeded() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?surg|AddSurgeryProcedureChecklist|CHECKLIST/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__spcAdd'; }");
        try {
            page.locator("#__spcAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("SurgeryProcedureChecklist.openFormIfNeeded: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__spcAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(1500);
    ...
    // ---- OT Theatre's own fields ----------------------------------------

    public String lastEntryPinned = "", lastCategory = "";

    /**
     * Write a value to a field, pinned to {@code OTTheatre.<model>} where that exists and otherwise
     * matched on the model's LAST SEGMENT.
     *
     * <p>The fallback never matches on a keyword that also appears in the screen's own prefix — every
     * model here starts with the screen name, so "code" as a substring would match almost anything.</p>
     */
    private String put(String tail, String value) {
        Object r = page.evaluate("(a) => {" + JS
                ...
                + "   .filter(e=>!/colFilter|pagination|row\\.entity|search/i.test(ngOf(e)));"
                + " const exact=boxes.find(e=>ngOf(e)==='OTTheatre.'+a.t);"
                + " const byTail=boxes.find(e=>new RegExp('^'+a.t+'$','i').test(tail(e)));"
                + " const e=exact||byTail;"
                + " return e? setEl(e,a.v)+' ['+(ngOf(e)||'?')+']' : '(no field for '+a.t+')'; }",
                java.util.Map.of("t", tail, "v", value));
        return r == null ? "" : r.toString();
    }
    ...
    /** Enter the <b>Code</b> and the <b>Remark</b>. */
    public String enterCodeAndRemark(String code, String remark) {
        String r1 = put("code", code);
        String r2 = put("description", remark);
        if (r2.startsWith("(no field")) r2 = put("remark", remark);
        lastEntryPinned = "Code=" + r1 + " | Remark=" + r2;
        waitForAngular(500);
        System.out.println("SurgeryProcedureChecklist: " + lastEntryPinned);
        return lastEntryPinned;
    }

    public boolean codeAndRemarkEntered(String code, String remark) {
        return lastEntryPinned != null && !lastEntryPinned.contains("(no field")
                && lastEntryPinned.contains("Code=" + code)
                && lastEntryPinned.contains("Remark=" + remark);
    }

    /**
     * Select the <b>Category</b>.
     *
     * <p>Found by the model's last segment, or failing that by its label — never by a keyword against
     * the whole model, since every model here begins with the screen's own name. Which category is used
     * varies per run so repeated runs do not all point at the same one.</p>
     */
    public String selectCategory(int seed) {
        Object info = page.evaluate("() => {" + JS
                + " const sels=[...document.querySelectorAll('select')].filter(vis)"
                + "   .filter(e=>!/pagination/i.test(ngOf(e)));"
                + " const e=sels.find(x=>/category/i.test(tail(x)))"
                + "   || sels.find(x=>/category/i.test(labelOf(x)))"
                + "   || sels[0];"
                + " if(!e) return null; e.id='__spcCategory';"
                + " const reals=[...e.options].map((o,i)=>({o,i}))"
                + "   .filter(x=>x.o.value && !/^-*\\s*select/i.test(norm(x.o.text)));"
                + " return {ng:(ngOf(e)||'?'), count:reals.length, indexes:reals.map(x=>x.i)}; }");
        if (info == null) { lastCategory = "(no Category dropdown on this form)"; return lastCategory; }
        @SuppressWarnings("unchecked")
        java.util.Map<String, Object> m = (java.util.Map<String, Object>) info;
        java.util.List<?> idx = (java.util.List<?>) m.get("indexes");
        if (idx.isEmpty()) { lastCategory = "(the Category list has no real option) [" + m.get("ng") + "]"; return lastCategory; }
        int pick = ((Number) idx.get(Math.floorMod(seed, idx.size()))).intValue();
        try {
            page.locator("#__spcCategory").selectOption(
                    new com.microsoft.playwright.options.SelectOption().setIndex(pick));
        } catch (Exception e) {
            lastCategory = "(select failed: " + e.getMessage().split("\n")[0] + ")";
            return lastCategory;
        }
        waitForAngular(800);
        Object t = page.evaluate("() => { const e=document.getElementById('__spcCategory');"
                + " return e? ((e.options[e.selectedIndex]||{}).text||'').trim() : ''; }");
        lastCategory = (t == null ? "" : t.toString()) + " [" + m.get("ng") + ", " + m.get("count")
                + " categories]";
        System.out.println("SurgeryProcedureChecklist: category -> " + lastCategory);
        return lastCategory;
    }

    public boolean categorySelected() { return chosen(lastCategory); }

    public String lastAnswer = "";

    /**
     * Enter the <b>Answer</b>.
     *
     * <p>Addressed by the model's last segment, and the Code and Remark boxes are excluded by name so a
     * broad match cannot write the answer over one of them.</p>
     */
    public String enterAnswer(String answer) {
        Object r = page.evaluate("(v) => {" + JS
                + " const boxes=[...document.querySelectorAll('input,textarea')].filter(vis)"
                + "   .filter(e=>e.type!=='hidden' && e.type!=='checkbox' && e.type!=='radio')"
                + "   .filter(e=>!/colFilter|pagination|row\\.entity|search/i.test(ngOf(e)))"
                + "   .filter(e=>!/^(code|description|remark)$/i.test(tail(e)));"
                + " const e=boxes.find(x=>/^answer/i.test(tail(x)))"
                + "   || boxes.find(x=>/answer/i.test(tail(x)+' '+(x.placeholder||'')+' '+labelOf(x)));"
                + " return e? setEl(e,v)+' ['+(ngOf(e)||'?')+']'"
                + "   : '(no Answer field. The boxes on this form: '"
                + "     +boxes.map(x=>ngOf(x)||'?').join(', ')+')'; }", answer);
        lastAnswer = r == null ? "" : r.toString();
        waitForAngular(400);
        System.out.println("SurgeryProcedureChecklist: answer -> " + lastAnswer);
        return lastAnswer;
    }

    public boolean answerEntered(String answer) {
        return lastAnswer != null && !lastAnswer.startsWith("(no ") && lastAnswer.startsWith(answer);
    }

    public String lastDetailRemark = "", lastAnswerAdd = "";
    private int answerLinesBefore = -1, answerLinesAfter = -1;

    /**
     * Enter the detail <b>Remark</b> ({@code SurgicalProcedureCheckList1.remark}).
     *
     * <p>This form carries TWO remarks: the record's own ({@code SurgicalProcedureCheckList.description},
     * beside the Code) and this one, which belongs to the answer line. They are told apart by their
     * model, since both read "remark" on screen.</p>
     */
    public String enterDetailRemark(String remark) {
        Object r = page.evaluate("(v) => {" + JS
                + " const e=[...document.querySelectorAll("
                + "   \"[ng-model='SurgicalProcedureCheckList1.remark']\")].filter(vis)[0];"
                + " return e? setEl(e,v)+' [SurgicalProcedureCheckList1.remark]'"
                + "   : '(no detail remark field)'; }", remark);
        lastDetailRemark = r == null ? "" : r.toString();
        waitForAngular(400);
        System.out.println("SurgeryProcedureChecklist: detail remark -> " + lastDetailRemark);
        return lastDetailRemark;
    }

    public boolean detailRemarkEntered(String remark) {
        return lastDetailRemark != null && !lastDetailRemark.startsWith("(no ")
                && lastDetailRemark.startsWith(remark);
    }

    /** How many answer lines the controller holds — authoritative, unlike counting rendered rows. */
    private int answerLines() {
        Object n = page.evaluate("() => {" + JS
                + " const e=[...document.querySelectorAll("
                + "   \"[ng-model='SurgicalProcedureCheckList1.answer']\")].filter(vis)[0];"
                + " if(!e) return -1;"
                + " try{ const l=angular.element(e).scope().$eval('AnswerRemarklist')||[];"
                + "      return l.length; }catch(err){ return -1; } }");
        return n instanceof Number ? ((Number) n).intValue() : -1;
    }

    /**
     * Click <b>Add</b> ({@code AddSurgicalProcedureCheckList}) to put the answer and its remark into the
     * screen's answer list.
     *
     * <p>Counted from the controller's own {@code AnswerRemarklist} rather than from rendered rows, and
     * the click is never credited on its own.</p>
     */
    public String addAnswerLine() {
        answerLinesBefore = answerLines();
        page.evaluate("() => {" + JS
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].filter(vis)"
                + "   .find(x=>/AddSurgicalProcedureCheckList/i.test(x.getAttribute('ng-click')||''));"
                + " if(b) b.click(); }");
        waitForAngular(2000);
        answerLinesAfter = answerLines();
        lastAnswerAdd = "the controller's answer lines " + answerLinesBefore + " -> " + answerLinesAfter
                + (answerLinesAfter > answerLinesBefore ? "" : "; NO line was added");
        System.out.println("SurgeryProcedureChecklist: add answer -> " + lastAnswerAdd);
        return lastAnswerAdd;
    }

    public boolean answerLineAdded() { return answerLinesAfter > answerLinesBefore; }
    ...
    public String confirmSave() {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!s2.isEmpty()) {
                waitForAngular(1200);
                System.out.println("SurgeryProcedureChecklist: confirm -> " + s2);
                return s2;
            }
            page.waitForTimeout(400);
        }
        System.out.println("SurgeryProcedureChecklist: confirm -> no confirmation dialog appeared within 4s");
    ...
    public String submitAndGetToast() {
        ...
        try {
            page.evaluate("() => { window.__spcSeen = [];"
                    + " if(window.__spcObs) window.__spcObs.disconnect();"
                    + " window.__spcObs = new MutationObserver(ms=>{"
                    + "   for(const m of ms){ for(const n of m.addedNodes){"
                    + "     if(n.nodeType!==1) continue;"
                    + "     const t=(n.textContent||'').replace(/\\s+/g,' ').trim();"
                    + "     const cls=String(n.className||'');"
                    + "     if(t && t.length<200 && /modal|alert|dialog|toast|jAlert|sweet/i.test(cls+' '+n.id))"
                    + "       window.__spcSeen.push(cls+'#'+(n.id||'')+' :: '+t.slice(0,140)); } } });"
                    + " window.__spcObs.observe(document.body,{childList:true,subtree:true}); }");
        } catch (Exception ignore) { }
        ...
        String seen;
        try {
            Object s3 = page.evaluate("() => { const a=window.__spcSeen||[];"
                    + " if(window.__spcObs) window.__spcObs.disconnect();"
        ...
        lastSaveDiagnostics = (clicked == null ? "" : clicked.toString())
                ...
                + (toastFromObserver
                    ...
                    : "");
        lastToast = toast;
        System.out.println("SurgeryProcedureChecklist: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String remark) {
        ...
        System.out.println("SurgeryProcedureChecklist: list check -> " + lastListCheck);
