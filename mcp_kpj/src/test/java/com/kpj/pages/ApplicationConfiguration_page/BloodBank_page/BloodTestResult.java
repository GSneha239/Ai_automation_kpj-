...
/**
 * Application Configuration &gt; Blood Bank &gt; <b>Blood Test Result</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Blood Bank</b> → <b>Blood Test Result</b> → (Add, if the
 * screen has one) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>Distinct from the existing {@code Blood Test Master} screen (different master; verified no clash by
 * name before creating this one). The Code / Remark fields are DISCOVERED at runtime rather than hard-coded
 * — same reusable shape as {@code Investigation_page.Agency} / {@code Billing_page.BillStatus} /
 * {@code BloodBank_page.BloodDiscardReason} / {@code BloodBank_page.BloodDonorType} (this screen's exact
 * route and ng-model names were not yet confirmed live; sibling masters can live at routes that do not
 * literally match their menu label — e.g. Cancellation Reason is {@code #/CANCELLATION} — so
 * {@link #onScreen()} verifies by the rendered HEADER text, not a guessed URL substring). Grid filter boxes
 * ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class BloodTestResult extends BasePage {

    public BloodTestResult(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/BloodTestResult";
    ...
    private static final String[] REMARKS = {
            "Reactive", "Non Reactive", "Positive", "Negative", "Indeterminate"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            // spelled menu item on the Billing submenu (see CancellationReason.nav's "Receipt Cancellation"
            // trap). Also anchored so "Blood Test Result" never matches the existing "Blood Test Master" link.
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    + " const links=[...document.querySelectorAll('a[href]')];"
                    + " const a = links.find(x=>/^\\s*blood\\s*test\\s*result\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__btrMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("BloodTestResult.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            if (href.toString().startsWith("#")) menuHref = href.toString();
            try { page.locator("#__btrMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("BloodTestResult.nav: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__btrMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(2500);
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("BloodTestResult.nav: direct route " + route);
    ...
    /**
     * On the Blood Test Result screen — verified by the rendered HEADER text rather than a guessed URL
     * substring (sibling masters can live at routes that do not literally match their menu label — see the
     * class doc). Anchored so this never matches "Blood Test Master"'s header. Plus a Code/Remark box or an
    ...
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                ...
                + " const urlOk=/bloodtestresult/i.test(location.hash||'');"
                + " if(!/blood\\s*test\\s*result/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        boolean tagged = Boolean.TRUE.equals(page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const b=[...document.querySelectorAll('button,a,input[type=button]')].find(x=>/^\\s*add\\s*$/i.test(norm(x.textContent||x.value)) && x.offsetParent!==null); if(!b) return false; b.id='__btrAdd'; return true; }"));
        if (!tagged) return "no Add button and no inline form";
        try { page.locator("#__btrAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
        catch (Exception e) { System.out.println("BloodTestResult.clickAddIfPresent: click failed - " + e.getMessage()); }
        page.evaluate("() => { const e=document.getElementById('__btrAdd'); if(e) e.removeAttribute('id'); }");
        waitForAngular(2000);
    ...
    public String fillDetails(int attempt) {
        String code = "TR" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__btrToasts=[]; if(window.__btrObs) window.__btrObs.disconnect();"
                ...
                + " const grab=()=>{ document.querySelectorAll('.toast-message,.toast,[id^=toast]').forEach(el=>{ const t=(el.textContent||'').replace(/\\s+/g,' ').trim(); if(t && !window.__btrToasts.includes(t)) window.__btrToasts.push(t); }); };"
                + " window.__btrObs=new MutationObserver(grab); window.__btrObs.observe(document.body,{childList:true,subtree:true}); grab(); }");
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("bloodtestresult") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__btrSubmit'; return 'click'; }"
        ...
        System.out.println("BloodTestResult submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__btrSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("BloodTestResult submit: click failed - " + e.getMessage()); }
            page.evaluate("() => { const e=document.getElementById('__btrSubmit'); if(e) e.removeAttribute('id'); }");
        }
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__btrToasts||[]).some(a=>/test|result|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("BloodTestResult save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__btrToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
