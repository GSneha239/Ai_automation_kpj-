...
/**
 * Application Configuration &gt; General &gt; <b>Team</b> — Page Object.
 *
 * <p>Flow: list → <b>Add</b> → enter <b>Code</b> and <b>Description</b> → <b>Submit</b> → success
 * toast.</p>
 *
 * <p>Both boxes are addressed by the <b>last segment</b> of the ng-model, and the Description lookup
 * excludes the box already claimed by the Code, so one value cannot be written over the other — the
 * defect that put the area name into the Code field on Area/Town while the run still reported a pass.</p>
 */
public class Team extends BasePage {

    public Team(Page page) { super(page); }
    ...
    /**
     * Application Configuration &rarr; General &rarr; Team.
     *
    ...
    public boolean navigateViaMenu(String baseUrl) {
        ...
        Object href = page.evaluate("() => {" + JS
                ...
                + "   .find(x=>/^\\s*team\\s*$/i.test(norm(x.textContent)));"
                + " if(!a) return ''; a.id='__tmMenu'; return a.getAttribute('href')||''; }");
        lastRoute = href == null ? "" : href.toString();
        if (!lastRoute.isEmpty()) {
            try { page.locator("#__tmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("Team.nav: " + e.getMessage().split("\n")[0]); }
            page.evaluate("() => { const e=document.getElementById('__tmMenu'); if(e) e.removeAttribute('id'); }");
            waitForAngular(3000);
        }
    ...
    /** On the Team screen — matched on the hash route, never on page text. */
    ...
    public String describeControls() {
        ...
        System.out.println("=== Team CONTROLS ===\n" + lastControls);
        return lastControls;
    }
    ...
    public boolean clickAdd() {
        ...
        page.evaluate("() => {" + JS
                ...
                + "        || /add-?team|AddTeam/i.test((x.getAttribute('href')||'')"
                + "             +' '+(x.getAttribute('ng-click')||'')));"
                + " if(b) b.id='__tmAdd'; }");
        try {
            page.locator("#__tmAdd").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000));
        } catch (Exception e) {
            System.out.println("Team.clickAdd: " + e.getMessage().split("\n")[0]);
        }
        page.evaluate("() => { const e=document.getElementById('__tmAdd'); if(e) e.removeAttribute('id'); }");
        try {
        ...
        } catch (Exception ignore) { System.out.println("Team.clickAdd: Code field never appeared"); }
        waitForAngular(900);
        return formOpen();
    }
    ...
    /**
     * Enter <b>Code</b> and <b>Description</b>.
     *
     * <p>The Description is searched for among the boxes NOT already claimed by the Code, and by the
     * model's last segment — never by "team", which prefixes every model on this screen.</p>
     */
    public String enterDetails(String code, String description) {
        Object r = page.evaluate("(a) => {" + JS
                ...
                + " const descEl=rest.find(e=>/^(description|teamname|name|remark)$/i.test(tail(e)))"
                + "   || rest.find(e=>/(description|name)$/i.test(tail(e)))"
                + "   || rest.find(e=>/description|remark|name/i.test((e.placeholder||'')+' '+labelOf(e)));"
        ...
        waitForAngular(500);
        System.out.println("Team: " + lastEntry);
        return lastEntry;
    }

    /** Both boxes hold their OWN value — the check that catches one written over the other. */
    ...
    public String submitAndGetToast() {
        ...
        lastToast = toast;
        System.out.println("Team: submit -> " + lastSaveDiagnostics + " toast=\"" + toast + "\"");
        return toast;
    }
    ...
    public boolean codeInList(String code, String description) {
        ...
        System.out.println("Team: list check -> " + lastListCheck);
