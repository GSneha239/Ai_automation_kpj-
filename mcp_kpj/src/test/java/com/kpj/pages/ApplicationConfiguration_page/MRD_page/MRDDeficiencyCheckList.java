...
/**
 * Application Configuration &gt; MRD &gt; <b>MRD Deficiency CheckList</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>MRD</b> (submenu) → <b>MRD Deficiency CheckList</b>
 * ({@code #/MRDdeficiencyCheckList}) → enter <b>Code*</b> ({@code commonmaster.Code}) + <b>Remark*</b>
 ...
 * ng-submit="fnIUDCommonMaster()">}) — the same screen shape as MRD's {@link FileType} and the Nursing
 * Advise/Allergy/Bifocals masters. It also carries a mandatory <b>Form Name*</b> select (which arrives
 ...
 *
 * <p>The route's own casing is irregular — {@code #/MRDdeficiencyCheckList} (lowercase "d") — so the URL check is
 * case-insensitive.</p>
 */
public class MRDDeficiencyCheckList extends BasePage {

    public MRDDeficiencyCheckList(Page page) { super(page); }

    public static final String ROUTE = "#/MRDdeficiencyCheckList";
    ...
    private static final String[] REMARKS = {
            "Missing Discharge Summary", "Unsigned Operation Notes", "Incomplete Consent Form",
            "Missing Progress Notes", "Unsigned Doctor Order", "Missing Nursing Assessment"};
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            waitForAngular(1500);
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); document.querySelectorAll('#__dcMenu').forEach(e=>e.removeAttribute('id')); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*mrd\\s*deficiency\\s*check\\s*list\\s*$/i.test(norm(x.textContent)) || /mrddeficiencychecklist/i.test(x.getAttribute('href')||'')); if(!a) return ''; a.id='__dcMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("MRDDeficiencyCheckList.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            try { page.locator("#__dcMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(4000)); }
            catch (Exception e) { page.evaluate("() => { const a=document.getElementById('__dcMenu'); if(a) a.click(); }"); }
            ...
            } catch (Exception ignore) { System.out.println("MRDDeficiencyCheckList.nav: screen not confirmed (attempt " + (attempt + 1) + ")"); }
            waitForAngular(1500);
        }
        return onScreen();
    }
    ...
    /**
     * On the REAL MRD Deficiency CheckList screen — verified by this screen's own <b>Remark</b> field
    ...
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("mrddeficiencychecklist")) return false;
    ...
    public String fillDetails() {
        String code = "DC" + String.format("%05d", Math.abs(System.nanoTime() % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        page.evaluate("() => { window.__dcToasts=[]; if(window.__dcObs) window.__dcObs.disconnect();"
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            Object res = page.evaluate("(a) => { const A=window.angular; const code=a.code, desc=a.remark;"
                    ...
                    + " const b=[...document.querySelectorAll('button[type=submit],input[type=submit],button')].find(x=>/^\\s*submit\\s*$/i.test((x.textContent||x.value||'').trim()) && x.offsetParent!==null); if(b){ b.id='__dcSubmit'; return 'click'; } return 'none'; }",
                    java.util.Map.of("code", lastCode, "remark", lastRemark));
            System.out.println("MRDDeficiencyCheckList submit[" + attempt + "] => " + res);
            if ("click".equals(String.valueOf(res))) {
                try { page.locator("#__dcSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(5000)); } catch (Exception ignore) { }
            ...
            try {
                page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(el.textContent||''))"
                        + " || (window.__dcToasts||[]).some(a=>/deficiency|master|saved|success|added|updated|please|enter|select|required|exist|error|not found/i.test(a))",
                        null, new Page.WaitForFunctionOptions().setTimeout(9000));
            } catch (Exception ignore) { page.waitForTimeout(1500); }
            captureToastShot();
            Object t = page.evaluate("() => { const a=window.__dcToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>x) || ''; }");
            ...
            page.evaluate("() => { window.__dcToasts=[]; document.querySelectorAll('.toast-message,.toast').forEach(t=>t.remove()); }");
        }
        return last;
    }
}
