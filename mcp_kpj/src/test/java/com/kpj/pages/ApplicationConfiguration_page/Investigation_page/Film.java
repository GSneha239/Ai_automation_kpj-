...
/**
 * Application Configuration &gt; Investigation &gt; <b>Film</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Investigation</b> (submenu) → <b>Film</b> → (Add, if the
...
public class Film extends BasePage {
    ...
    public Film(Page page) { super(page); }

    public static final String ROUTE = "#/Film";
    public String lastCode = "", lastRemark = "", lastCodeModel = "", lastRemarkModel = "";
    private String menuHref = "";
    public byte[] toastPng;

    /**
     * Remarks are FILM SIZES in the screen's own notation (user: {@code 10"x12"} "like this"), not machine noise.
     * The value is passed to the browser as an evaluate ARGUMENT, so the embedded double quotes need no escaping
     * beyond Java's own.
     */
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            try {
                ...
                waitForAngular(1500);
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim(); const a=[...document.querySelectorAll('a[href]')].find(x=>/^\\s*film\\s*$/i.test(norm(x.textContent)) || /(^|\\/)film/i.test(x.getAttribute('href')||'')); if(!a) return ''; a.id='__fmMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) { System.out.println("Film.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
                ...
                try { page.locator("#__fmMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("Film.nav: real click failed, dispatching in-page click - " + e.getMessage());
                ...
                System.out.println("Film.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("Film.nav: direct route " + route);
    ...
    /** On the Film screen — URL plus something real on the page (a Code/Remark box or an Add button). */
    public boolean onScreen() {
        if (!page.url().toLowerCase().contains("film")) return false;
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("Film.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String fillDetails(int attempt) {
        String code = "FM" + String.format("%05d", Math.abs((System.nanoTime() + attempt * 7919L) % 100000));
        lastCode = code;
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("film") || resp.status() >= 400)) return;
                String body = "";
        ...
        System.out.println("Film submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("Film submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__fmToasts||[]).some(a=>/film|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("Film save HTTP => " + lastSaveHttp);
