...
/**
 * Application Configuration &gt; Patient &gt; <b>Camp Type</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Patient</b> (submenu) → <b>Camp Type</b> → (Add, if
 * the screen has one) → enter <b>Code*</b> + <b>Remark*</b> → <b>Submit</b> → success toast.</p>
 *
 * <p>Not to be confused with {@code Patient_page.CampDetails}, whose form has a {@code CampDetails.CampTypeID}
 * SELECT that this screen's own entries populate — this screen is the master list feeding that dropdown, a
 * separate simple Code+Remark form.</p>
 *
 * <p>The Code / Remark fields are DISCOVERED at runtime rather than hard-coded — same reusable shape as
 * {@code Investigation_page.Agency} / {@code Patient_page.BookingType} (this screen's exact route and
 ...
 */
public class CampType extends BasePage {

    public CampType(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/CampType";
    ...
    private static final String[] REMARKS = {
            "Corporate Health Screening", "School Health Camp", "Community Blood Donation Drive",
            "Free Eye Screening Camp", "Diabetes Awareness Camp", "Vaccination Drive"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            try {
                ...
                } catch (Exception ignore) { System.out.println("CampType.nav: Patient submenu did not expand in time (attempt " + (attempt + 1) + ")"); }
                // EXACT text match ONLY, ANCHORED — "Camp Type" must not match "Camp Details" on the same submenu.
                Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                        + " const links=[...document.querySelectorAll('a[href]')];"
                        + " const a = links.find(x=>/^\\s*camp\\s*type\\s*$/i.test(norm(x.textContent)));"
                        + " if(!a) return ''; a.id='__ctMenu'; return a.getAttribute('href')||'link'; }");
                if (href == null || href.toString().isEmpty()) {
                    ...
                    System.out.println("CampType.nav DEBUG: visible links => " + dbg);
                    System.out.println("CampType.nav: menu link not found (attempt " + (attempt + 1) + ")");
                    continue;
                }
                ...
                try { page.locator("#__ctMenu").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
                    ...
                    System.out.println("CampType.nav: real click failed, dispatching in-page click - " + e.getMessage());
                    try { page.evaluate("() => { const a=document.getElementById('__ctMenu'); if(a) a.click(); }"); }
                    catch (Exception ignore) { }
                }
                page.evaluate("() => { const e=document.getElementById('__ctMenu'); if(e) e.removeAttribute('id'); }");
                waitForAngular(2500);
            } catch (Exception e) {
                System.out.println("CampType.nav: menu entry raced the SPA route change (attempt " + (attempt + 1) + ") - " + e.getMessage());
                waitForAngular(2000);
            }
        }
        if (!onScreen()) {
            String route = menuHref.isEmpty() ? ROUTE : menuHref;
            System.out.println("CampType.nav: direct route " + route);
    ...
    /**
     * On the Camp Type screen — confirmed live to be a CommonMaster-family screen (route {@code #/CAMPTYP},
     * same family as {@code Investigation_page.Result}) whose header selector picks up adjacent "Welcome:
     * Employee ..." text alongside the real "Camp Type" title, so the check is a plain substring match rather
     * than an exact-equals anchor — safe here since "Camp Type" and the sibling "Camp Details" screen differ
     * by their second word, so a substring match cannot confuse the two.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " const urlOk=/camptype/i.test(location.hash||'');"
                + " if(!/camp\\s*type/i.test(hdr) && !urlOk) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("CampType.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("camptype") || resp.status() >= 400)) return;
                String body = "";
        ...
        Object how = page.evaluate("(a) => { const A=window.angular; const norm=s=>(s||'').replace(/\\s+/g,' ').trim();" + FIND_FIELDS
                ...
                + " if(b){ b.id='__ctSubmit'; return 'click'; }"
        ...
        System.out.println("CampType submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            try { page.locator("#__ctSubmit").click(new com.microsoft.playwright.Locator.ClickOptions().setTimeout(6000)); }
            catch (Exception e) { System.out.println("CampType submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ctToasts||[]).some(a=>/camp|type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("CampType save HTTP => " + lastSaveHttp);
        Object t = page.evaluate("() => { const a=window.__ctToasts||[]; return a.find(x=>/saved|added|success/i.test(x)) || a.find(x=>/exist|already/i.test(x)) || a.find(x=>x) || ''; }");
