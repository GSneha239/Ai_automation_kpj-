...
/**
 * Application Configuration &gt; Billing &gt; <b>Code Type</b> — configuration screen Page Object.
 *
 * <p>Flow: <b>Application Configuration</b> → <b>Billing</b> → <b>Code Type</b> → (Add, if the screen has
 ...
 * (this screen's exact route and ng-model names were not yet confirmed live; sibling masters on this Billing
 * submenu turned out to live on the shared CommonMaster "Form Name" screen at an UPPERCASE route that does not
 * literally match the menu label — e.g. Cancellation Reason is {@code #/CANCELLATION} — so {@link #onScreen()}
 * verifies by the rendered HEADER text, not a guessed URL substring). Grid filter boxes
 * ({@code colFilter.term}) and the textAngular artefact are excluded.</p>
 */
public class CodeType extends BasePage {

    public CodeType(Page page) { super(page); }

    /** Best-effort direct-route fallback if the menu link cannot be found at all. */
    public static final String ROUTE = "#/CODETYPE";
    ...
    private static final String[] REMARKS = {
            "Service Code", "Package Code", "Discount Code", "Promotion Code", "Insurance Code"
    };
    ...
    public boolean navigateViaMenu() {
        for (int attempt = 0; attempt < 3 && !onScreen(); attempt++) {
            ...
            Object href = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                    ...
                    + " const a = links.find(x=>/^\\s*code\\s*type\\s*$/i.test(norm(x.textContent)));"
                    + " if(!a) return ''; a.id='__ctMenu'; return a.getAttribute('href')||'link'; }");
            if (href == null || href.toString().isEmpty()) { System.out.println("CodeType.nav: menu link not found (attempt " + (attempt + 1) + ")"); continue; }
            ...
            catch (Exception e) { System.out.println("CodeType.nav: click failed - " + e.getMessage()); }
        ...
        if (!onScreen()) {
            ...
            System.out.println("CodeType.nav: direct route " + route);
    ...
    /**
     * On the Code Type screen — verified by the rendered HEADER text ("Code Type …") rather than a guessed URL
     * substring (sibling masters on this submenu live at routes that do not literally match their menu label —
     * see the class doc), plus a Code/Remark box or an Add button actually present.
     */
    public boolean onScreen() {
        Object r = page.evaluate("() => { const norm=s=>(s||'').replace(/\\s+/g,' ').trim();"
                + " const hdr=norm((document.querySelector('.content-header,.main-header,h1,section.content-header')||{}).textContent||'');"
                + " if(!/code\\s*type/i.test(hdr)) return false;" + FIND_FIELDS
    ...
    public String clickAddIfPresent() {
        ...
        catch (Exception e) { System.out.println("CodeType.clickAddIfPresent: click failed - " + e.getMessage()); }
    ...
    public String submitAndGetToast() {
        ...
        java.util.function.Consumer<com.microsoft.playwright.Response> onResp = resp -> {
            try {
                if (!(resp.url().toLowerCase().contains("codetype") || resp.status() >= 400)) return;
                String body = "";
        ...
        System.out.println("CodeType submit => " + how);
        if ("click".equals(String.valueOf(how))) {
            ...
            catch (Exception e) { System.out.println("CodeType submit: click failed - " + e.getMessage()); }
        ...
        try {
            page.waitForFunction("() => [...document.querySelectorAll('.toast-message,.toast,[id^=toast]')].some(el=>el.offsetParent!==null && /saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(el.textContent||''))"
                    + " || (window.__ctToasts||[]).some(a=>/type|master|saved|success|added|updated|please|enter|select|required|exist|already|error/i.test(a))",
                    null, new Page.WaitForFunctionOptions().setTimeout(15000));
        ...
        System.out.println("CodeType save HTTP => " + lastSaveHttp);
